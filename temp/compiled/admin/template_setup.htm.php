...
<div class="list-div">
  <form name="theForm" action="template.php" method="post">
    <table width="100%" cellpadding="3" cellspacing="1">
      ...
      <?php $_from = $this->_var['temp_options'];
      if (!is_array($_from) && !is_object($_from)) {
        settype($_from, 'array');
      };
      $this->push_vars('lib_name', 'library');
      if (count($_from)) :
      ...
      unset($_from); ?><?php $this->pop_vars();; ?>
          ...
          <?php $_from = $this->_var['cate_goods'];
          if (!is_array($_from) && !is_object($_from)) {
            settype($_from, 'array');
          };
          $this->push_vars('lib_name', 'library');
          ...
          ?>
              <tr>
                ...
                <td><select name="regions[cat_goods][]">
                    <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('values' => $this->_var['temp_regions'], 'output' => $this->_var['temp_regions'], 'selected' => $this->_var['library']['region'])); ?>
                  </select></td>
                <td><input type="text" name="sort_order[cat_goods][]" value="<?php echo $this->_var['library']['sort_order']; ?>" size="4" /></td>
                <td><select name="categories[cat_goods][]">
                    <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->_var['library']['cats']; ?>
                  </select></td>
          ...
          unset($_from); ?><?php $this->pop_vars();; ?>
              ...
              <?php $_from = $this->_var['brand_goods'];
              if (!is_array($_from) && !is_object($_from)) {
                settype($_from, 'array');
              };
              $this->push_vars('lib_name', 'library');
              ...
              ?>
                  <tr>
                    <td class="first-cell" align="right"><a href="javascript:;" onclick="removeRow(this)">[-]</a></td>
                    <td><select name="regions[brand_goods][]">
                        <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('values' => $this->_var['temp_regions'], 'output' => $this->_var['temp_regions'], 'selected' => $this->_var['library']['region'])); ?>
                      </select></td>
                    <td><input type="text" name="sort_order[brand_goods][]" value="<?php echo $this->_var['library']['sort_order']; ?>" size="4" /></td>
                    <td><select name="brands[brand_goods][]">
                        <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('options' => $this->_var['arr_brands'], 'selected' => $this->_var['library']['brand'])); ?>
                      </select></td>
              ...
              unset($_from); ?><?php $this->pop_vars();; ?>
                  ...
                  <?php $_from = $this->_var['cat_articles'];
                  if (!is_array($_from) && !is_object($_from)) {
                    settype($_from, 'array');
                  };
                  $this->push_vars('lib_name', 'library');
                  ...
                  ?>
                      <tr>
                        <td class="first-cell" align="right"><a href="javascript:;" onclick="removeRow(this)">[-]</a></td>
                        <td><select name="regions[cat_articles][]">
                            <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('values' => $this->_var['temp_regions'], 'output' => $this->_var['temp_regions'], 'selected' => $this->_var['library']['region'])); ?>
                          </select></td>
                        <td><input type="text" name="sort_order[cat_articles][]" value="<?php echo $this->_var['library']['sort_order']; ?>" size="4" /></td>
                        <td><select name="article_cat[cat_articles][]">
                            <option value="0"><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->_var['library']['cat']; ?>
                          </select></td>
                  ...
                  unset($_from); ?><?php $this->pop_vars();; ?>
                      ...
                      <?php $_from = $this->_var['ad_positions'];
                      if (!is_array($_from) && !is_object($_from)) {
                        settype($_from, 'array');
                      };
                      $this->push_vars('lib_name', 'ad_position');
                      if (count($_from)) :
                        foreach ($_from as $this->_var['lib_name'] => $this->_var['ad_position']) :
                      ?>
                          <tr>
                            <td class="first-cell" align="right"><a href="javascript:;" onclick="removeRow(this)">[-]</a></td>
                            <td><select name="regions[ad_position][]">
                                <option value=""><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('values' => $this->_var['temp_regions'], 'output' => $this->_var['temp_regions'], 'selected' => $this->_var['ad_position']['region'])); ?>
                              </select></td>
                            ...
                            <td><select name="ad_position[]">
                                <option value="0"><?php echo $this->_var['lang']['select_plz']; ?></option><?php echo $this->html_options(array('options' => $this->_var['arr_ad_positions'], 'selected' => $this->_var['ad_position']['ad_pos'])); ?>
                              </select></td>
...
<script language="JavaScript">
  ...
  <?php $_from = $this->_var['temp_regions'];
  if (!is_array($_from) && !is_object($_from)) {
    settype($_from, 'array');
  };
  $this->push_vars('id', 'region');
  if (count($_from)) :
  ...
  unset($_from); ?><?php $this->pop_vars();; ?>
      ...
      <?php $_from = $this->_var['arr_brands'];
      if (!is_array($_from) && !is_object($_from)) {
        settype($_from, 'array');
      };
      $this->push_vars('id', 'brand');
      if (count($_from)) :
      ...
      unset($_from); ?><?php $this->pop_vars();; ?>
          ...
          <?php $_from = $this->_var['arr_article_cats'];
          if (!is_array($_from) && !is_object($_from)) {
            settype($_from, 'array');
          };
          $this->push_vars('id', 'cat');
          if (count($_from)) :
          ...
          unset($_from); ?><?php $this->pop_vars();; ?>
              ...
              <?php $_from = $this->_var['arr_ad_positions'];
              if (!is_array($_from) && !is_object($_from)) {
                settype($_from, 'array');
              };
              $this->push_vars('id', 'ad_position');
              if (count($_from)) :
