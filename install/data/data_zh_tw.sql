-- phpMyAdmin SQL Dump
-- version 2.8.0.3
-- http://www.phpmyadmin.net
--
-- 主機: localhost
-- 生成日期: 2006 年 11 月 02 日 16:55
-- 服務器版本: 3.23.58
-- PHP 版本: 4.4.2
--
-- 數據庫: `ecs_liuw`
--
--
-- 導出表中的數據 `ecs_admin_action`
--
INSERT INTO `ecs_admin_action` (
    `action_id`,
    `parent_id`,
    `action_code`,
    `relevance`
  )
VALUES (1, 0, 'goods', ''),
  (2, 0, 'cms_manage', ''),
  (3, 0, 'users_manage', ''),
  (4, 0, 'priv_manage', ''),
  (5, 0, 'sys_manage', ''),
  (6, 0, 'order_manage', ''),
  (7, 0, 'promotion', ''),
  (8, 0, 'email', ''),
  (9, 0, 'templates_manage', ''),
  (10, 0, 'db_manage', ''),
  (11, 0, 'sms_manage', ''),
  (21, 1, 'goods_manage', ''),
  (22, 1, 'remove_back', ''),
  (23, 1, 'cat_manage', ''),
  (24, 1, 'cat_drop', 'cat_manage'),
  (25, 1, 'attr_manage', ''),
  (26, 1, 'brand_manage', ''),
  (27, 1, 'comment_priv', ''),
  (84, 1, 'tag_manage', ''),
  (30, 2, 'article_cat', ''),
  (31, 2, 'article_manage', ''),
  (32, 2, 'shopinfo_manage', ''),
  (33, 2, 'shophelp_manage', ''),
  (34, 2, 'vote_priv', ''),
  (35, 7, 'topic_manage', ''),
  (74, 4, 'template_manage', ''),
  (73, 3, 'feedback_priv', ''),
  (38, 3, 'integrate_users', ''),
  (39, 3, 'sync_users', 'integrate_users'),
  (40, 3, 'users_manage', ''),
  (41, 3, 'users_drop', 'users_manage'),
  (42, 3, 'user_rank', ''),
  (85, 3, 'surplus_manage', 'account_manage'),
  (43, 4, 'admin_manage', ''),
  (44, 4, 'admin_drop', 'admin_manage'),
  (45, 4, 'allot_priv', 'admin_manage'),
  (46, 4, 'logs_manage', ''),
  (47, 4, 'logs_drop', 'logs_manage'),
  (48, 5, 'shop_config', ''),
  (49, 5, 'ship_manage', ''),
  (50, 5, 'payment', ''),
  (51, 5, 'shiparea_manage', ''),
  (52, 5, 'area_manage', ''),
  (53, 6, 'order_os_edit', ''),
  (54, 6, 'order_ps_edit', 'order_os_edit'),
  (55, 6, 'order_ss_edit', 'order_os_edit'),
  (56, 6, 'order_edit', 'order_os_edit'),
  (57, 6, 'order_view', ''),
  (58, 6, 'order_view_finished', ''),
  (59, 6, 'repay_manage', ''),
  (60, 6, 'booking', ''),
  (61, 6, 'sale_order_stats', ''),
  (62, 6, 'client_flow_stats', ''),
  (78, 7, 'snatch_manage', ''),
  (83, 7, 'ad_manage', ''),
  (80, 7, 'gift_manage', ''),
  (81, 7, 'card_manage', ''),
  (70, 1, 'goods_type', ''),
  (82, 7, 'pack', ''),
  (79, 7, 'bonus_manage', ''),
  (75, 5, 'friendlink', ''),
  (76, 5, 'db_backup', ''),
  (77, 5, 'db_renew', 'db_backup'),
  (86, 4, 'agency_manage', ''),
  (87, 3, 'account_manage', ''),
  (88, 5, 'flash_manage', ''),
  (89, 5, 'navigator', ''),
  (90, 7, 'auction', ''),
  (91, 7, 'group_by', ''),
  (92, 7, 'favourable', ''),
  (93, 7, 'whole_sale', ''),
  (94, 1, 'goods_auto', ''),
  (95, 2, 'article_auto', ''),
  (96, 5, 'cron', ''),
  (97, 5, 'affiliate', ''),
  (98, 5, 'affiliate_ck', ''),
  (99, 8, 'attention_list', ''),
  (100, 8, 'email_list', ''),
  (101, 8, 'magazine_list', ''),
  (102, 8, 'view_sendlist', ''),
  (103, 1, 'virualcard', ''),
  (104, 7, 'package_manage', ''),
  (105, 1, 'picture_batch', ''),
  (106, 1, 'goods_export', ''),
  (107, 1, 'goods_batch', ''),
  (108, 1, 'gen_goods_script', ''),
  (109, 5, 'sitemap', ''),
  (110, 5, 'file_priv', ''),
  (111, 5, 'file_check', ''),
  (112, 9, 'template_select', ''),
  (113, 9, 'template_setup', ''),
  (114, 9, 'library_manage', ''),
  (115, 9, 'lang_edit', ''),
  (116, 9, 'backup_setting', ''),
  (117, 9, 'mail_template', ''),
  (118, 10, 'db_backup', ''),
  (119, 10, 'db_renew', ''),
  (120, 10, 'db_optimize', ''),
  (121, 10, 'sql_query', ''),
  (122, 10, 'convert', ''),
  (124, 11, 'sms_send', ''),
  (128, 7, 'exchange_goods', ''),
  (129, 6, 'delivery_view', ''),
  (130, 6, 'back_view', ''),
  (131, 5, 'reg_fields', ''),
  (132, 5, 'shop_authorized', ''),
  (133, 5, 'webcollect_manage', ''),
  (134, 4, 'suppliers_manage', ''),
  (135, 4, 'role_manage', '');

--
--  `ecs_mail_templates`
--
INSERT INTO `ecs_mail_templates` (
    `template_id`,
    `template_code`,
    `is_html`,
    `template_subject`,
    `template_content`,
    `last_modify`,
    `last_send`,
    `type`
  )
VALUES (
    1,
    'send_password',
    1,
    '密碼找回',
    '{$user_name}您好！<br>\n<br>\n您已經進行了密碼重置的操作，請點擊以下鏈接(或者復制到您的瀏覽器):<br>\n<br>\n<a href="{$reset_email}" target="_blank">{$reset_email}</a><br>\n<br>\n以確認您的新密碼重置操作！<br>\n<br>\n{$shop_name}<br>\n{$send_date}',
    1194824789,
    0,
    'template'
  ),
  (
    2,
    'order_confirm',
    0,
    '訂單確認通知',
    '親愛的{$order.consignee}，妳好！ \n\n我們已經收到您於 {$order.formated_add_time} 提交的訂單，該訂單編號為：{$order.order_sn} 請記住這個編號以便日後的查詢。\n\n{$shop_name}\n{$sent_date}\n\n\n',
    1158226370,
    0,
    'template'
  ),
  (
    3,
    'deliver_notice',
    1,
    '發貨通知',
    '親愛的{$order.consignee}。妳好！</br></br>\n\n您的訂單{$order.order_sn}已於{$send_time}按照您預定的配送方式給您發貨了。</br>\n</br>\n{if $order.invoice_no}發貨單號是{$order.invoice_no}。</br>{/if}\n</br>\n在您收到貨物之後請點擊下面的鏈接確認您已經收到貨物：</br>\n<a href="{$confirm_url}" target="_blank">{$confirm_url}</a></br></br>\n如果您還沒有收到貨物可以點擊以下鏈接給我們留言：</br></br>\n<a href="{$send_msg_url}" target="_blank">{$send_msg_url}</a></br>\n<br>\n再次感謝您對我們的支持。歡迎您的再次光臨。 <br>\n<br>\n{$shop_name} </br>\n{$send_date}',
    1194823291,
    0,
    'template'
  ),
  (
    4,
    'order_cancel',
    0,
    '訂單取消',
    '親愛的{$order.consignee}，妳好！ \n\n您的編號為：{$order.order_sn}的訂單已取消。\n\n{$shop_name}\n{$send_date}',
    1156491130,
    0,
    'template'
  ),
  (
    5,
    'order_invalid',
    0,
    '訂單無效',
    '親愛的{$order.consignee}，妳好！\n\n您的編號為：{$order.order_sn}的訂單無效。\n\n{$shop_name}\n{$send_date}',
    1156491164,
    0,
    'template'
  ),
  (
    6,
    'send_bonus',
    0,
    '發紅包',
    '親愛的{$user_name}您好！\n\n恭喜您獲得了{$count}個紅包，金額{if $count > 1}分別{/if}為{$money}\n\n{$shop_name}\n{$send_date}\n',
    1156491184,
    0,
    'template'
  ),
  (
    7,
    'group_buy',
    1,
    '團購商品',
    '親愛的{$consignee}，您好！<br>\n<br>\n您於{$order_time}在本店參加團購商品活動，所購買的商品名稱為：{$goods_name}，數量：{$goods_number}，訂單號為：{$order_sn}，訂單金額為：{$order_amount}<br>\n<br>\n此團購商品現在已到結束日期，並達到最低價格，您現在可以對該訂單付款。<br>\n<br>\n請點擊下面的鏈接：<br>\n<a href="{$shop_url}" target="_blank">{$shop_url}</a><br>\n<br>\n請盡快登錄到用戶中心，查看您的訂單詳情信息。 <br>\n<br>\n{$shop_name} <br>\n<br>\n{$send_date}',
    1194824668,
    0,
    'template'
  ),
  (
    8,
    'register_validate',
    1,
    '郵件驗證',
    '{$user_name}您好！<br><br>\r\n\r\n這封郵件是 {$shop_name} 發送的。妳收到這封郵件是為了驗證妳註冊郵件地址是否有效。如果您已經通過驗證了，請忽略這封郵件。<br>\r\n請點擊以下鏈接(或者復制到您的瀏覽器)來驗證妳的郵件地址:<br>\r\n<a href="{$validate_email}" target="_blank">{$validate_email}</a><br><br>\r\n\r\n{$shop_name}<br>\r\n{$send_date}',
    1162201031,
    0,
    'template'
  ),
  (
    9,
    'virtual_card',
    0,
    '虛擬卡片',
    '親愛的{$order.consignee}\r\n妳好！您的訂單{$order.order_sn}中{$goods.goods_name} 商品的詳細信息如下:\r\n{foreach from=$virtual_card item=card}\r\n{if $card.card_sn}卡號：{$card.card_sn}{/if}{if $card.card_password}卡片密碼：{$card.card_password}{/if}{if $card.end_date}截至日期：{$card.end_date}{/if}\r\n{/foreach}\r\n再次感謝您對我們的支持。歡迎您的再次光臨。\r\n\r\n{$shop_name} \r\n{$send_date}',
    1162201031,
    0,
    'template'
  ),
  (
    10,
    'attention_list',
    0,
    '關註商品',
    '親愛的{$user_name}您好~\n\n您關註的商品 : {$goods_name} 最近已經更新,請您查看最新的商品信息\n\n{$goods_url}\r\n\r\n{$shop_name} \r\n{$send_date}',
    1183851073,
    0,
    'template'
  ),
  (
    11,
    'remind_of_new_order',
    0,
    '新訂單通知',
    '親愛的店長，您好：\n   快來看看吧，又有新訂單了。\n    訂單號:{$order.order_sn} \n 訂單金額:{$order.order_amount}，\n 用戶購買商品:{foreach from=$goods_list item=goods_data}{$goods_data.goods_name}(貨號:{$goods_data.goods_sn})    {/foreach} \n\n 收貨人:{$order.consignee}， \n 收貨人地址:{$order.address}，\n 收貨人電話:{$order.tel} {$order.mobile}, \n 配送方式:{$order.shipping_name}(費用:{$order.shipping_fee}), \n 付款方式:{$order.pay_name}(費用:{$order.pay_fee})。\n\n               系統提醒\n               {$send_date}',
    1196239170,
    0,
    'template'
  ),
  (
    12,
    'goods_booking',
    1,
    '缺貨回復',
    '親愛的{$user_name}。你好！</br></br>{$dispose_note}</br></br>您提交的缺貨商品鏈接為</br></br><a href="{$goods_link}" target="_blank">{$goods_name}</a></br><br>{$shop_name} </br>{$send_date}',
    0,
    0,
    'template'
  ),
  (
    13,
    'user_message',
    1,
    '留言回復',
    '親愛的{$user_name}。你好！</br></br>對您的留言：</br>{$message_content}</br></br>店主作了如下回復：</br>{$message_note}</br></br>您可以隨時回到店中和店主繼續溝通。</br>{$shop_name}</br>{$send_date}',
    0,
    0,
    'template'
  ),
  (
    14,
    'recomment',
    1,
    '用戶評論回復',
    '親愛的{$user_name}。你好！</br></br>對您的評論：</br>“{$comment}”</br></br>店主作了如下回復：</br>“{$recomment}”</br></br>您可以隨時回到店中和店主繼續溝通。</br>{$shop_name}</br>{$send_date}',
    0,
    0,
    'template'
  );

--
--  `ecs_region`
--
INSERT INTO `ecs_region` (
    `region_id`,
    `parent_id`,
    `region_name`,
    `region_type`,
    `agency_id`
  )
VALUES (1, 0, '中國', 0, 0),
  (2, 1, '北京', 1, 0),
  (3, 1, '安徽', 1, 0),
  (4, 1, '福建', 1, 0),
  (5, 1, '甘肅', 1, 0),
  (6, 1, '廣東', 1, 0),
  (7, 1, '廣西', 1, 0),
  (8, 1, '貴州', 1, 0),
  (9, 1, '海南', 1, 0),
  (10, 1, '河北', 1, 0),
  (11, 1, '河南', 1, 0),
  (12, 1, '黑龍江', 1, 0),
  (13, 1, '湖北', 1, 0),
  (14, 1, '湖南', 1, 0),
  (15, 1, '吉林', 1, 0),
  (16, 1, '江蘇', 1, 0),
  (17, 1, '江西', 1, 0),
  (18, 1, '遼寧', 1, 0),
  (19, 1, '內蒙古', 1, 0),
  (20, 1, '寧夏', 1, 0),
  (21, 1, '青海', 1, 0),
  (22, 1, '山東', 1, 0),
  (23, 1, '山西', 1, 0),
  (24, 1, '陜西', 1, 0),
  (25, 1, '上海', 1, 0),
  (26, 1, '四川', 1, 0),
  (27, 1, '天津', 1, 0),
  (28, 1, '西藏', 1, 0),
  (29, 1, '新疆', 1, 0),
  (30, 1, '雲南', 1, 0),
  (31, 1, '浙江', 1, 0),
  (32, 1, '重慶', 1, 0),
  (33, 1, '香港', 1, 0),
  (34, 1, '澳門', 1, 0),
  (35, 1, '臺灣', 1, 0),
  (36, 3, '安慶', 2, 0),
  (37, 3, '蚌埠', 2, 0),
  (38, 3, '巢湖', 2, 0),
  (39, 3, '池州', 2, 0),
  (40, 3, '滁州', 2, 0),
  (41, 3, '阜陽', 2, 0),
  (42, 3, '淮北', 2, 0),
  (43, 3, '淮南', 2, 0),
  (44, 3, '黃山', 2, 0),
  (45, 3, '六安', 2, 0),
  (46, 3, '馬鞍山', 2, 0),
  (47, 3, '宿州', 2, 0),
  (48, 3, '銅陵', 2, 0),
  (49, 3, '蕪湖', 2, 0),
  (50, 3, '宣城', 2, 0),
  (51, 3, '亳州', 2, 0),
  (52, 2, '北京', 2, 0),
  (53, 4, '福州', 2, 0),
  (54, 4, '龍巖', 2, 0),
  (55, 4, '南平', 2, 0),
  (56, 4, '寧德', 2, 0),
  (57, 4, '莆田', 2, 0),
  (58, 4, '泉州', 2, 0),
  (59, 4, '三明', 2, 0),
  (60, 4, '廈門', 2, 0),
  (61, 4, '漳州', 2, 0),
  (62, 5, '蘭州', 2, 0),
  (63, 5, '白銀', 2, 0),
  (64, 5, '定西', 2, 0),
  (65, 5, '甘南', 2, 0),
  (66, 5, '嘉峪關', 2, 0),
  (67, 5, '金昌', 2, 0),
  (68, 5, '酒泉', 2, 0),
  (69, 5, '臨夏', 2, 0),
  (70, 5, '隴南', 2, 0),
  (71, 5, '平涼', 2, 0),
  (72, 5, '慶陽', 2, 0),
  (73, 5, '天水', 2, 0),
  (74, 5, '武威', 2, 0),
  (75, 5, '張掖', 2, 0),
  (76, 6, '廣州', 2, 0),
  (77, 6, '深圳', 2, 0),
  (78, 6, '潮州', 2, 0),
  (79, 6, '東莞', 2, 0),
  (80, 6, '佛山', 2, 0),
  (81, 6, '河源', 2, 0),
  (82, 6, '惠州', 2, 0),
  (83, 6, '江門', 2, 0),
  (84, 6, '揭陽', 2, 0),
  (85, 6, '茂名', 2, 0),
  (86, 6, '梅州', 2, 0),
  (87, 6, '清遠', 2, 0),
  (88, 6, '汕頭', 2, 0),
  (89, 6, '汕尾', 2, 0),
  (90, 6, '韶關', 2, 0),
  (91, 6, '陽江', 2, 0),
  (92, 6, '雲浮', 2, 0),
  (93, 6, '湛江', 2, 0),
  (94, 6, '肇慶', 2, 0),
  (95, 6, '中山', 2, 0),
  (96, 6, '珠海', 2, 0),
  (97, 7, '南寧', 2, 0),
  (98, 7, '桂林', 2, 0),
  (99, 7, '百色', 2, 0),
  (100, 7, '北海', 2, 0),
  (101, 7, '崇左', 2, 0),
  (102, 7, '防城港', 2, 0),
  (103, 7, '貴港', 2, 0),
  (104, 7, '河池', 2, 0),
  (105, 7, '賀州', 2, 0),
  (106, 7, '來賓', 2, 0),
  (107, 7, '柳州', 2, 0),
  (108, 7, '欽州', 2, 0),
  (109, 7, '梧州', 2, 0),
  (110, 7, '玉林', 2, 0),
  (111, 8, '貴陽', 2, 0),
  (112, 8, '安順', 2, 0),
  (113, 8, '畢節', 2, 0),
  (114, 8, '六盤水', 2, 0),
  (115, 8, '黔東南', 2, 0),
  (116, 8, '黔南', 2, 0),
  (117, 8, '黔西南', 2, 0),
  (118, 8, '銅仁', 2, 0),
  (119, 8, '遵義', 2, 0),
  (120, 9, '海口', 2, 0),
  (121, 9, '三亞', 2, 0),
  (122, 9, '白沙', 2, 0),
  (123, 9, '保亭', 2, 0),
  (124, 9, '昌江', 2, 0),
  (125, 9, '澄邁縣', 2, 0),
  (126, 9, '定安縣', 2, 0),
  (127, 9, '東方', 2, 0),
  (128, 9, '樂東', 2, 0),
  (129, 9, '臨高縣', 2, 0),
  (130, 9, '陵水', 2, 0),
  (131, 9, '瓊海', 2, 0),
  (132, 9, '瓊中', 2, 0),
  (133, 9, '屯昌縣', 2, 0),
  (134, 9, '萬寧', 2, 0),
  (135, 9, '文昌', 2, 0),
  (136, 9, '五指山', 2, 0),
  (137, 9, '儋州', 2, 0),
  (138, 10, '石家莊', 2, 0),
  (139, 10, '保定', 2, 0),
  (140, 10, '滄州', 2, 0),
  (141, 10, '承德', 2, 0),
  (142, 10, '邯鄲', 2, 0),
  (143, 10, '衡水', 2, 0),
  (144, 10, '廊坊', 2, 0),
  (145, 10, '秦皇島', 2, 0),
  (146, 10, '唐山', 2, 0),
  (147, 10, '邢臺', 2, 0),
  (148, 10, '張家口', 2, 0),
  (149, 11, '鄭州', 2, 0),
  (150, 11, '洛陽', 2, 0),
  (151, 11, '開封', 2, 0),
  (152, 11, '安陽', 2, 0),
  (153, 11, '鶴壁', 2, 0),
  (154, 11, '濟源', 2, 0),
  (155, 11, '焦作', 2, 0),
  (156, 11, '南陽', 2, 0),
  (157, 11, '平頂山', 2, 0),
  (158, 11, '三門峽', 2, 0),
  (159, 11, '商丘', 2, 0),
  (160, 11, '新鄉', 2, 0),
  (161, 11, '信陽', 2, 0),
  (162, 11, '許昌', 2, 0),
  (163, 11, '周口', 2, 0),
  (164, 11, '駐馬店', 2, 0),
  (165, 11, '漯河', 2, 0),
  (166, 11, '濮陽', 2, 0),
  (167, 12, '哈爾濱', 2, 0),
  (168, 12, '大慶', 2, 0),
  (169, 12, '大興安嶺', 2, 0),
  (170, 12, '鶴崗', 2, 0),
  (171, 12, '黑河', 2, 0),
  (172, 12, '雞西', 2, 0),
  (173, 12, '佳木斯', 2, 0),
  (174, 12, '牡丹江', 2, 0),
  (175, 12, '七臺河', 2, 0),
  (176, 12, '齊齊哈爾', 2, 0),
  (177, 12, '雙鴨山', 2, 0),
  (178, 12, '綏化', 2, 0),
  (179, 12, '伊春', 2, 0),
  (180, 13, '武漢', 2, 0),
  (181, 13, '仙桃', 2, 0),
  (182, 13, '鄂州', 2, 0),
  (183, 13, '黃岡', 2, 0),
  (184, 13, '黃石', 2, 0),
  (185, 13, '荊門', 2, 0),
  (186, 13, '荊州', 2, 0),
  (187, 13, '潛江', 2, 0),
  (188, 13, '神農架林區', 2, 0),
  (189, 13, '十堰', 2, 0),
  (190, 13, '隨州', 2, 0),
  (191, 13, '天門', 2, 0),
  (192, 13, '鹹寧', 2, 0),
  (193, 13, '襄樊', 2, 0),
  (194, 13, '孝感', 2, 0),
  (195, 13, '宜昌', 2, 0),
  (196, 13, '恩施', 2, 0),
  (197, 14, '長沙', 2, 0),
  (198, 14, '張家界', 2, 0),
  (199, 14, '常德', 2, 0),
  (200, 14, '郴州', 2, 0),
  (201, 14, '衡陽', 2, 0),
  (202, 14, '懷化', 2, 0),
  (203, 14, '婁底', 2, 0),
  (204, 14, '邵陽', 2, 0),
  (205, 14, '湘潭', 2, 0),
  (206, 14, '湘西', 2, 0),
  (207, 14, '益陽', 2, 0),
  (208, 14, '永州', 2, 0),
  (209, 14, '嶽陽', 2, 0),
  (210, 14, '株洲', 2, 0),
  (211, 15, '長春', 2, 0),
  (212, 15, '吉林', 2, 0),
  (213, 15, '白城', 2, 0),
  (214, 15, '白山', 2, 0),
  (215, 15, '遼源', 2, 0),
  (216, 15, '四平', 2, 0),
  (217, 15, '松原', 2, 0),
  (218, 15, '通化', 2, 0),
  (219, 15, '延邊', 2, 0),
  (220, 16, '南京', 2, 0),
  (221, 16, '蘇州', 2, 0),
  (222, 16, '無錫', 2, 0),
  (223, 16, '常州', 2, 0),
  (224, 16, '淮安', 2, 0),
  (225, 16, '連雲港', 2, 0),
  (226, 16, '南通', 2, 0),
  (227, 16, '宿遷', 2, 0),
  (228, 16, '泰州', 2, 0),
  (229, 16, '徐州', 2, 0),
  (230, 16, '鹽城', 2, 0),
  (231, 16, '揚州', 2, 0),
  (232, 16, '鎮江', 2, 0),
  (233, 17, '南昌', 2, 0),
  (234, 17, '撫州', 2, 0),
  (235, 17, '贛州', 2, 0),
  (236, 17, '吉安', 2, 0),
  (237, 17, '景德鎮', 2, 0),
  (238, 17, '九江', 2, 0),
  (239, 17, '萍鄉', 2, 0),
  (240, 17, '上饒', 2, 0),
  (241, 17, '新余', 2, 0),
  (242, 17, '宜春', 2, 0),
  (243, 17, '鷹潭', 2, 0),
  (244, 18, '沈陽', 2, 0),
  (245, 18, '大連', 2, 0),
  (246, 18, '鞍山', 2, 0),
  (247, 18, '本溪', 2, 0),
  (248, 18, '朝陽', 2, 0),
  (249, 18, '丹東', 2, 0),
  (250, 18, '撫順', 2, 0),
  (251, 18, '阜新', 2, 0),
  (252, 18, '葫蘆島', 2, 0),
  (253, 18, '錦州', 2, 0),
  (254, 18, '遼陽', 2, 0),
  (255, 18, '盤錦', 2, 0),
  (256, 18, '鐵嶺', 2, 0),
  (257, 18, '營口', 2, 0),
  (258, 19, '呼和浩特', 2, 0),
  (259, 19, '阿拉善盟', 2, 0),
  (260, 19, '巴彥淖爾盟', 2, 0),
  (261, 19, '包頭', 2, 0),
  (262, 19, '赤峰', 2, 0),
  (263, 19, '鄂爾多斯', 2, 0),
  (264, 19, '呼倫貝爾', 2, 0),
  (265, 19, '通遼', 2, 0),
  (266, 19, '烏海', 2, 0),
  (267, 19, '烏蘭察布市', 2, 0),
  (268, 19, '錫林郭勒盟', 2, 0),
  (269, 19, '興安盟', 2, 0),
  (270, 20, '銀川', 2, 0),
  (271, 20, '固原', 2, 0),
  (272, 20, '石嘴山', 2, 0),
  (273, 20, '吳忠', 2, 0),
  (274, 20, '中衛', 2, 0),
  (275, 21, '西寧', 2, 0),
  (276, 21, '果洛', 2, 0),
  (277, 21, '海北', 2, 0),
  (278, 21, '海東', 2, 0),
  (279, 21, '海南', 2, 0),
  (280, 21, '海西', 2, 0),
  (281, 21, '黃南', 2, 0),
  (282, 21, '玉樹', 2, 0),
  (283, 22, '濟南', 2, 0),
  (284, 22, '青島', 2, 0),
  (285, 22, '濱州', 2, 0),
  (286, 22, '德州', 2, 0),
  (287, 22, '東營', 2, 0),
  (288, 22, '菏澤', 2, 0),
  (289, 22, '濟寧', 2, 0),
  (290, 22, '萊蕪', 2, 0),
  (291, 22, '聊城', 2, 0),
  (292, 22, '臨沂', 2, 0),
  (293, 22, '日照', 2, 0),
  (294, 22, '泰安', 2, 0),
  (295, 22, '威海', 2, 0),
  (296, 22, '濰坊', 2, 0),
  (297, 22, '煙臺', 2, 0),
  (298, 22, '棗莊', 2, 0),
  (299, 22, '淄博', 2, 0),
  (300, 23, '太原', 2, 0),
  (301, 23, '長治', 2, 0),
  (302, 23, '大同', 2, 0),
  (303, 23, '晉城', 2, 0),
  (304, 23, '晉中', 2, 0),
  (305, 23, '臨汾', 2, 0),
  (306, 23, '呂梁', 2, 0),
  (307, 23, '朔州', 2, 0),
  (308, 23, '忻州', 2, 0),
  (309, 23, '陽泉', 2, 0),
  (310, 23, '運城', 2, 0),
  (311, 24, '西安', 2, 0),
  (312, 24, '安康', 2, 0),
  (313, 24, '寶雞', 2, 0),
  (314, 24, '漢中', 2, 0),
  (315, 24, '商洛', 2, 0),
  (316, 24, '銅川', 2, 0),
  (317, 24, '渭南', 2, 0),
  (318, 24, '鹹陽', 2, 0),
  (319, 24, '延安', 2, 0),
  (320, 24, '榆林', 2, 0),
  (321, 25, '上海', 2, 0),
  (322, 26, '成都', 2, 0),
  (323, 26, '綿陽', 2, 0),
  (324, 26, '阿壩', 2, 0),
  (325, 26, '巴中', 2, 0),
  (326, 26, '達州', 2, 0),
  (327, 26, '德陽', 2, 0),
  (328, 26, '甘孜', 2, 0),
  (329, 26, '廣安', 2, 0),
  (330, 26, '廣元', 2, 0),
  (331, 26, '樂山', 2, 0),
  (332, 26, '涼山', 2, 0),
  (333, 26, '眉山', 2, 0),
  (334, 26, '南充', 2, 0),
  (335, 26, '內江', 2, 0),
  (336, 26, '攀枝花', 2, 0),
  (337, 26, '遂寧', 2, 0),
  (338, 26, '雅安', 2, 0),
  (339, 26, '宜賓', 2, 0),
  (340, 26, '資陽', 2, 0),
  (341, 26, '自貢', 2, 0),
  (342, 26, '瀘州', 2, 0),
  (343, 27, '天津', 2, 0),
  (344, 28, '拉薩', 2, 0),
  (345, 28, '阿裏', 2, 0),
  (346, 28, '昌都', 2, 0),
  (347, 28, '林芝', 2, 0),
  (348, 28, '那曲', 2, 0),
  (349, 28, '日喀則', 2, 0),
  (350, 28, '山南', 2, 0),
  (351, 29, '烏魯木齊', 2, 0),
  (352, 29, '阿克蘇', 2, 0),
  (353, 29, '阿拉爾', 2, 0),
  (354, 29, '巴音郭楞', 2, 0),
  (355, 29, '博爾塔拉', 2, 0),
  (356, 29, '昌吉', 2, 0),
  (357, 29, '哈密', 2, 0),
  (358, 29, '和田', 2, 0),
  (359, 29, '喀什', 2, 0),
  (360, 29, '克拉瑪依', 2, 0),
  (361, 29, '克孜勒蘇', 2, 0),
  (362, 29, '石河子', 2, 0),
  (363, 29, '圖木舒克', 2, 0),
  (364, 29, '吐魯番', 2, 0),
  (365, 29, '五家渠', 2, 0),
  (366, 29, '伊犁', 2, 0),
  (367, 30, '昆明', 2, 0),
  (368, 30, '怒江', 2, 0),
  (369, 30, '普洱', 2, 0),
  (370, 30, '麗江', 2, 0),
  (371, 30, '保山', 2, 0),
  (372, 30, '楚雄', 2, 0),
  (373, 30, '大理', 2, 0),
  (374, 30, '德宏', 2, 0),
  (375, 30, '迪慶', 2, 0),
  (376, 30, '紅河', 2, 0),
  (377, 30, '臨滄', 2, 0),
  (378, 30, '曲靖', 2, 0),
  (379, 30, '文山', 2, 0),
  (380, 30, '西雙版納', 2, 0),
  (381, 30, '玉溪', 2, 0),
  (382, 30, '昭通', 2, 0),
  (383, 31, '杭州', 2, 0),
  (384, 31, '湖州', 2, 0),
  (385, 31, '嘉興', 2, 0),
  (386, 31, '金華', 2, 0),
  (387, 31, '麗水', 2, 0),
  (388, 31, '寧波', 2, 0),
  (389, 31, '紹興', 2, 0),
  (390, 31, '臺州', 2, 0),
  (391, 31, '溫州', 2, 0),
  (392, 31, '舟山', 2, 0),
  (393, 31, '衢州', 2, 0),
  (394, 32, '重慶', 2, 0),
  (395, 33, '香港', 2, 0),
  (396, 34, '澳門', 2, 0),
  (397, 35, '臺灣', 2, 0),
  (398, 36, '迎江區', 3, 0),
  (399, 36, '大觀區', 3, 0),
  (400, 36, '宜秀區', 3, 0),
  (401, 36, '桐城市', 3, 0),
  (402, 36, '懷寧縣', 3, 0),
  (403, 36, '樅陽縣', 3, 0),
  (404, 36, '潛山縣', 3, 0),
  (405, 36, '太湖縣', 3, 0),
  (406, 36, '宿松縣', 3, 0),
  (407, 36, '望江縣', 3, 0),
  (408, 36, '嶽西縣', 3, 0),
  (409, 37, '中市區', 3, 0),
  (410, 37, '東市區', 3, 0),
  (411, 37, '西市區', 3, 0),
  (412, 37, '郊區', 3, 0),
  (413, 37, '懷遠縣', 3, 0),
  (414, 37, '五河縣', 3, 0),
  (415, 37, '固鎮縣', 3, 0),
  (416, 38, '居巢區', 3, 0),
  (417, 38, '廬江縣', 3, 0),
  (418, 38, '無為縣', 3, 0),
  (419, 38, '含山縣', 3, 0),
  (420, 38, '和縣', 3, 0),
  (421, 39, '貴池區', 3, 0),
  (422, 39, '東至縣', 3, 0),
  (423, 39, '石臺縣', 3, 0),
  (424, 39, '青陽縣', 3, 0),
  (425, 40, '瑯琊區', 3, 0),
  (426, 40, '南譙區', 3, 0),
  (427, 40, '天長市', 3, 0),
  (428, 40, '明光市', 3, 0),
  (429, 40, '來安縣', 3, 0),
  (430, 40, '全椒縣', 3, 0),
  (431, 40, '定遠縣', 3, 0),
  (432, 40, '鳳陽縣', 3, 0),
  (433, 41, '蚌山區', 3, 0),
  (434, 41, '龍子湖區', 3, 0),
  (435, 41, '禹會區', 3, 0),
  (436, 41, '淮上區', 3, 0),
  (437, 41, '潁州區', 3, 0),
  (438, 41, '潁東區', 3, 0),
  (439, 41, '潁泉區', 3, 0),
  (440, 41, '界首市', 3, 0),
  (441, 41, '臨泉縣', 3, 0),
  (442, 41, '太和縣', 3, 0),
  (443, 41, '阜南縣', 3, 0),
  (444, 41, '穎上縣', 3, 0),
  (445, 42, '相山區', 3, 0),
  (446, 42, '杜集區', 3, 0),
  (447, 42, '烈山區', 3, 0),
  (448, 42, '濉溪縣', 3, 0),
  (449, 43, '田家庵區', 3, 0),
  (450, 43, '大通區', 3, 0),
  (451, 43, '謝家集區', 3, 0),
  (452, 43, '八公山區', 3, 0),
  (453, 43, '潘集區', 3, 0),
  (454, 43, '鳳臺縣', 3, 0),
  (455, 44, '屯溪區', 3, 0),
  (456, 44, '黃山區', 3, 0),
  (457, 44, '徽州區', 3, 0),
  (458, 44, '歙縣', 3, 0),
  (459, 44, '休寧縣', 3, 0),
  (460, 44, '黟縣', 3, 0),
  (461, 44, '祁門縣', 3, 0),
  (462, 45, '金安區', 3, 0),
  (463, 45, '裕安區', 3, 0),
  (464, 45, '壽縣', 3, 0),
  (465, 45, '霍邱縣', 3, 0),
  (466, 45, '舒城縣', 3, 0),
  (467, 45, '金寨縣', 3, 0),
  (468, 45, '霍山縣', 3, 0),
  (469, 46, '雨山區', 3, 0),
  (470, 46, '花山區', 3, 0),
  (471, 46, '金家莊區', 3, 0),
  (472, 46, '當塗縣', 3, 0),
  (473, 47, '埇橋區', 3, 0),
  (474, 47, '碭山縣', 3, 0),
  (475, 47, '蕭縣', 3, 0),
  (476, 47, '靈璧縣', 3, 0),
  (477, 47, '泗縣', 3, 0),
  (478, 48, '銅官山區', 3, 0),
  (479, 48, '獅子山區', 3, 0),
  (480, 48, '郊區', 3, 0),
  (481, 48, '銅陵縣', 3, 0),
  (482, 49, '鏡湖區', 3, 0),
  (483, 49, '弋江區', 3, 0),
  (484, 49, '鳩江區', 3, 0),
  (485, 49, '三山區', 3, 0),
  (486, 49, '蕪湖縣', 3, 0),
  (487, 49, '繁昌縣', 3, 0),
  (488, 49, '南陵縣', 3, 0),
  (489, 50, '宣州區', 3, 0),
  (490, 50, '寧國市', 3, 0),
  (491, 50, '郎溪縣', 3, 0),
  (492, 50, '廣德縣', 3, 0),
  (493, 50, '涇縣', 3, 0),
  (494, 50, '績溪縣', 3, 0),
  (495, 50, '旌德縣', 3, 0),
  (496, 51, '渦陽縣', 3, 0),
  (497, 51, '蒙城縣', 3, 0),
  (498, 51, '利辛縣', 3, 0),
  (499, 51, '譙城區', 3, 0),
  (500, 52, '東城區', 3, 0),
  (501, 52, '西城區', 3, 0),
  (502, 52, '海澱區', 3, 0),
  (503, 52, '朝陽區', 3, 0),
  (504, 52, '崇文區', 3, 0),
  (505, 52, '宣武區', 3, 0),
  (506, 52, '豐臺區', 3, 0),
  (507, 52, '石景山區', 3, 0),
  (508, 52, '房山區', 3, 0),
  (509, 52, '門頭溝區', 3, 0),
  (510, 52, '通州區', 3, 0),
  (511, 52, '順義區', 3, 0),
  (512, 52, '昌平區', 3, 0),
  (513, 52, '懷柔區', 3, 0),
  (514, 52, '平谷區', 3, 0),
  (515, 52, '大興區', 3, 0),
  (516, 52, '密雲縣', 3, 0),
  (517, 52, '延慶縣', 3, 0),
  (518, 53, '鼓樓區', 3, 0),
  (519, 53, '臺江區', 3, 0),
  (520, 53, '倉山區', 3, 0),
  (521, 53, '馬尾區', 3, 0),
  (522, 53, '晉安區', 3, 0),
  (523, 53, '福清市', 3, 0),
  (524, 53, '長樂市', 3, 0),
  (525, 53, '閩侯縣', 3, 0),
  (526, 53, '連江縣', 3, 0),
  (527, 53, '羅源縣', 3, 0),
  (528, 53, '閩清縣', 3, 0),
  (529, 53, '永泰縣', 3, 0),
  (530, 53, '平潭縣', 3, 0),
  (531, 54, '新羅區', 3, 0),
  (532, 54, '漳平市', 3, 0),
  (533, 54, '長汀縣', 3, 0),
  (534, 54, '永定縣', 3, 0),
  (535, 54, '上杭縣', 3, 0),
  (536, 54, '武平縣', 3, 0),
  (537, 54, '連城縣', 3, 0),
  (538, 55, '延平區', 3, 0),
  (539, 55, '邵武市', 3, 0),
  (540, 55, '武夷山市', 3, 0),
  (541, 55, '建甌市', 3, 0),
  (542, 55, '建陽市', 3, 0),
  (543, 55, '順昌縣', 3, 0),
  (544, 55, '浦城縣', 3, 0),
  (545, 55, '光澤縣', 3, 0),
  (546, 55, '松溪縣', 3, 0),
  (547, 55, '政和縣', 3, 0),
  (548, 56, '蕉城區', 3, 0),
  (549, 56, '福安市', 3, 0),
  (550, 56, '福鼎市', 3, 0),
  (551, 56, '霞浦縣', 3, 0),
  (552, 56, '古田縣', 3, 0),
  (553, 56, '屏南縣', 3, 0),
  (554, 56, '壽寧縣', 3, 0),
  (555, 56, '周寧縣', 3, 0),
  (556, 56, '柘榮縣', 3, 0),
  (557, 57, '城廂區', 3, 0),
  (558, 57, '涵江區', 3, 0),
  (559, 57, '荔城區', 3, 0),
  (560, 57, '秀嶼區', 3, 0),
  (561, 57, '仙遊縣', 3, 0),
  (562, 58, '鯉城區', 3, 0),
  (563, 58, '豐澤區', 3, 0),
  (564, 58, '洛江區', 3, 0),
  (565, 58, '清濛開發區', 3, 0),
  (566, 58, '泉港區', 3, 0),
  (567, 58, '石獅市', 3, 0),
  (568, 58, '晉江市', 3, 0),
  (569, 58, '南安市', 3, 0),
  (570, 58, '惠安縣', 3, 0),
  (571, 58, '安溪縣', 3, 0),
  (572, 58, '永春縣', 3, 0),
  (573, 58, '德化縣', 3, 0),
  (574, 58, '金門縣', 3, 0),
  (575, 59, '梅列區', 3, 0),
  (576, 59, '三元區', 3, 0),
  (577, 59, '永安市', 3, 0),
  (578, 59, '明溪縣', 3, 0),
  (579, 59, '清流縣', 3, 0),
  (580, 59, '寧化縣', 3, 0),
  (581, 59, '大田縣', 3, 0),
  (582, 59, '尤溪縣', 3, 0),
  (583, 59, '沙縣', 3, 0),
  (584, 59, '將樂縣', 3, 0),
  (585, 59, '泰寧縣', 3, 0),
  (586, 59, '建寧縣', 3, 0),
  (587, 60, '思明區', 3, 0),
  (588, 60, '海滄區', 3, 0),
  (589, 60, '湖裏區', 3, 0),
  (590, 60, '集美區', 3, 0),
  (591, 60, '同安區', 3, 0),
  (592, 60, '翔安區', 3, 0),
  (593, 61, '薌城區', 3, 0),
  (594, 61, '龍文區', 3, 0),
  (595, 61, '龍海市', 3, 0),
  (596, 61, '雲霄縣', 3, 0),
  (597, 61, '漳浦縣', 3, 0),
  (598, 61, '詔安縣', 3, 0),
  (599, 61, '長泰縣', 3, 0),
  (600, 61, '東山縣', 3, 0),
  (601, 61, '南靖縣', 3, 0),
  (602, 61, '平和縣', 3, 0),
  (603, 61, '華安縣', 3, 0),
  (604, 62, '臯蘭縣', 3, 0),
  (605, 62, '城關區', 3, 0),
  (606, 62, '七裏河區', 3, 0),
  (607, 62, '西固區', 3, 0),
  (608, 62, '安寧區', 3, 0),
  (609, 62, '紅古區', 3, 0),
  (610, 62, '永登縣', 3, 0),
  (611, 62, '榆中縣', 3, 0),
  (612, 63, '白銀區', 3, 0),
  (613, 63, '平川區', 3, 0),
  (614, 63, '會寧縣', 3, 0),
  (615, 63, '景泰縣', 3, 0),
  (616, 63, '靖遠縣', 3, 0),
  (617, 64, '臨桃縣', 3, 0),
  (618, 64, '隴西縣', 3, 0),
  (619, 64, '通渭縣', 3, 0),
  (620, 64, '渭源縣', 3, 0),
  (621, 64, '漳縣', 3, 0),
  (622, 64, '岷縣', 3, 0),
  (623, 64, '安定區', 3, 0),
  (624, 64, '安定區', 3, 0),
  (625, 65, '合作市', 3, 0),
  (626, 65, '臨潭縣', 3, 0),
  (627, 65, '卓尼縣', 3, 0),
  (628, 65, '舟曲縣', 3, 0),
  (629, 65, '叠部縣', 3, 0),
  (630, 65, '瑪曲縣', 3, 0),
  (631, 65, '碌曲縣', 3, 0),
  (632, 65, '夏河縣', 3, 0),
  (633, 66, '嘉峪關市', 3, 0),
  (634, 67, '金川區', 3, 0),
  (635, 67, '永昌縣', 3, 0),
  (636, 68, '肅州區', 3, 0),
  (637, 68, '玉門市', 3, 0),
  (638, 68, '敦煌市', 3, 0),
  (639, 68, '金塔縣', 3, 0),
  (640, 68, '瓜州縣', 3, 0),
  (641, 68, '肅北', 3, 0),
  (642, 68, '阿克塞', 3, 0),
  (643, 69, '臨夏市', 3, 0),
  (644, 69, '臨夏縣', 3, 0),
  (645, 69, '康樂縣', 3, 0),
  (646, 69, '永靖縣', 3, 0),
  (647, 69, '廣河縣', 3, 0),
  (648, 69, '和政縣', 3, 0),
  (649, 69, '東鄉族自治縣', 3, 0),
  (650, 69, '積石山', 3, 0),
  (651, 70, '成縣', 3, 0),
  (652, 70, '徽縣', 3, 0),
  (653, 70, '康縣', 3, 0),
  (654, 70, '禮縣', 3, 0),
  (655, 70, '兩當縣', 3, 0),
  (656, 70, '文縣', 3, 0),
  (657, 70, '西和縣', 3, 0),
  (658, 70, '宕昌縣', 3, 0),
  (659, 70, '武都區', 3, 0),
  (660, 71, '崇信縣', 3, 0),
  (661, 71, '華亭縣', 3, 0),
  (662, 71, '靜寧縣', 3, 0),
  (663, 71, '靈臺縣', 3, 0),
  (664, 71, '崆峒區', 3, 0),
  (665, 71, '莊浪縣', 3, 0),
  (666, 71, '涇川縣', 3, 0),
  (667, 72, '合水縣', 3, 0),
  (668, 72, '華池縣', 3, 0),
  (669, 72, '環縣', 3, 0),
  (670, 72, '寧縣', 3, 0),
  (671, 72, '慶城縣', 3, 0),
  (672, 72, '西峰區', 3, 0),
  (673, 72, '鎮原縣', 3, 0),
  (674, 72, '正寧縣', 3, 0),
  (675, 73, '甘谷縣', 3, 0),
  (676, 73, '秦安縣', 3, 0),
  (677, 73, '清水縣', 3, 0),
  (678, 73, '秦州區', 3, 0),
  (679, 73, '麥積區', 3, 0),
  (680, 73, '武山縣', 3, 0),
  (681, 73, '張家川', 3, 0),
  (682, 74, '古浪縣', 3, 0),
  (683, 74, '民勤縣', 3, 0),
  (684, 74, '天祝', 3, 0),
  (685, 74, '涼州區', 3, 0),
  (686, 75, '高臺縣', 3, 0),
  (687, 75, '臨澤縣', 3, 0),
  (688, 75, '民樂縣', 3, 0),
  (689, 75, '山丹縣', 3, 0),
  (690, 75, '肅南', 3, 0),
  (691, 75, '甘州區', 3, 0),
  (692, 76, '從化市', 3, 0),
  (693, 76, '天河區', 3, 0),
  (694, 76, '東山區', 3, 0),
  (695, 76, '白雲區', 3, 0),
  (696, 76, '海珠區', 3, 0),
  (697, 76, '荔灣區', 3, 0),
  (698, 76, '越秀區', 3, 0),
  (699, 76, '黃埔區', 3, 0),
  (700, 76, '番禺區', 3, 0),
  (701, 76, '花都區', 3, 0),
  (702, 76, '增城區', 3, 0),
  (703, 76, '從化區', 3, 0),
  (704, 76, '市郊', 3, 0),
  (705, 77, '福田區', 3, 0),
  (706, 77, '羅湖區', 3, 0),
  (707, 77, '南山區', 3, 0),
  (708, 77, '寶安區', 3, 0),
  (709, 77, '龍崗區', 3, 0),
  (710, 77, '鹽田區', 3, 0),
  (711, 78, '湘橋區', 3, 0),
  (712, 78, '潮安縣', 3, 0),
  (713, 78, '饒平縣', 3, 0),
  (714, 79, '南城區', 3, 0),
  (715, 79, '東城區', 3, 0),
  (716, 79, '萬江區', 3, 0),
  (717, 79, '莞城區', 3, 0),
  (718, 79, '石龍鎮', 3, 0),
  (719, 79, '虎門鎮', 3, 0),
  (720, 79, '麻湧鎮', 3, 0),
  (721, 79, '道浩鎮', 3, 0),
  (722, 79, '石碣鎮', 3, 0),
  (723, 79, '沙田鎮', 3, 0),
  (724, 79, '望牛墩鎮', 3, 0),
  (725, 79, '洪梅鎮', 3, 0),
  (726, 79, '茶山鎮', 3, 0),
  (727, 79, '寮步鎮', 3, 0),
  (728, 79, '大嶺山鎮', 3, 0),
  (729, 79, '大朗鎮', 3, 0),
  (730, 79, '黃江鎮', 3, 0),
  (731, 79, '樟木頭', 3, 0),
  (732, 79, '鳳崗鎮', 3, 0),
  (733, 79, '塘廈鎮', 3, 0),
  (734, 79, '謝崗鎮', 3, 0),
  (735, 79, '厚街鎮', 3, 0),
  (736, 79, '清溪鎮', 3, 0),
  (737, 79, '常平鎮', 3, 0),
  (738, 79, '橋頭鎮', 3, 0),
  (739, 79, '橫瀝鎮', 3, 0),
  (740, 79, '東坑鎮', 3, 0),
  (741, 79, '企石鎮', 3, 0),
  (742, 79, '石排鎮', 3, 0),
  (743, 79, '長安鎮', 3, 0),
  (744, 79, '中堂鎮', 3, 0),
  (745, 79, '高埗鎮', 3, 0),
  (746, 80, '禪城區', 3, 0),
  (747, 80, '南海區', 3, 0),
  (748, 80, '順德區', 3, 0),
  (749, 80, '三水區', 3, 0),
  (750, 80, '高明區', 3, 0),
  (751, 81, '東源縣', 3, 0),
  (752, 81, '和平縣', 3, 0),
  (753, 81, '源城區', 3, 0),
  (754, 81, '連平縣', 3, 0),
  (755, 81, '龍川縣', 3, 0),
  (756, 81, '紫金縣', 3, 0),
  (757, 82, '惠陽區', 3, 0),
  (758, 82, '惠城區', 3, 0),
  (759, 82, '大亞灣', 3, 0),
  (760, 82, '博羅縣', 3, 0),
  (761, 82, '惠東縣', 3, 0),
  (762, 82, '龍門縣', 3, 0),
  (763, 83, '江海區', 3, 0),
  (764, 83, '蓬江區', 3, 0),
  (765, 83, '新會區', 3, 0),
  (766, 83, '臺山市', 3, 0),
  (767, 83, '開平市', 3, 0),
  (768, 83, '鶴山市', 3, 0),
  (769, 83, '恩平市', 3, 0),
  (770, 84, '榕城區', 3, 0),
  (771, 84, '普寧市', 3, 0),
  (772, 84, '揭東縣', 3, 0),
  (773, 84, '揭西縣', 3, 0),
  (774, 84, '惠來縣', 3, 0),
  (775, 85, '茂南區', 3, 0),
  (776, 85, '茂港區', 3, 0),
  (777, 85, '高州市', 3, 0),
  (778, 85, '化州市', 3, 0),
  (779, 85, '信宜市', 3, 0),
  (780, 85, '電白縣', 3, 0),
  (781, 86, '梅縣', 3, 0),
  (782, 86, '梅江區', 3, 0),
  (783, 86, '興寧市', 3, 0),
  (784, 86, '大埔縣', 3, 0),
  (785, 86, '豐順縣', 3, 0),
  (786, 86, '五華縣', 3, 0),
  (787, 86, '平遠縣', 3, 0),
  (788, 86, '蕉嶺縣', 3, 0),
  (789, 87, '清城區', 3, 0),
  (790, 87, '英德市', 3, 0),
  (791, 87, '連州市', 3, 0),
  (792, 87, '佛岡縣', 3, 0),
  (793, 87, '陽山縣', 3, 0),
  (794, 87, '清新縣', 3, 0),
  (795, 87, '連山', 3, 0),
  (796, 87, '連南', 3, 0),
  (797, 88, '南澳縣', 3, 0),
  (798, 88, '潮陽區', 3, 0),
  (799, 88, '澄海區', 3, 0),
  (800, 88, '龍湖區', 3, 0),
  (801, 88, '金平區', 3, 0),
  (802, 88, '豪江區', 3, 0),
  (803, 88, '潮南區', 3, 0),
  (804, 89, '城區', 3, 0),
  (805, 89, '陸豐市', 3, 0),
  (806, 89, '海豐縣', 3, 0),
  (807, 89, '陸河縣', 3, 0),
  (808, 90, '曲江縣', 3, 0),
  (809, 90, '湞江區', 3, 0),
  (810, 90, '武江區', 3, 0),
  (811, 90, '曲江區', 3, 0),
  (812, 90, '樂昌市', 3, 0),
  (813, 90, '南雄市', 3, 0),
  (814, 90, '始興縣', 3, 0),
  (815, 90, '仁化縣', 3, 0),
  (816, 90, '翁源縣', 3, 0),
  (817, 90, '新豐縣', 3, 0),
  (818, 90, '乳源', 3, 0),
  (819, 91, '江城區', 3, 0),
  (820, 91, '陽春市', 3, 0),
  (821, 91, '陽西縣', 3, 0),
  (822, 91, '陽東縣', 3, 0),
  (823, 92, '雲城區', 3, 0),
  (824, 92, '羅定市', 3, 0),
  (825, 92, '新興縣', 3, 0),
  (826, 92, '郁南縣', 3, 0),
  (827, 92, '雲安縣', 3, 0),
  (828, 93, '赤坎區', 3, 0),
  (829, 93, '霞山區', 3, 0),
  (830, 93, '坡頭區', 3, 0),
  (831, 93, '麻章區', 3, 0),
  (832, 93, '廉江市', 3, 0),
  (833, 93, '雷州市', 3, 0),
  (834, 93, '吳川市', 3, 0),
  (835, 93, '遂溪縣', 3, 0),
  (836, 93, '徐聞縣', 3, 0),
  (837, 94, '肇慶市', 3, 0),
  (838, 94, '高要市', 3, 0),
  (839, 94, '四會市', 3, 0),
  (840, 94, '廣寧縣', 3, 0),
  (841, 94, '懷集縣', 3, 0),
  (842, 94, '封開縣', 3, 0),
  (843, 94, '德慶縣', 3, 0),
  (844, 95, '石岐街道', 3, 0),
  (845, 95, '東區街道', 3, 0),
  (846, 95, '西區街道', 3, 0),
  (847, 95, '環城街道', 3, 0),
  (848, 95, '中山港街道', 3, 0),
  (849, 95, '五桂山街道', 3, 0),
  (850, 96, '香洲區', 3, 0),
  (851, 96, '鬥門區', 3, 0),
  (852, 96, '金灣區', 3, 0),
  (853, 97, '邕寧區', 3, 0),
  (854, 97, '青秀區', 3, 0),
  (855, 97, '興寧區', 3, 0),
  (856, 97, '良慶區', 3, 0),
  (857, 97, '西鄉塘區', 3, 0),
  (858, 97, '江南區', 3, 0),
  (859, 97, '武鳴縣', 3, 0),
  (860, 97, '隆安縣', 3, 0),
  (861, 97, '馬山縣', 3, 0),
  (862, 97, '上林縣', 3, 0),
  (863, 97, '賓陽縣', 3, 0),
  (864, 97, '橫縣', 3, 0),
  (865, 98, '秀峰區', 3, 0),
  (866, 98, '疊彩區', 3, 0),
  (867, 98, '象山區', 3, 0),
  (868, 98, '七星區', 3, 0),
  (869, 98, '雁山區', 3, 0),
  (870, 98, '陽朔縣', 3, 0),
  (871, 98, '臨桂縣', 3, 0),
  (872, 98, '靈川縣', 3, 0),
  (873, 98, '全州縣', 3, 0),
  (874, 98, '平樂縣', 3, 0),
  (875, 98, '興安縣', 3, 0),
  (876, 98, '灌陽縣', 3, 0),
  (877, 98, '荔浦縣', 3, 0),
  (878, 98, '資源縣', 3, 0),
  (879, 98, '永福縣', 3, 0),
  (880, 98, '龍勝', 3, 0),
  (881, 98, '恭城', 3, 0),
  (882, 99, '右江區', 3, 0),
  (883, 99, '淩雲縣', 3, 0),
  (884, 99, '平果縣', 3, 0),
  (885, 99, '西林縣', 3, 0),
  (886, 99, '樂業縣', 3, 0),
  (887, 99, '德保縣', 3, 0),
  (888, 99, '田林縣', 3, 0),
  (889, 99, '田陽縣', 3, 0),
  (890, 99, '靖西縣', 3, 0),
  (891, 99, '田東縣', 3, 0),
  (892, 99, '那坡縣', 3, 0),
  (893, 99, '隆林', 3, 0),
  (894, 100, '海城區', 3, 0),
  (895, 100, '銀海區', 3, 0),
  (896, 100, '鐵山港區', 3, 0),
  (897, 100, '合浦縣', 3, 0),
  (898, 101, '江州區', 3, 0),
  (899, 101, '憑祥市', 3, 0),
  (900, 101, '寧明縣', 3, 0),
  (901, 101, '扶綏縣', 3, 0),
  (902, 101, '龍州縣', 3, 0),
  (903, 101, '大新縣', 3, 0),
  (904, 101, '天等縣', 3, 0),
  (905, 102, '港口區', 3, 0),
  (906, 102, '防城區', 3, 0),
  (907, 102, '東興市', 3, 0),
  (908, 102, '上思縣', 3, 0),
  (909, 103, '港北區', 3, 0),
  (910, 103, '港南區', 3, 0),
  (911, 103, '覃塘區', 3, 0),
  (912, 103, '桂平市', 3, 0),
  (913, 103, '平南縣', 3, 0),
  (914, 104, '金城江區', 3, 0),
  (915, 104, '宜州市', 3, 0),
  (916, 104, '天峨縣', 3, 0),
  (917, 104, '鳳山縣', 3, 0),
  (918, 104, '南丹縣', 3, 0),
  (919, 104, '東蘭縣', 3, 0),
  (920, 104, '都安', 3, 0),
  (921, 104, '羅城', 3, 0),
  (922, 104, '巴馬', 3, 0),
  (923, 104, '環江', 3, 0),
  (924, 104, '大化', 3, 0),
  (925, 105, '八步區', 3, 0),
  (926, 105, '鐘山縣', 3, 0),
  (927, 105, '昭平縣', 3, 0),
  (928, 105, '富川', 3, 0),
  (929, 106, '興賓區', 3, 0),
  (930, 106, '合山市', 3, 0),
  (931, 106, '象州縣', 3, 0),
  (932, 106, '武宣縣', 3, 0),
  (933, 106, '忻城縣', 3, 0),
  (934, 106, '金秀', 3, 0),
  (935, 107, '城中區', 3, 0),
  (936, 107, '魚峰區', 3, 0),
  (937, 107, '柳北區', 3, 0),
  (938, 107, '柳南區', 3, 0),
  (939, 107, '柳江縣', 3, 0),
  (940, 107, '柳城縣', 3, 0),
  (941, 107, '鹿寨縣', 3, 0),
  (942, 107, '融安縣', 3, 0),
  (943, 107, '融水', 3, 0),
  (944, 107, '三江', 3, 0),
  (945, 108, '欽南區', 3, 0),
  (946, 108, '欽北區', 3, 0),
  (947, 108, '靈山縣', 3, 0),
  (948, 108, '浦北縣', 3, 0),
  (949, 109, '萬秀區', 3, 0),
  (950, 109, '蝶山區', 3, 0),
  (951, 109, '長洲區', 3, 0),
  (952, 109, '岑溪市', 3, 0),
  (953, 109, '蒼梧縣', 3, 0),
  (954, 109, '藤縣', 3, 0),
  (955, 109, '蒙山縣', 3, 0),
  (956, 110, '玉州區', 3, 0),
  (957, 110, '北流市', 3, 0),
  (958, 110, '容縣', 3, 0),
  (959, 110, '陸川縣', 3, 0),
  (960, 110, '博白縣', 3, 0),
  (961, 110, '興業縣', 3, 0),
  (962, 111, '南明區', 3, 0),
  (963, 111, '雲巖區', 3, 0),
  (964, 111, '花溪區', 3, 0),
  (965, 111, '烏當區', 3, 0),
  (966, 111, '白雲區', 3, 0),
  (967, 111, '小河區', 3, 0),
  (968, 111, '金陽新區', 3, 0),
  (969, 111, '新天園區', 3, 0),
  (970, 111, '清鎮市', 3, 0),
  (971, 111, '開陽縣', 3, 0),
  (972, 111, '修文縣', 3, 0),
  (973, 111, '息烽縣', 3, 0),
  (974, 112, '西秀區', 3, 0),
  (975, 112, '關嶺', 3, 0),
  (976, 112, '鎮寧', 3, 0),
  (977, 112, '紫雲', 3, 0),
  (978, 112, '平壩縣', 3, 0),
  (979, 112, '普定縣', 3, 0),
  (980, 113, '畢節市', 3, 0),
  (981, 113, '大方縣', 3, 0),
  (982, 113, '黔西縣', 3, 0),
  (983, 113, '金沙縣', 3, 0),
  (984, 113, '織金縣', 3, 0),
  (985, 113, '納雍縣', 3, 0),
  (986, 113, '赫章縣', 3, 0),
  (987, 113, '威寧', 3, 0),
  (988, 114, '鐘山區', 3, 0),
  (989, 114, '六枝特區', 3, 0),
  (990, 114, '水城縣', 3, 0),
  (991, 114, '盤縣', 3, 0),
  (992, 115, '凱裏市', 3, 0),
  (993, 115, '黃平縣', 3, 0),
  (994, 115, '施秉縣', 3, 0),
  (995, 115, '三穗縣', 3, 0),
  (996, 115, '鎮遠縣', 3, 0),
  (997, 115, '岑鞏縣', 3, 0),
  (998, 115, '天柱縣', 3, 0),
  (999, 115, '錦屏縣', 3, 0),
  (1000, 115, '劍河縣', 3, 0),
  (1001, 115, '臺江縣', 3, 0),
  (1002, 115, '黎平縣', 3, 0),
  (1003, 115, '榕江縣', 3, 0),
  (1004, 115, '從江縣', 3, 0),
  (1005, 115, '雷山縣', 3, 0),
  (1006, 115, '麻江縣', 3, 0),
  (1007, 115, '丹寨縣', 3, 0),
  (1008, 116, '都勻市', 3, 0),
  (1009, 116, '福泉市', 3, 0),
  (1010, 116, '荔波縣', 3, 0),
  (1011, 116, '貴定縣', 3, 0),
  (1012, 116, '甕安縣', 3, 0),
  (1013, 116, '獨山縣', 3, 0),
  (1014, 116, '平塘縣', 3, 0),
  (1015, 116, '羅甸縣', 3, 0),
  (1016, 116, '長順縣', 3, 0),
  (1017, 116, '龍裏縣', 3, 0),
  (1018, 116, '惠水縣', 3, 0),
  (1019, 116, '三都', 3, 0),
  (1020, 117, '興義市', 3, 0),
  (1021, 117, '興仁縣', 3, 0),
  (1022, 117, '普安縣', 3, 0),
  (1023, 117, '晴隆縣', 3, 0),
  (1024, 117, '貞豐縣', 3, 0),
  (1025, 117, '望謨縣', 3, 0),
  (1026, 117, '冊亨縣', 3, 0),
  (1027, 117, '安龍縣', 3, 0),
  (1028, 118, '銅仁市', 3, 0),
  (1029, 118, '江口縣', 3, 0),
  (1030, 118, '石阡縣', 3, 0),
  (1031, 118, '思南縣', 3, 0),
  (1032, 118, '德江縣', 3, 0),
  (1033, 118, '玉屏', 3, 0),
  (1034, 118, '印江', 3, 0),
  (1035, 118, '沿河', 3, 0),
  (1036, 118, '松桃', 3, 0),
  (1037, 118, '萬山特區', 3, 0),
  (1038, 119, '紅花崗區', 3, 0),
  (1039, 119, '務川縣', 3, 0),
  (1040, 119, '道真縣', 3, 0),
  (1041, 119, '匯川區', 3, 0),
  (1042, 119, '赤水市', 3, 0),
  (1043, 119, '仁懷市', 3, 0),
  (1044, 119, '遵義縣', 3, 0),
  (1045, 119, '桐梓縣', 3, 0),
  (1046, 119, '綏陽縣', 3, 0),
  (1047, 119, '正安縣', 3, 0),
  (1048, 119, '鳳岡縣', 3, 0),
  (1049, 119, '湄潭縣', 3, 0),
  (1050, 119, '余慶縣', 3, 0),
  (1051, 119, '習水縣', 3, 0),
  (1052, 119, '道真', 3, 0),
  (1053, 119, '務川', 3, 0),
  (1054, 120, '秀英區', 3, 0),
  (1055, 120, '龍華區', 3, 0),
  (1056, 120, '瓊山區', 3, 0),
  (1057, 120, '美蘭區', 3, 0),
  (1058, 137, '市區', 3, 0),
  (1059, 137, '洋浦開發區', 3, 0),
  (1060, 137, '那大鎮', 3, 0),
  (1061, 137, '王五鎮', 3, 0),
  (1062, 137, '雅星鎮', 3, 0),
  (1063, 137, '大成鎮', 3, 0),
  (1064, 137, '中和鎮', 3, 0),
  (1065, 137, '峨蔓鎮', 3, 0),
  (1066, 137, '南豐鎮', 3, 0),
  (1067, 137, '白馬井鎮', 3, 0),
  (1068, 137, '蘭洋鎮', 3, 0),
  (1069, 137, '和慶鎮', 3, 0),
  (1070, 137, '海頭鎮', 3, 0),
  (1071, 137, '排浦鎮', 3, 0),
  (1072, 137, '東成鎮', 3, 0),
  (1073, 137, '光村鎮', 3, 0),
  (1074, 137, '木棠鎮', 3, 0),
  (1075, 137, '新州鎮', 3, 0),
  (1076, 137, '三都鎮', 3, 0),
  (1077, 137, '其他', 3, 0),
  (1078, 138, '長安區', 3, 0),
  (1079, 138, '橋東區', 3, 0),
  (1080, 138, '橋西區', 3, 0),
  (1081, 138, '新華區', 3, 0),
  (1082, 138, '裕華區', 3, 0),
  (1083, 138, '井陘礦區', 3, 0),
  (1084, 138, '高新區', 3, 0),
  (1085, 138, '辛集市', 3, 0),
  (1086, 138, '槁城市', 3, 0),
  (1087, 138, '晉州市', 3, 0),
  (1088, 138, '新樂市', 3, 0),
  (1089, 138, '鹿泉市', 3, 0),
  (1090, 138, '井陘縣', 3, 0),
  (1091, 138, '正定縣', 3, 0),
  (1092, 138, '欒城縣', 3, 0),
  (1093, 138, '行唐縣', 3, 0),
  (1094, 138, '靈壽縣', 3, 0),
  (1095, 138, '高邑縣', 3, 0),
  (1096, 138, '深澤縣', 3, 0),
  (1097, 138, '贊皇縣', 3, 0),
  (1098, 138, '無極縣', 3, 0),
  (1099, 138, '平山縣', 3, 0),
  (1100, 138, '元氏縣', 3, 0),
  (1101, 138, '趙縣', 3, 0),
  (1102, 139, '新市區', 3, 0),
  (1103, 139, '南市區', 3, 0),
  (1104, 139, '北市區', 3, 0),
  (1105, 139, '涿州市', 3, 0),
  (1106, 139, '定州市', 3, 0),
  (1107, 139, '安國市', 3, 0),
  (1108, 139, '高碑店市', 3, 0),
  (1109, 139, '滿城縣', 3, 0),
  (1110, 139, '清苑縣', 3, 0),
  (1111, 139, '淶水縣', 3, 0),
  (1112, 139, '阜平縣', 3, 0),
  (1113, 139, '徐水縣', 3, 0),
  (1114, 139, '定興縣', 3, 0),
  (1115, 139, '唐縣', 3, 0),
  (1116, 139, '高陽縣', 3, 0),
  (1117, 139, '容城縣', 3, 0),
  (1118, 139, '淶源縣', 3, 0),
  (1119, 139, '望都縣', 3, 0),
  (1120, 139, '安新縣', 3, 0),
  (1121, 139, '易縣', 3, 0),
  (1122, 139, '曲陽縣', 3, 0),
  (1123, 139, '蠡縣', 3, 0),
  (1124, 139, '順平縣', 3, 0),
  (1125, 139, '博野縣', 3, 0),
  (1126, 139, '雄縣', 3, 0),
  (1127, 140, '運河區', 3, 0),
  (1128, 140, '新華區', 3, 0),
  (1129, 140, '泊頭市', 3, 0),
  (1130, 140, '任丘市', 3, 0),
  (1131, 140, '黃驊市', 3, 0),
  (1132, 140, '河間市', 3, 0),
  (1133, 140, '滄縣', 3, 0),
  (1134, 140, '青縣', 3, 0),
  (1135, 140, '東光縣', 3, 0),
  (1136, 140, '海興縣', 3, 0),
  (1137, 140, '鹽山縣', 3, 0),
  (1138, 140, '肅寧縣', 3, 0),
  (1139, 140, '南皮縣', 3, 0),
  (1140, 140, '吳橋縣', 3, 0),
  (1141, 140, '獻縣', 3, 0),
  (1142, 140, '孟村', 3, 0),
  (1143, 141, '雙橋區', 3, 0),
  (1144, 141, '雙灤區', 3, 0),
  (1145, 141, '鷹手營子礦區', 3, 0),
  (1146, 141, '承德縣', 3, 0),
  (1147, 141, '興隆縣', 3, 0),
  (1148, 141, '平泉縣', 3, 0),
  (1149, 141, '灤平縣', 3, 0),
  (1150, 141, '隆化縣', 3, 0),
  (1151, 141, '豐寧', 3, 0),
  (1152, 141, '寬城', 3, 0),
  (1153, 141, '圍場', 3, 0),
  (1154, 142, '從臺區', 3, 0),
  (1155, 142, '復興區', 3, 0),
  (1156, 142, '邯山區', 3, 0),
  (1157, 142, '峰峰礦區', 3, 0),
  (1158, 142, '武安市', 3, 0),
  (1159, 142, '邯鄲縣', 3, 0),
  (1160, 142, '臨漳縣', 3, 0),
  (1161, 142, '成安縣', 3, 0),
  (1162, 142, '大名縣', 3, 0),
  (1163, 142, '涉縣', 3, 0),
  (1164, 142, '磁縣', 3, 0),
  (1165, 142, '肥鄉縣', 3, 0),
  (1166, 142, '永年縣', 3, 0),
  (1167, 142, '邱縣', 3, 0),
  (1168, 142, '雞澤縣', 3, 0),
  (1169, 142, '廣平縣', 3, 0),
  (1170, 142, '館陶縣', 3, 0),
  (1171, 142, '魏縣', 3, 0),
  (1172, 142, '曲周縣', 3, 0),
  (1173, 143, '桃城區', 3, 0),
  (1174, 143, '冀州市', 3, 0),
  (1175, 143, '深州市', 3, 0),
  (1176, 143, '棗強縣', 3, 0),
  (1177, 143, '武邑縣', 3, 0),
  (1178, 143, '武強縣', 3, 0),
  (1179, 143, '饒陽縣', 3, 0),
  (1180, 143, '安平縣', 3, 0),
  (1181, 143, '故城縣', 3, 0),
  (1182, 143, '景縣', 3, 0),
  (1183, 143, '阜城縣', 3, 0),
  (1184, 144, '安次區', 3, 0),
  (1185, 144, '廣陽區', 3, 0),
  (1186, 144, '霸州市', 3, 0),
  (1187, 144, '三河市', 3, 0),
  (1188, 144, '固安縣', 3, 0),
  (1189, 144, '永清縣', 3, 0),
  (1190, 144, '香河縣', 3, 0),
  (1191, 144, '大城縣', 3, 0),
  (1192, 144, '文安縣', 3, 0),
  (1193, 144, '大廠', 3, 0),
  (1194, 145, '海港區', 3, 0),
  (1195, 145, '山海關區', 3, 0),
  (1196, 145, '北戴河區', 3, 0),
  (1197, 145, '昌黎縣', 3, 0),
  (1198, 145, '撫寧縣', 3, 0),
  (1199, 145, '盧龍縣', 3, 0),
  (1200, 145, '青龍', 3, 0),
  (1201, 146, '路北區', 3, 0),
  (1202, 146, '路南區', 3, 0),
  (1203, 146, '古冶區', 3, 0),
  (1204, 146, '開平區', 3, 0),
  (1205, 146, '豐南區', 3, 0),
  (1206, 146, '豐潤區', 3, 0),
  (1207, 146, '遵化市', 3, 0),
  (1208, 146, '遷安市', 3, 0),
  (1209, 146, '灤縣', 3, 0),
  (1210, 146, '灤南縣', 3, 0),
  (1211, 146, '樂亭縣', 3, 0),
  (1212, 146, '遷西縣', 3, 0),
  (1213, 146, '玉田縣', 3, 0),
  (1214, 146, '唐海縣', 3, 0),
  (1215, 147, '橋東區', 3, 0),
  (1216, 147, '橋西區', 3, 0),
  (1217, 147, '南宮市', 3, 0),
  (1218, 147, '沙河市', 3, 0),
  (1219, 147, '邢臺縣', 3, 0),
  (1220, 147, '臨城縣', 3, 0),
  (1221, 147, '內丘縣', 3, 0),
  (1222, 147, '柏鄉縣', 3, 0),
  (1223, 147, '隆堯縣', 3, 0),
  (1224, 147, '任縣', 3, 0),
  (1225, 147, '南和縣', 3, 0),
  (1226, 147, '寧晉縣', 3, 0),
  (1227, 147, '巨鹿縣', 3, 0),
  (1228, 147, '新河縣', 3, 0),
  (1229, 147, '廣宗縣', 3, 0),
  (1230, 147, '平鄉縣', 3, 0),
  (1231, 147, '威縣', 3, 0),
  (1232, 147, '清河縣', 3, 0),
  (1233, 147, '臨西縣', 3, 0),
  (1234, 148, '橋西區', 3, 0),
  (1235, 148, '橋東區', 3, 0),
  (1236, 148, '宣化區', 3, 0),
  (1237, 148, '下花園區', 3, 0),
  (1238, 148, '宣化縣', 3, 0),
  (1239, 148, '張北縣', 3, 0),
  (1240, 148, '康保縣', 3, 0),
  (1241, 148, '沽源縣', 3, 0),
  (1242, 148, '尚義縣', 3, 0),
  (1243, 148, '蔚縣', 3, 0),
  (1244, 148, '陽原縣', 3, 0),
  (1245, 148, '懷安縣', 3, 0),
  (1246, 148, '萬全縣', 3, 0),
  (1247, 148, '懷來縣', 3, 0),
  (1248, 148, '涿鹿縣', 3, 0),
  (1249, 148, '赤城縣', 3, 0),
  (1250, 148, '崇禮縣', 3, 0),
  (1251, 149, '金水區', 3, 0),
  (1252, 149, '邙山區', 3, 0),
  (1253, 149, '二七區', 3, 0),
  (1254, 149, '管城區', 3, 0),
  (1255, 149, '中原區', 3, 0),
  (1256, 149, '上街區', 3, 0),
  (1257, 149, '惠濟區', 3, 0),
  (1258, 149, '鄭東新區', 3, 0),
  (1259, 149, '經濟技術開發區', 3, 0),
  (1260, 149, '高新開發區', 3, 0),
  (1261, 149, '出口加工區', 3, 0),
  (1262, 149, '鞏義市', 3, 0),
  (1263, 149, '滎陽市', 3, 0),
  (1264, 149, '新密市', 3, 0),
  (1265, 149, '新鄭市', 3, 0),
  (1266, 149, '登封市', 3, 0),
  (1267, 149, '中牟縣', 3, 0),
  (1268, 150, '西工區', 3, 0),
  (1269, 150, '老城區', 3, 0),
  (1270, 150, '澗西區', 3, 0),
  (1271, 150, '纏河回族區', 3, 0),
  (1272, 150, '洛龍區', 3, 0),
  (1273, 150, '吉利區', 3, 0),
  (1274, 150, '偃師市', 3, 0),
  (1275, 150, '孟津縣', 3, 0),
  (1276, 150, '新安縣', 3, 0),
  (1277, 150, '欒川縣', 3, 0),
  (1278, 150, '嵩縣', 3, 0),
  (1279, 150, '汝陽縣', 3, 0),
  (1280, 150, '宜陽縣', 3, 0),
  (1281, 150, '洛寧縣', 3, 0),
  (1282, 150, '伊川縣', 3, 0),
  (1283, 151, '鼓樓區', 3, 0),
  (1284, 151, '龍亭區', 3, 0),
  (1285, 151, '順河回族區', 3, 0),
  (1286, 151, '金明區', 3, 0),
  (1287, 151, '禹王臺區', 3, 0),
  (1288, 151, '杞縣', 3, 0),
  (1289, 151, '通許縣', 3, 0),
  (1290, 151, '尉氏縣', 3, 0),
  (1291, 151, '開封縣', 3, 0),
  (1292, 151, '蘭考縣', 3, 0),
  (1293, 152, '北關區', 3, 0),
  (1294, 152, '文峰區', 3, 0),
  (1295, 152, '殷都區', 3, 0),
  (1296, 152, '龍安區', 3, 0),
  (1297, 152, '林州市', 3, 0),
  (1298, 152, '安陽縣', 3, 0),
  (1299, 152, '湯陰縣', 3, 0),
  (1300, 152, '滑縣', 3, 0),
  (1301, 152, '內黃縣', 3, 0),
  (1302, 153, '泣濱區', 3, 0),
  (1303, 153, '山城區', 3, 0),
  (1304, 153, '鶴山區', 3, 0),
  (1305, 153, '浚縣', 3, 0),
  (1306, 153, '泣縣', 3, 0),
  (1307, 154, '濟源市', 3, 0),
  (1308, 155, '解放區', 3, 0),
  (1309, 155, '中站區', 3, 0),
  (1310, 155, '馬村區', 3, 0),
  (1311, 155, '山陽區', 3, 0),
  (1312, 155, '沁陽市', 3, 0),
  (1313, 155, '孟州市', 3, 0),
  (1314, 155, '修武縣', 3, 0),
  (1315, 155, '博愛縣', 3, 0),
  (1316, 155, '武陟縣', 3, 0),
  (1317, 155, '溫縣', 3, 0),
  (1318, 156, '臥龍區', 3, 0),
  (1319, 156, '宛城區', 3, 0),
  (1320, 156, '鄧州市', 3, 0),
  (1321, 156, '南召縣', 3, 0),
  (1322, 156, '方城縣', 3, 0),
  (1323, 156, '西峽縣', 3, 0),
  (1324, 156, '鎮平縣', 3, 0),
  (1325, 156, '內鄉縣', 3, 0),
  (1326, 156, '浙川縣', 3, 0),
  (1327, 156, '社旗縣', 3, 0),
  (1328, 156, '唐河縣', 3, 0),
  (1329, 156, '新野縣', 3, 0),
  (1330, 156, '桐柏縣', 3, 0),
  (1331, 157, '新華區', 3, 0),
  (1332, 157, '衛東區', 3, 0),
  (1333, 157, '湛河區', 3, 0),
  (1334, 157, '石龍區', 3, 0),
  (1335, 157, '舞鋼市', 3, 0),
  (1336, 157, '汝州市', 3, 0),
  (1337, 157, '寶豐縣', 3, 0),
  (1338, 157, '葉縣', 3, 0),
  (1339, 157, '魯山縣', 3, 0),
  (1340, 157, '郟縣', 3, 0),
  (1341, 158, '湖濱區', 3, 0),
  (1342, 158, '義馬市', 3, 0),
  (1343, 158, '靈寶市', 3, 0),
  (1344, 158, '澠池縣', 3, 0),
  (1345, 158, '陜縣', 3, 0),
  (1346, 158, '盧氏縣', 3, 0),
  (1347, 159, '梁園區', 3, 0),
  (1348, 159, '睢陽區', 3, 0),
  (1349, 159, '永城市', 3, 0),
  (1350, 159, '民權縣', 3, 0),
  (1351, 159, '睢縣', 3, 0),
  (1352, 159, '寧陵縣', 3, 0),
  (1353, 159, '虞城縣', 3, 0),
  (1354, 159, '柘城縣', 3, 0),
  (1355, 159, '夏邑縣', 3, 0),
  (1356, 160, '衛濱區', 3, 0),
  (1357, 160, '紅旗區', 3, 0),
  (1358, 160, '鳳泉區', 3, 0),
  (1359, 160, '牧野區', 3, 0),
  (1360, 160, '衛輝市', 3, 0),
  (1361, 160, '輝縣市', 3, 0),
  (1362, 160, '新鄉縣', 3, 0),
  (1363, 160, '獲嘉縣', 3, 0),
  (1364, 160, '原陽縣', 3, 0),
  (1365, 160, '延津縣', 3, 0),
  (1366, 160, '封丘縣', 3, 0),
  (1367, 160, '長垣縣', 3, 0),
  (1368, 161, '獅河區', 3, 0),
  (1369, 161, '平橋區', 3, 0),
  (1370, 161, '羅山縣', 3, 0),
  (1371, 161, '光山縣', 3, 0),
  (1372, 161, '新縣', 3, 0),
  (1373, 161, '商城縣', 3, 0),
  (1374, 161, '固始縣', 3, 0),
  (1375, 161, '潢川縣', 3, 0),
  (1376, 161, '淮濱縣', 3, 0),
  (1377, 161, '息縣', 3, 0),
  (1378, 162, '魏都區', 3, 0),
  (1379, 162, '禹州市', 3, 0),
  (1380, 162, '長葛市', 3, 0),
  (1381, 162, '許昌縣', 3, 0),
  (1382, 162, '鄢陵縣', 3, 0),
  (1383, 162, '襄城縣', 3, 0),
  (1384, 163, '川匯區', 3, 0),
  (1385, 163, '項城市', 3, 0),
  (1386, 163, '扶溝縣', 3, 0),
  (1387, 163, '西華縣', 3, 0),
  (1388, 163, '商水縣', 3, 0),
  (1389, 163, '沈丘縣', 3, 0),
  (1390, 163, '鄲城縣', 3, 0),
  (1391, 163, '淮陽縣', 3, 0),
  (1392, 163, '太康縣', 3, 0),
  (1393, 163, '鹿邑縣', 3, 0),
  (1394, 164, '驛城區', 3, 0),
  (1395, 164, '西平縣', 3, 0),
  (1396, 164, '上蔡縣', 3, 0),
  (1397, 164, '平輿縣', 3, 0),
  (1398, 164, '正陽縣', 3, 0),
  (1399, 164, '確山縣', 3, 0),
  (1400, 164, '泌陽縣', 3, 0),
  (1401, 164, '汝南縣', 3, 0),
  (1402, 164, '遂平縣', 3, 0),
  (1403, 164, '新蔡縣', 3, 0),
  (1404, 165, '郾城區', 3, 0),
  (1405, 165, '源匯區', 3, 0),
  (1406, 165, '召陵區', 3, 0),
  (1407, 165, '舞陽縣', 3, 0),
  (1408, 165, '臨潁縣', 3, 0),
  (1409, 166, '華龍區', 3, 0),
  (1410, 166, '清豐縣', 3, 0),
  (1411, 166, '南樂縣', 3, 0),
  (1412, 166, '範縣', 3, 0),
  (1413, 166, '臺前縣', 3, 0),
  (1414, 166, '濮陽縣', 3, 0),
  (1415, 167, '道裏區', 3, 0),
  (1416, 167, '南崗區', 3, 0),
  (1417, 167, '動力區', 3, 0),
  (1418, 167, '平房區', 3, 0),
  (1419, 167, '香坊區', 3, 0),
  (1420, 167, '太平區', 3, 0),
  (1421, 167, '道外區', 3, 0),
  (1422, 167, '阿城區', 3, 0),
  (1423, 167, '呼蘭區', 3, 0),
  (1424, 167, '松北區', 3, 0),
  (1425, 167, '尚誌市', 3, 0),
  (1426, 167, '雙城市', 3, 0),
  (1427, 167, '五常市', 3, 0),
  (1428, 167, '方正縣', 3, 0),
  (1429, 167, '賓縣', 3, 0),
  (1430, 167, '依蘭縣', 3, 0),
  (1431, 167, '巴彥縣', 3, 0),
  (1432, 167, '通河縣', 3, 0),
  (1433, 167, '木蘭縣', 3, 0),
  (1434, 167, '延壽縣', 3, 0),
  (1435, 168, '薩爾圖區', 3, 0),
  (1436, 168, '紅崗區', 3, 0),
  (1437, 168, '龍鳳區', 3, 0),
  (1438, 168, '讓胡路區', 3, 0),
  (1439, 168, '大同區', 3, 0),
  (1440, 168, '肇州縣', 3, 0),
  (1441, 168, '肇源縣', 3, 0),
  (1442, 168, '林甸縣', 3, 0),
  (1443, 168, '杜爾伯特', 3, 0),
  (1444, 169, '呼瑪縣', 3, 0),
  (1445, 169, '漠河縣', 3, 0),
  (1446, 169, '塔河縣', 3, 0),
  (1447, 170, '興山區', 3, 0),
  (1448, 170, '工農區', 3, 0),
  (1449, 170, '南山區', 3, 0),
  (1450, 170, '興安區', 3, 0),
  (1451, 170, '向陽區', 3, 0),
  (1452, 170, '東山區', 3, 0),
  (1453, 170, '蘿北縣', 3, 0),
  (1454, 170, '綏濱縣', 3, 0),
  (1455, 171, '愛輝區', 3, 0),
  (1456, 171, '五大連池市', 3, 0),
  (1457, 171, '北安市', 3, 0),
  (1458, 171, '嫩江縣', 3, 0),
  (1459, 171, '遜克縣', 3, 0),
  (1460, 171, '孫吳縣', 3, 0),
  (1461, 172, '雞冠區', 3, 0),
  (1462, 172, '恒山區', 3, 0),
  (1463, 172, '城子河區', 3, 0),
  (1464, 172, '滴道區', 3, 0),
  (1465, 172, '梨樹區', 3, 0),
  (1466, 172, '虎林市', 3, 0),
  (1467, 172, '密山市', 3, 0),
  (1468, 172, '雞東縣', 3, 0),
  (1469, 173, '前進區', 3, 0),
  (1470, 173, '郊區', 3, 0),
  (1471, 173, '向陽區', 3, 0),
  (1472, 173, '東風區', 3, 0),
  (1473, 173, '同江市', 3, 0),
  (1474, 173, '富錦市', 3, 0),
  (1475, 173, '樺南縣', 3, 0),
  (1476, 173, '樺川縣', 3, 0),
  (1477, 173, '湯原縣', 3, 0),
  (1478, 173, '撫遠縣', 3, 0),
  (1479, 174, '愛民區', 3, 0),
  (1480, 174, '東安區', 3, 0),
  (1481, 174, '陽明區', 3, 0),
  (1482, 174, '西安區', 3, 0),
  (1483, 174, '綏芬河市', 3, 0),
  (1484, 174, '海林市', 3, 0),
  (1485, 174, '寧安市', 3, 0),
  (1486, 174, '穆棱市', 3, 0),
  (1487, 174, '東寧縣', 3, 0),
  (1488, 174, '林口縣', 3, 0),
  (1489, 175, '桃山區', 3, 0),
  (1490, 175, '新興區', 3, 0),
  (1491, 175, '茄子河區', 3, 0),
  (1492, 175, '勃利縣', 3, 0),
  (1493, 176, '龍沙區', 3, 0),
  (1494, 176, '昂昂溪區', 3, 0),
  (1495, 176, '鐵峰區', 3, 0),
  (1496, 176, '建華區', 3, 0),
  (1497, 176, '富拉爾基區', 3, 0),
  (1498, 176, '碾子山區', 3, 0),
  (1499, 176, '梅裏斯達斡爾區', 3, 0),
  (1500, 176, '訥河市', 3, 0),
  (1501, 176, '龍江縣', 3, 0),
  (1502, 176, '依安縣', 3, 0),
  (1503, 176, '泰來縣', 3, 0),
  (1504, 176, '甘南縣', 3, 0),
  (1505, 176, '富裕縣', 3, 0),
  (1506, 176, '克山縣', 3, 0),
  (1507, 176, '克東縣', 3, 0),
  (1508, 176, '拜泉縣', 3, 0),
  (1509, 177, '尖山區', 3, 0),
  (1510, 177, '嶺東區', 3, 0),
  (1511, 177, '四方臺區', 3, 0),
  (1512, 177, '寶山區', 3, 0),
  (1513, 177, '集賢縣', 3, 0),
  (1514, 177, '友誼縣', 3, 0),
  (1515, 177, '寶清縣', 3, 0),
  (1516, 177, '饒河縣', 3, 0),
  (1517, 178, '北林區', 3, 0),
  (1518, 178, '安達市', 3, 0),
  (1519, 178, '肇東市', 3, 0),
  (1520, 178, '海倫市', 3, 0),
  (1521, 178, '望奎縣', 3, 0),
  (1522, 178, '蘭西縣', 3, 0),
  (1523, 178, '青岡縣', 3, 0),
  (1524, 178, '慶安縣', 3, 0),
  (1525, 178, '明水縣', 3, 0),
  (1526, 178, '綏棱縣', 3, 0),
  (1527, 179, '伊春區', 3, 0),
  (1528, 179, '帶嶺區', 3, 0),
  (1529, 179, '南岔區', 3, 0),
  (1530, 179, '金山屯區', 3, 0),
  (1531, 179, '西林區', 3, 0),
  (1532, 179, '美溪區', 3, 0),
  (1533, 179, '烏馬河區', 3, 0),
  (1534, 179, '翠巒區', 3, 0),
  (1535, 179, '友好區', 3, 0),
  (1536, 179, '上甘嶺區', 3, 0),
  (1537, 179, '五營區', 3, 0),
  (1538, 179, '紅星區', 3, 0),
  (1539, 179, '新青區', 3, 0),
  (1540, 179, '湯旺河區', 3, 0),
  (1541, 179, '烏伊嶺區', 3, 0),
  (1542, 179, '鐵力市', 3, 0),
  (1543, 179, '嘉蔭縣', 3, 0),
  (1544, 180, '江岸區', 3, 0),
  (1545, 180, '武昌區', 3, 0),
  (1546, 180, '江漢區', 3, 0),
  (1547, 180, '硚口區', 3, 0),
  (1548, 180, '漢陽區', 3, 0),
  (1549, 180, '青山區', 3, 0),
  (1550, 180, '洪山區', 3, 0),
  (1551, 180, '東西湖區', 3, 0),
  (1552, 180, '漢南區', 3, 0),
  (1553, 180, '蔡甸區', 3, 0),
  (1554, 180, '江夏區', 3, 0),
  (1555, 180, '黃陂區', 3, 0),
  (1556, 180, '新洲區', 3, 0),
  (1557, 180, '經濟開發區', 3, 0),
  (1558, 181, '仙桃市', 3, 0),
  (1559, 182, '鄂城區', 3, 0),
  (1560, 182, '華容區', 3, 0),
  (1561, 182, '梁子湖區', 3, 0),
  (1562, 183, '黃州區', 3, 0),
  (1563, 183, '麻城市', 3, 0),
  (1564, 183, '武穴市', 3, 0),
  (1565, 183, '團風縣', 3, 0),
  (1566, 183, '紅安縣', 3, 0),
  (1567, 183, '羅田縣', 3, 0),
  (1568, 183, '英山縣', 3, 0),
  (1569, 183, '稀水縣', 3, 0),
  (1570, 183, '蘄春縣', 3, 0),
  (1571, 183, '黃梅縣', 3, 0),
  (1572, 184, '黃石港區', 3, 0),
  (1573, 184, '西塞山區', 3, 0),
  (1574, 184, '下陸區', 3, 0),
  (1575, 184, '鐵山區', 3, 0),
  (1576, 184, '大冶市', 3, 0),
  (1577, 184, '陽新縣', 3, 0),
  (1578, 185, '東寶區', 3, 0),
  (1579, 185, '掇刀區', 3, 0),
  (1580, 185, '鐘祥市', 3, 0),
  (1581, 185, '京山縣', 3, 0),
  (1582, 185, '沙洋縣', 3, 0),
  (1583, 186, '沙市區', 3, 0),
  (1584, 186, '荊州區', 3, 0),
  (1585, 186, '石首市', 3, 0),
  (1586, 186, '洪湖市', 3, 0),
  (1587, 186, '松滋市', 3, 0),
  (1588, 186, '公安縣', 3, 0),
  (1589, 186, '監利縣', 3, 0),
  (1590, 186, '江陵縣', 3, 0),
  (1591, 187, '潛江市', 3, 0),
  (1592, 188, '神農架林區', 3, 0),
  (1593, 189, '張灣區', 3, 0),
  (1594, 189, '茅箭區', 3, 0),
  (1595, 189, '丹江口市', 3, 0),
  (1596, 189, '鄖縣', 3, 0),
  (1597, 189, '鄖西縣', 3, 0),
  (1598, 189, '竹山縣', 3, 0),
  (1599, 189, '竹溪縣', 3, 0),
  (1600, 189, '房縣', 3, 0),
  (1601, 190, '曾都區', 3, 0),
  (1602, 190, '廣水市', 3, 0),
  (1603, 191, '天門市', 3, 0),
  (1604, 192, '鹹安區', 3, 0),
  (1605, 192, '赤壁市', 3, 0),
  (1606, 192, '嘉魚縣', 3, 0),
  (1607, 192, '通城縣', 3, 0),
  (1608, 192, '崇陽縣', 3, 0),
  (1609, 192, '通山縣', 3, 0),
  (1610, 193, '襄城區', 3, 0),
  (1611, 193, '樊城區', 3, 0),
  (1612, 193, '襄陽區', 3, 0),
  (1613, 193, '老河口市', 3, 0),
  (1614, 193, '棗陽市', 3, 0),
  (1615, 193, '宜城市', 3, 0),
  (1616, 193, '南漳縣', 3, 0),
  (1617, 193, '谷城縣', 3, 0),
  (1618, 193, '保康縣', 3, 0),
  (1619, 194, '孝南區', 3, 0),
  (1620, 194, '應城市', 3, 0),
  (1621, 194, '安陸市', 3, 0),
  (1622, 194, '漢川市', 3, 0),
  (1623, 194, '孝昌縣', 3, 0),
  (1624, 194, '大悟縣', 3, 0),
  (1625, 194, '雲夢縣', 3, 0),
  (1626, 195, '長陽', 3, 0),
  (1627, 195, '五峰', 3, 0),
  (1628, 195, '西陵區', 3, 0),
  (1629, 195, '伍家崗區', 3, 0),
  (1630, 195, '點軍區', 3, 0),
  (1631, 195, '猇亭區', 3, 0),
  (1632, 195, '夷陵區', 3, 0),
  (1633, 195, '宜都市', 3, 0),
  (1634, 195, '當陽市', 3, 0),
  (1635, 195, '枝江市', 3, 0),
  (1636, 195, '遠安縣', 3, 0),
  (1637, 195, '興山縣', 3, 0),
  (1638, 195, '秭歸縣', 3, 0),
  (1639, 196, '恩施市', 3, 0),
  (1640, 196, '利川市', 3, 0),
  (1641, 196, '建始縣', 3, 0),
  (1642, 196, '巴東縣', 3, 0),
  (1643, 196, '宣恩縣', 3, 0),
  (1644, 196, '鹹豐縣', 3, 0),
  (1645, 196, '來鳳縣', 3, 0),
  (1646, 196, '鶴峰縣', 3, 0),
  (1647, 197, '嶽麓區', 3, 0),
  (1648, 197, '芙蓉區', 3, 0),
  (1649, 197, '天心區', 3, 0),
  (1650, 197, '開福區', 3, 0),
  (1651, 197, '雨花區', 3, 0),
  (1652, 197, '開發區', 3, 0),
  (1653, 197, '瀏陽市', 3, 0),
  (1654, 197, '長沙縣', 3, 0),
  (1655, 197, '望城縣', 3, 0),
  (1656, 197, '寧鄉縣', 3, 0),
  (1657, 198, '永定區', 3, 0),
  (1658, 198, '武陵源區', 3, 0),
  (1659, 198, '慈利縣', 3, 0),
  (1660, 198, '桑植縣', 3, 0),
  (1661, 199, '武陵區', 3, 0),
  (1662, 199, '鼎城區', 3, 0),
  (1663, 199, '津市市', 3, 0),
  (1664, 199, '安鄉縣', 3, 0),
  (1665, 199, '漢壽縣', 3, 0),
  (1666, 199, '澧縣', 3, 0),
  (1667, 199, '臨澧縣', 3, 0),
  (1668, 199, '桃源縣', 3, 0),
  (1669, 199, '石門縣', 3, 0),
  (1670, 200, '北湖區', 3, 0),
  (1671, 200, '蘇仙區', 3, 0),
  (1672, 200, '資興市', 3, 0),
  (1673, 200, '桂陽縣', 3, 0),
  (1674, 200, '宜章縣', 3, 0),
  (1675, 200, '永興縣', 3, 0),
  (1676, 200, '嘉禾縣', 3, 0),
  (1677, 200, '臨武縣', 3, 0),
  (1678, 200, '汝城縣', 3, 0),
  (1679, 200, '桂東縣', 3, 0),
  (1680, 200, '安仁縣', 3, 0),
  (1681, 201, '雁峰區', 3, 0),
  (1682, 201, '珠暉區', 3, 0),
  (1683, 201, '石鼓區', 3, 0),
  (1684, 201, '蒸湘區', 3, 0),
  (1685, 201, '南嶽區', 3, 0),
  (1686, 201, '耒陽市', 3, 0),
  (1687, 201, '常寧市', 3, 0),
  (1688, 201, '衡陽縣', 3, 0),
  (1689, 201, '衡南縣', 3, 0),
  (1690, 201, '衡山縣', 3, 0),
  (1691, 201, '衡東縣', 3, 0),
  (1692, 201, '祁東縣', 3, 0),
  (1693, 202, '鶴城區', 3, 0),
  (1694, 202, '靖州', 3, 0),
  (1695, 202, '麻陽', 3, 0),
  (1696, 202, '通道', 3, 0),
  (1697, 202, '新晃', 3, 0),
  (1698, 202, '芝江', 3, 0),
  (1699, 202, '元陵縣', 3, 0),
  (1700, 202, '辰溪縣', 3, 0),
  (1701, 202, '漵浦縣', 3, 0),
  (1702, 202, '中方縣', 3, 0),
  (1703, 202, '會同縣', 3, 0),
  (1704, 202, '洪江市', 3, 0),
  (1705, 203, '婁星區', 3, 0),
  (1706, 203, '冷水江市', 3, 0),
  (1707, 203, '漣源市', 3, 0),
  (1708, 203, '雙峰縣', 3, 0),
  (1709, 203, '新化縣', 3, 0),
  (1710, 204, '城步', 3, 0),
  (1711, 204, '雙清區', 3, 0),
  (1712, 204, '大祥區', 3, 0),
  (1713, 204, '北塔區', 3, 0),
  (1714, 204, '武岡市', 3, 0),
  (1715, 204, '邵東縣', 3, 0),
  (1716, 204, '新邵縣', 3, 0),
  (1717, 204, '邵陽縣', 3, 0),
  (1718, 204, '隆回縣', 3, 0),
  (1719, 204, '洞口縣', 3, 0),
  (1720, 204, '綏寧縣', 3, 0),
  (1721, 204, '新寧縣', 3, 0),
  (1722, 205, '嶽塘區', 3, 0),
  (1723, 205, '雨湖區', 3, 0),
  (1724, 205, '湘鄉市', 3, 0),
  (1725, 205, '韶山市', 3, 0),
  (1726, 205, '湘潭縣', 3, 0),
  (1727, 206, '吉首市', 3, 0),
  (1728, 206, '瀘溪縣', 3, 0),
  (1729, 206, '鳳凰縣', 3, 0),
  (1730, 206, '花垣縣', 3, 0),
  (1731, 206, '保靖縣', 3, 0),
  (1732, 206, '古丈縣', 3, 0),
  (1733, 206, '永順縣', 3, 0),
  (1734, 206, '龍山縣', 3, 0),
  (1735, 207, '赫山區', 3, 0),
  (1736, 207, '資陽區', 3, 0),
  (1737, 207, '元江市', 3, 0),
  (1738, 207, '南縣', 3, 0),
  (1739, 207, '桃江縣', 3, 0),
  (1740, 207, '安化縣', 3, 0),
  (1741, 208, '江華', 3, 0),
  (1742, 208, '冷水灘區', 3, 0),
  (1743, 208, '零陵區', 3, 0),
  (1744, 208, '祁陽縣', 3, 0),
  (1745, 208, '東安縣', 3, 0),
  (1746, 208, '雙牌縣', 3, 0),
  (1747, 208, '道縣', 3, 0),
  (1748, 208, '江永縣', 3, 0),
  (1749, 208, '寧遠縣', 3, 0),
  (1750, 208, '藍山縣', 3, 0),
  (1751, 208, '新田縣', 3, 0),
  (1752, 209, '嶽陽樓區', 3, 0),
  (1753, 209, '君山區', 3, 0),
  (1754, 209, '雲溪區', 3, 0),
  (1755, 209, '淚羅市', 3, 0),
  (1756, 209, '臨湘市', 3, 0),
  (1757, 209, '嶽陽縣', 3, 0),
  (1758, 209, '華容縣', 3, 0),
  (1759, 209, '湘陰縣', 3, 0),
  (1760, 209, '平江縣', 3, 0),
  (1761, 210, '天元區', 3, 0),
  (1762, 210, '荷塘區', 3, 0),
  (1763, 210, '蘆淞區', 3, 0),
  (1764, 210, '石峰區', 3, 0),
  (1765, 210, '醴陵市', 3, 0),
  (1766, 210, '株洲縣', 3, 0),
  (1767, 210, '攸縣', 3, 0),
  (1768, 210, '茶陵縣', 3, 0),
  (1769, 210, '炎陵縣', 3, 0),
  (1770, 211, '朝陽區', 3, 0);

INSERT INTO `ecs_region`
VALUES (1771, 211, '寬城區', 3, 0),
  (1772, 211, '二道區', 3, 0),
  (1773, 211, '南關區', 3, 0),
  (1774, 211, '綠園區', 3, 0),
  (1775, 211, '雙陽區', 3, 0),
  (1776, 211, '凈月潭開發區', 3, 0),
  (1777, 211, '高新技術開發區', 3, 0),
  (1778, 211, '經濟技術開發區', 3, 0),
  (1779, 211, '汽車產業開發區', 3, 0),
  (1780, 211, '德惠市', 3, 0),
  (1781, 211, '九臺市', 3, 0),
  (1782, 211, '榆樹市', 3, 0),
  (1783, 211, '農安縣', 3, 0),
  (1784, 212, '船營區', 3, 0),
  (1785, 212, '昌邑區', 3, 0),
  (1786, 212, '龍潭區', 3, 0),
  (1787, 212, '豐滿區', 3, 0),
  (1788, 212, '蛟河市', 3, 0),
  (1789, 212, '樺甸市', 3, 0),
  (1790, 212, '舒蘭市', 3, 0),
  (1791, 212, '磐石市', 3, 0),
  (1792, 212, '永吉縣', 3, 0),
  (1793, 213, '桃北區', 3, 0),
  (1794, 213, '桃南市', 3, 0),
  (1795, 213, '大安市', 3, 0),
  (1796, 213, '鎮賚縣', 3, 0),
  (1797, 213, '通榆縣', 3, 0),
  (1798, 214, '江源區', 3, 0),
  (1799, 214, '八道江區', 3, 0),
  (1800, 214, '長白', 3, 0),
  (1801, 214, '臨江市', 3, 0),
  (1802, 214, '撫松縣', 3, 0),
  (1803, 214, '靖宇縣', 3, 0),
  (1804, 215, '龍山區', 3, 0),
  (1805, 215, '西安區', 3, 0),
  (1806, 215, '東豐縣', 3, 0),
  (1807, 215, '東遼縣', 3, 0),
  (1808, 216, '鐵西區', 3, 0),
  (1809, 216, '鐵東區', 3, 0),
  (1810, 216, '伊通', 3, 0),
  (1811, 216, '公主嶺市', 3, 0),
  (1812, 216, '雙遼市', 3, 0),
  (1813, 216, '梨樹縣', 3, 0),
  (1814, 217, '前郭爾羅斯', 3, 0),
  (1815, 217, '寧江區', 3, 0),
  (1816, 217, '長嶺縣', 3, 0),
  (1817, 217, '乾安縣', 3, 0),
  (1818, 217, '扶余縣', 3, 0),
  (1819, 218, '東昌區', 3, 0),
  (1820, 218, '二道江區', 3, 0),
  (1821, 218, '梅河口市', 3, 0),
  (1822, 218, '集安市', 3, 0),
  (1823, 218, '通化縣', 3, 0),
  (1824, 218, '輝南縣', 3, 0),
  (1825, 218, '柳河縣', 3, 0),
  (1826, 219, '延吉市', 3, 0),
  (1827, 219, '圖們市', 3, 0),
  (1828, 219, '敦化市', 3, 0),
  (1829, 219, '琿春市', 3, 0),
  (1830, 219, '龍井市', 3, 0),
  (1831, 219, '和龍市', 3, 0),
  (1832, 219, '安圖縣', 3, 0),
  (1833, 219, '汪清縣', 3, 0),
  (1834, 220, '玄武區', 3, 0),
  (1835, 220, '鼓樓區', 3, 0),
  (1836, 220, '白下區', 3, 0),
  (1837, 220, '建鄴區', 3, 0),
  (1838, 220, '秦淮區', 3, 0),
  (1839, 220, '雨花臺區', 3, 0),
  (1840, 220, '下關區', 3, 0),
  (1841, 220, '棲霞區', 3, 0),
  (1842, 220, '浦口區', 3, 0),
  (1843, 220, '江寧區', 3, 0),
  (1844, 220, '六合區', 3, 0),
  (1845, 220, '溧水縣', 3, 0),
  (1846, 220, '高淳縣', 3, 0),
  (1847, 221, '滄浪區', 3, 0),
  (1848, 221, '金閶區', 3, 0),
  (1849, 221, '平江區', 3, 0),
  (1850, 221, '虎丘區', 3, 0),
  (1851, 221, '吳中區', 3, 0),
  (1852, 221, '相城區', 3, 0),
  (1853, 221, '園區', 3, 0),
  (1854, 221, '新區', 3, 0),
  (1855, 221, '常熟市', 3, 0),
  (1856, 221, '張家港市', 3, 0),
  (1857, 221, '玉山鎮', 3, 0),
  (1858, 221, '巴城鎮', 3, 0),
  (1859, 221, '周市鎮', 3, 0),
  (1860, 221, '陸家鎮', 3, 0),
  (1861, 221, '花橋鎮', 3, 0),
  (1862, 221, '澱山湖鎮', 3, 0),
  (1863, 221, '張浦鎮', 3, 0),
  (1864, 221, '周莊鎮', 3, 0),
  (1865, 221, '千燈鎮', 3, 0),
  (1866, 221, '錦溪鎮', 3, 0),
  (1867, 221, '開發區', 3, 0),
  (1868, 221, '吳江市', 3, 0),
  (1869, 221, '太倉市', 3, 0),
  (1870, 222, '崇安區', 3, 0),
  (1871, 222, '北塘區', 3, 0),
  (1872, 222, '南長區', 3, 0),
  (1873, 222, '錫山區', 3, 0),
  (1874, 222, '惠山區', 3, 0),
  (1875, 222, '濱湖區', 3, 0),
  (1876, 222, '新區', 3, 0),
  (1877, 222, '江陰市', 3, 0),
  (1878, 222, '宜興市', 3, 0),
  (1879, 223, '天寧區', 3, 0),
  (1880, 223, '鐘樓區', 3, 0),
  (1881, 223, '戚墅堰區', 3, 0),
  (1882, 223, '郊區', 3, 0),
  (1883, 223, '新北區', 3, 0),
  (1884, 223, '武進區', 3, 0),
  (1885, 223, '溧陽市', 3, 0),
  (1886, 223, '金壇市', 3, 0),
  (1887, 224, '清河區', 3, 0),
  (1888, 224, '清浦區', 3, 0),
  (1889, 224, '楚州區', 3, 0),
  (1890, 224, '淮陰區', 3, 0),
  (1891, 224, '漣水縣', 3, 0),
  (1892, 224, '洪澤縣', 3, 0),
  (1893, 224, '盱眙縣', 3, 0),
  (1894, 224, '金湖縣', 3, 0),
  (1895, 225, '新浦區', 3, 0),
  (1896, 225, '連雲區', 3, 0),
  (1897, 225, '海州區', 3, 0),
  (1898, 225, '贛榆縣', 3, 0),
  (1899, 225, '東海縣', 3, 0),
  (1900, 225, '灌雲縣', 3, 0),
  (1901, 225, '灌南縣', 3, 0),
  (1902, 226, '崇川區', 3, 0),
  (1903, 226, '港閘區', 3, 0),
  (1904, 226, '經濟開發區', 3, 0),
  (1905, 226, '啟東市', 3, 0),
  (1906, 226, '如臯市', 3, 0),
  (1907, 226, '通州市', 3, 0),
  (1908, 226, '海門市', 3, 0),
  (1909, 226, '海安縣', 3, 0),
  (1910, 226, '如東縣', 3, 0),
  (1911, 227, '宿城區', 3, 0),
  (1912, 227, '宿豫區', 3, 0),
  (1913, 227, '宿豫縣', 3, 0),
  (1914, 227, '述陽縣', 3, 0),
  (1915, 227, '泗陽縣', 3, 0),
  (1916, 227, '泗洪縣', 3, 0),
  (1917, 228, '海陵區', 3, 0),
  (1918, 228, '高港區', 3, 0),
  (1919, 228, '興化市', 3, 0),
  (1920, 228, '靖江市', 3, 0),
  (1921, 228, '泰興市', 3, 0),
  (1922, 228, '姜堰市', 3, 0),
  (1923, 229, '雲龍區', 3, 0),
  (1924, 229, '鼓樓區', 3, 0),
  (1925, 229, '九裏區', 3, 0),
  (1926, 229, '賈汪區', 3, 0),
  (1927, 229, '泉山區', 3, 0),
  (1928, 229, '新沂市', 3, 0),
  (1929, 229, '邳州市', 3, 0),
  (1930, 229, '豐縣', 3, 0),
  (1931, 229, '沛縣', 3, 0),
  (1932, 229, '銅山縣', 3, 0),
  (1933, 229, '睢寧縣', 3, 0),
  (1934, 230, '城區', 3, 0),
  (1935, 230, '亭湖區', 3, 0),
  (1936, 230, '鹽都區', 3, 0),
  (1937, 230, '鹽都縣', 3, 0),
  (1938, 230, '東臺市', 3, 0),
  (1939, 230, '大豐市', 3, 0),
  (1940, 230, '響水縣', 3, 0),
  (1941, 230, '濱海縣', 3, 0),
  (1942, 230, '阜寧縣', 3, 0),
  (1943, 230, '射陽縣', 3, 0),
  (1944, 230, '建湖縣', 3, 0),
  (1945, 231, '廣陵區', 3, 0),
  (1946, 231, '維揚區', 3, 0),
  (1947, 231, '邗江區', 3, 0),
  (1948, 231, '儀征市', 3, 0),
  (1949, 231, '高郵市', 3, 0),
  (1950, 231, '江都市', 3, 0),
  (1951, 231, '寶應縣', 3, 0),
  (1952, 232, '京口區', 3, 0),
  (1953, 232, '潤州區', 3, 0),
  (1954, 232, '丹徒區', 3, 0),
  (1955, 232, '丹陽市', 3, 0),
  (1956, 232, '揚中市', 3, 0),
  (1957, 232, '句容市', 3, 0),
  (1958, 233, '東湖區', 3, 0),
  (1959, 233, '西湖區', 3, 0),
  (1960, 233, '青雲譜區', 3, 0),
  (1961, 233, '灣裏區', 3, 0),
  (1962, 233, '青山湖區', 3, 0),
  (1963, 233, '紅谷灘新區', 3, 0),
  (1964, 233, '昌北區', 3, 0),
  (1965, 233, '高新區', 3, 0),
  (1966, 233, '南昌縣', 3, 0),
  (1967, 233, '新建縣', 3, 0),
  (1968, 233, '安義縣', 3, 0),
  (1969, 233, '進賢縣', 3, 0),
  (1970, 234, '臨川區', 3, 0),
  (1971, 234, '南城縣', 3, 0),
  (1972, 234, '黎川縣', 3, 0),
  (1973, 234, '南豐縣', 3, 0),
  (1974, 234, '崇仁縣', 3, 0),
  (1975, 234, '樂安縣', 3, 0),
  (1976, 234, '宜黃縣', 3, 0),
  (1977, 234, '金溪縣', 3, 0),
  (1978, 234, '資溪縣', 3, 0),
  (1979, 234, '東鄉縣', 3, 0),
  (1980, 234, '廣昌縣', 3, 0),
  (1981, 235, '章貢區', 3, 0),
  (1982, 235, '於都縣', 3, 0),
  (1983, 235, '瑞金市', 3, 0),
  (1984, 235, '南康市', 3, 0),
  (1985, 235, '贛縣', 3, 0),
  (1986, 235, '信豐縣', 3, 0),
  (1987, 235, '大余縣', 3, 0),
  (1988, 235, '上猶縣', 3, 0),
  (1989, 235, '崇義縣', 3, 0),
  (1990, 235, '安遠縣', 3, 0),
  (1991, 235, '龍南縣', 3, 0),
  (1992, 235, '定南縣', 3, 0),
  (1993, 235, '全南縣', 3, 0),
  (1994, 235, '寧都縣', 3, 0),
  (1995, 235, '興國縣', 3, 0),
  (1996, 235, '會昌縣', 3, 0),
  (1997, 235, '尋烏縣', 3, 0),
  (1998, 235, '石城縣', 3, 0),
  (1999, 236, '安福縣', 3, 0),
  (2000, 236, '吉州區', 3, 0),
  (2001, 236, '青原區', 3, 0),
  (2002, 236, '井岡山市', 3, 0),
  (2003, 236, '吉安縣', 3, 0),
  (2004, 236, '吉水縣', 3, 0),
  (2005, 236, '峽江縣', 3, 0),
  (2006, 236, '新幹縣', 3, 0),
  (2007, 236, '永豐縣', 3, 0),
  (2008, 236, '泰和縣', 3, 0),
  (2009, 236, '遂川縣', 3, 0),
  (2010, 236, '萬安縣', 3, 0),
  (2011, 236, '永新縣', 3, 0),
  (2012, 237, '珠山區', 3, 0),
  (2013, 237, '昌江區', 3, 0),
  (2014, 237, '樂平市', 3, 0),
  (2015, 237, '浮梁縣', 3, 0),
  (2016, 238, '潯陽區', 3, 0),
  (2017, 238, '廬山區', 3, 0),
  (2018, 238, '瑞昌市', 3, 0),
  (2019, 238, '九江縣', 3, 0),
  (2020, 238, '武寧縣', 3, 0),
  (2021, 238, '修水縣', 3, 0),
  (2022, 238, '永修縣', 3, 0),
  (2023, 238, '德安縣', 3, 0),
  (2024, 238, '星子縣', 3, 0),
  (2025, 238, '都昌縣', 3, 0),
  (2026, 238, '湖口縣', 3, 0),
  (2027, 238, '彭澤縣', 3, 0),
  (2028, 239, '安源區', 3, 0),
  (2029, 239, '湘東區', 3, 0),
  (2030, 239, '蓮花縣', 3, 0),
  (2031, 239, '蘆溪縣', 3, 0),
  (2032, 239, '上栗縣', 3, 0),
  (2033, 240, '信州區', 3, 0),
  (2034, 240, '德興市', 3, 0),
  (2035, 240, '上饒縣', 3, 0),
  (2036, 240, '廣豐縣', 3, 0),
  (2037, 240, '玉山縣', 3, 0),
  (2038, 240, '鉛山縣', 3, 0),
  (2039, 240, '橫峰縣', 3, 0),
  (2040, 240, '弋陽縣', 3, 0),
  (2041, 240, '余幹縣', 3, 0),
  (2042, 240, '波陽縣', 3, 0),
  (2043, 240, '萬年縣', 3, 0),
  (2044, 240, '婺源縣', 3, 0),
  (2045, 241, '渝水區', 3, 0),
  (2046, 241, '分宜縣', 3, 0),
  (2047, 242, '袁州區', 3, 0),
  (2048, 242, '豐城市', 3, 0),
  (2049, 242, '樟樹市', 3, 0),
  (2050, 242, '高安市', 3, 0),
  (2051, 242, '奉新縣', 3, 0),
  (2052, 242, '萬載縣', 3, 0),
  (2053, 242, '上高縣', 3, 0),
  (2054, 242, '宜豐縣', 3, 0),
  (2055, 242, '靖安縣', 3, 0),
  (2056, 242, '銅鼓縣', 3, 0),
  (2057, 243, '月湖區', 3, 0),
  (2058, 243, '貴溪市', 3, 0),
  (2059, 243, '余江縣', 3, 0),
  (2060, 244, '沈河區', 3, 0),
  (2061, 244, '皇姑區', 3, 0),
  (2062, 244, '和平區', 3, 0),
  (2063, 244, '大東區', 3, 0),
  (2064, 244, '鐵西區', 3, 0),
  (2065, 244, '蘇家屯區', 3, 0),
  (2066, 244, '東陵區', 3, 0),
  (2067, 244, '沈北新區', 3, 0),
  (2068, 244, '於洪區', 3, 0),
  (2069, 244, '渾南新區', 3, 0),
  (2070, 244, '新民市', 3, 0),
  (2071, 244, '遼中縣', 3, 0),
  (2072, 244, '康平縣', 3, 0),
  (2073, 244, '法庫縣', 3, 0),
  (2074, 245, '西崗區', 3, 0),
  (2075, 245, '中山區', 3, 0),
  (2076, 245, '沙河口區', 3, 0),
  (2077, 245, '甘井子區', 3, 0),
  (2078, 245, '旅順口區', 3, 0),
  (2079, 245, '金州區', 3, 0),
  (2080, 245, '開發區', 3, 0),
  (2081, 245, '瓦房店市', 3, 0),
  (2082, 245, '普蘭店市', 3, 0),
  (2083, 245, '莊河市', 3, 0),
  (2084, 245, '長海縣', 3, 0),
  (2085, 246, '鐵東區', 3, 0),
  (2086, 246, '鐵西區', 3, 0),
  (2087, 246, '立山區', 3, 0),
  (2088, 246, '千山區', 3, 0),
  (2089, 246, '岫巖', 3, 0),
  (2090, 246, '海城市', 3, 0),
  (2091, 246, '臺安縣', 3, 0),
  (2092, 247, '本溪', 3, 0),
  (2093, 247, '平山區', 3, 0),
  (2094, 247, '明山區', 3, 0),
  (2095, 247, '溪湖區', 3, 0),
  (2096, 247, '南芬區', 3, 0),
  (2097, 247, '桓仁', 3, 0),
  (2098, 248, '雙塔區', 3, 0),
  (2099, 248, '龍城區', 3, 0),
  (2100, 248, '喀喇沁左翼蒙古族自治縣', 3, 0),
  (2101, 248, '北票市', 3, 0),
  (2102, 248, '淩源市', 3, 0),
  (2103, 248, '朝陽縣', 3, 0),
  (2104, 248, '建平縣', 3, 0),
  (2105, 249, '振興區', 3, 0),
  (2106, 249, '元寶區', 3, 0),
  (2107, 249, '振安區', 3, 0),
  (2108, 249, '寬甸', 3, 0),
  (2109, 249, '東港市', 3, 0),
  (2110, 249, '鳳城市', 3, 0),
  (2111, 250, '順城區', 3, 0),
  (2112, 250, '新撫區', 3, 0),
  (2113, 250, '東洲區', 3, 0),
  (2114, 250, '望花區', 3, 0),
  (2115, 250, '清原', 3, 0),
  (2116, 250, '新賓', 3, 0),
  (2117, 250, '撫順縣', 3, 0),
  (2118, 251, '阜新', 3, 0),
  (2119, 251, '海州區', 3, 0),
  (2120, 251, '新邱區', 3, 0),
  (2121, 251, '太平區', 3, 0),
  (2122, 251, '清河門區', 3, 0),
  (2123, 251, '細河區', 3, 0),
  (2124, 251, '彰武縣', 3, 0),
  (2125, 252, '龍港區', 3, 0),
  (2126, 252, '南票區', 3, 0),
  (2127, 252, '連山區', 3, 0),
  (2128, 252, '興城市', 3, 0),
  (2129, 252, '綏中縣', 3, 0),
  (2130, 252, '建昌縣', 3, 0),
  (2131, 253, '太和區', 3, 0),
  (2132, 253, '古塔區', 3, 0),
  (2133, 253, '淩河區', 3, 0),
  (2134, 253, '淩海市', 3, 0),
  (2135, 253, '北鎮市', 3, 0),
  (2136, 253, '黑山縣', 3, 0),
  (2137, 253, '義縣', 3, 0),
  (2138, 254, '白塔區', 3, 0),
  (2139, 254, '文聖區', 3, 0),
  (2140, 254, '宏偉區', 3, 0),
  (2141, 254, '太子河區', 3, 0),
  (2142, 254, '弓長嶺區', 3, 0),
  (2143, 254, '燈塔市', 3, 0),
  (2144, 254, '遼陽縣', 3, 0),
  (2145, 255, '雙臺子區', 3, 0),
  (2146, 255, '興隆臺區', 3, 0),
  (2147, 255, '大窪縣', 3, 0),
  (2148, 255, '盤山縣', 3, 0),
  (2149, 256, '銀州區', 3, 0),
  (2150, 256, '清河區', 3, 0),
  (2151, 256, '調兵山市', 3, 0),
  (2152, 256, '開原市', 3, 0),
  (2153, 256, '鐵嶺縣', 3, 0),
  (2154, 256, '西豐縣', 3, 0),
  (2155, 256, '昌圖縣', 3, 0),
  (2156, 257, '站前區', 3, 0),
  (2157, 257, '西市區', 3, 0),
  (2158, 257, '鮁魚圈區', 3, 0),
  (2159, 257, '老邊區', 3, 0),
  (2160, 257, '蓋州市', 3, 0),
  (2161, 257, '大石橋市', 3, 0),
  (2162, 258, '回民區', 3, 0),
  (2163, 258, '玉泉區', 3, 0),
  (2164, 258, '新城區', 3, 0),
  (2165, 258, '賽罕區', 3, 0),
  (2166, 258, '清水河縣', 3, 0),
  (2167, 258, '土默特左旗', 3, 0),
  (2168, 258, '托克托縣', 3, 0),
  (2169, 258, '和林格爾縣', 3, 0),
  (2170, 258, '武川縣', 3, 0),
  (2171, 259, '阿拉善左旗', 3, 0),
  (2172, 259, '阿拉善右旗', 3, 0),
  (2173, 259, '額濟納旗', 3, 0),
  (2174, 260, '臨河區', 3, 0),
  (2175, 260, '五原縣', 3, 0),
  (2176, 260, '磴口縣', 3, 0),
  (2177, 260, '烏拉特前旗', 3, 0),
  (2178, 260, '烏拉特中旗', 3, 0),
  (2179, 260, '烏拉特後旗', 3, 0),
  (2180, 260, '杭錦後旗', 3, 0),
  (2181, 261, '昆都侖區', 3, 0),
  (2182, 261, '青山區', 3, 0),
  (2183, 261, '東河區', 3, 0),
  (2184, 261, '九原區', 3, 0),
  (2185, 261, '石拐區', 3, 0),
  (2186, 261, '白雲礦區', 3, 0),
  (2187, 261, '土默特右旗', 3, 0),
  (2188, 261, '固陽縣', 3, 0),
  (2189, 261, '達爾罕茂明安聯合旗', 3, 0),
  (2190, 262, '紅山區', 3, 0),
  (2191, 262, '元寶山區', 3, 0),
  (2192, 262, '松山區', 3, 0),
  (2193, 262, '阿魯科爾沁旗', 3, 0),
  (2194, 262, '巴林左旗', 3, 0),
  (2195, 262, '巴林右旗', 3, 0),
  (2196, 262, '林西縣', 3, 0),
  (2197, 262, '克什克騰旗', 3, 0),
  (2198, 262, '翁牛特旗', 3, 0),
  (2199, 262, '喀喇沁旗', 3, 0),
  (2200, 262, '寧城縣', 3, 0),
  (2201, 262, '敖漢旗', 3, 0),
  (2202, 263, '東勝區', 3, 0),
  (2203, 263, '達拉特旗', 3, 0),
  (2204, 263, '準格爾旗', 3, 0),
  (2205, 263, '鄂托克前旗', 3, 0),
  (2206, 263, '鄂托克旗', 3, 0),
  (2207, 263, '杭錦旗', 3, 0),
  (2208, 263, '烏審旗', 3, 0),
  (2209, 263, '伊金霍洛旗', 3, 0),
  (2210, 264, '海拉爾區', 3, 0),
  (2211, 264, '莫力達瓦', 3, 0),
  (2212, 264, '滿洲裏市', 3, 0),
  (2213, 264, '牙克石市', 3, 0),
  (2214, 264, '紮蘭屯市', 3, 0),
  (2215, 264, '額爾古納市', 3, 0),
  (2216, 264, '根河市', 3, 0),
  (2217, 264, '阿榮旗', 3, 0),
  (2218, 264, '鄂倫春自治旗', 3, 0),
  (2219, 264, '鄂溫克族自治旗', 3, 0),
  (2220, 264, '陳巴爾虎旗', 3, 0),
  (2221, 264, '新巴爾虎左旗', 3, 0),
  (2222, 264, '新巴爾虎右旗', 3, 0),
  (2223, 265, '科爾沁區', 3, 0),
  (2224, 265, '霍林郭勒市', 3, 0),
  (2225, 265, '科爾沁左翼中旗', 3, 0),
  (2226, 265, '科爾沁左翼後旗', 3, 0),
  (2227, 265, '開魯縣', 3, 0),
  (2228, 265, '庫倫旗', 3, 0),
  (2229, 265, '奈曼旗', 3, 0),
  (2230, 265, '紮魯特旗', 3, 0),
  (2231, 266, '海勃灣區', 3, 0),
  (2232, 266, '烏達區', 3, 0),
  (2233, 266, '海南區', 3, 0),
  (2234, 267, '化德縣', 3, 0),
  (2235, 267, '集寧區', 3, 0),
  (2236, 267, '豐鎮市', 3, 0),
  (2237, 267, '卓資縣', 3, 0),
  (2238, 267, '商都縣', 3, 0),
  (2239, 267, '興和縣', 3, 0),
  (2240, 267, '涼城縣', 3, 0),
  (2241, 267, '察哈爾右翼前旗', 3, 0),
  (2242, 267, '察哈爾右翼中旗', 3, 0),
  (2243, 267, '察哈爾右翼後旗', 3, 0),
  (2244, 267, '四子王旗', 3, 0),
  (2245, 268, '二連浩特市', 3, 0),
  (2246, 268, '錫林浩特市', 3, 0),
  (2247, 268, '阿巴嘎旗', 3, 0),
  (2248, 268, '蘇尼特左旗', 3, 0),
  (2249, 268, '蘇尼特右旗', 3, 0),
  (2250, 268, '東烏珠穆沁旗', 3, 0),
  (2251, 268, '西烏珠穆沁旗', 3, 0),
  (2252, 268, '太仆寺旗', 3, 0),
  (2253, 268, '鑲黃旗', 3, 0),
  (2254, 268, '正鑲白旗', 3, 0),
  (2255, 268, '正藍旗', 3, 0),
  (2256, 268, '多倫縣', 3, 0),
  (2257, 269, '烏蘭浩特市', 3, 0),
  (2258, 269, '阿爾山市', 3, 0),
  (2259, 269, '科爾沁右翼前旗', 3, 0),
  (2260, 269, '科爾沁右翼中旗', 3, 0),
  (2261, 269, '紮賚特旗', 3, 0),
  (2262, 269, '突泉縣', 3, 0),
  (2263, 270, '西夏區', 3, 0),
  (2264, 270, '金鳳區', 3, 0),
  (2265, 270, '興慶區', 3, 0),
  (2266, 270, '靈武市', 3, 0),
  (2267, 270, '永寧縣', 3, 0),
  (2268, 270, '賀蘭縣', 3, 0),
  (2269, 271, '原州區', 3, 0),
  (2270, 271, '海原縣', 3, 0),
  (2271, 271, '西吉縣', 3, 0),
  (2272, 271, '隆德縣', 3, 0),
  (2273, 271, '涇源縣', 3, 0),
  (2274, 271, '彭陽縣', 3, 0),
  (2275, 272, '惠農縣', 3, 0),
  (2276, 272, '大武口區', 3, 0),
  (2277, 272, '惠農區', 3, 0),
  (2278, 272, '陶樂縣', 3, 0),
  (2279, 272, '平羅縣', 3, 0),
  (2280, 273, '利通區', 3, 0),
  (2281, 273, '中衛縣', 3, 0),
  (2282, 273, '青銅峽市', 3, 0),
  (2283, 273, '中寧縣', 3, 0),
  (2284, 273, '鹽池縣', 3, 0),
  (2285, 273, '同心縣', 3, 0),
  (2286, 274, '沙坡頭區', 3, 0),
  (2287, 274, '海原縣', 3, 0),
  (2288, 274, '中寧縣', 3, 0),
  (2289, 275, '城中區', 3, 0),
  (2290, 275, '城東區', 3, 0),
  (2291, 275, '城西區', 3, 0),
  (2292, 275, '城北區', 3, 0),
  (2293, 275, '湟中縣', 3, 0),
  (2294, 275, '湟源縣', 3, 0),
  (2295, 275, '大通', 3, 0),
  (2296, 276, '瑪沁縣', 3, 0),
  (2297, 276, '班瑪縣', 3, 0),
  (2298, 276, '甘德縣', 3, 0),
  (2299, 276, '達日縣', 3, 0),
  (2300, 276, '久治縣', 3, 0),
  (2301, 276, '瑪多縣', 3, 0),
  (2302, 277, '海晏縣', 3, 0),
  (2303, 277, '祁連縣', 3, 0),
  (2304, 277, '剛察縣', 3, 0),
  (2305, 277, '門源', 3, 0),
  (2306, 278, '平安縣', 3, 0),
  (2307, 278, '樂都縣', 3, 0),
  (2308, 278, '民和', 3, 0),
  (2309, 278, '互助', 3, 0),
  (2310, 278, '化隆', 3, 0),
  (2311, 278, '循化', 3, 0),
  (2312, 279, '共和縣', 3, 0),
  (2313, 279, '同德縣', 3, 0),
  (2314, 279, '貴德縣', 3, 0),
  (2315, 279, '興海縣', 3, 0),
  (2316, 279, '貴南縣', 3, 0),
  (2317, 280, '德令哈市', 3, 0),
  (2318, 280, '格爾木市', 3, 0),
  (2319, 280, '烏蘭縣', 3, 0),
  (2320, 280, '都蘭縣', 3, 0),
  (2321, 280, '天峻縣', 3, 0),
  (2322, 281, '同仁縣', 3, 0),
  (2323, 281, '尖紮縣', 3, 0),
  (2324, 281, '澤庫縣', 3, 0),
  (2325, 281, '河南蒙古族自治縣', 3, 0),
  (2326, 282, '玉樹縣', 3, 0),
  (2327, 282, '雜多縣', 3, 0),
  (2328, 282, '稱多縣', 3, 0),
  (2329, 282, '治多縣', 3, 0),
  (2330, 282, '囊謙縣', 3, 0),
  (2331, 282, '曲麻萊縣', 3, 0),
  (2332, 283, '市中區', 3, 0),
  (2333, 283, '歷下區', 3, 0),
  (2334, 283, '天橋區', 3, 0),
  (2335, 283, '槐蔭區', 3, 0),
  (2336, 283, '歷城區', 3, 0),
  (2337, 283, '長清區', 3, 0),
  (2338, 283, '章丘市', 3, 0),
  (2339, 283, '平陰縣', 3, 0),
  (2340, 283, '濟陽縣', 3, 0),
  (2341, 283, '商河縣', 3, 0),
  (2342, 284, '市南區', 3, 0),
  (2343, 284, '市北區', 3, 0),
  (2344, 284, '城陽區', 3, 0),
  (2345, 284, '四方區', 3, 0),
  (2346, 284, '李滄區', 3, 0),
  (2347, 284, '黃島區', 3, 0),
  (2348, 284, '嶗山區', 3, 0),
  (2349, 284, '膠州市', 3, 0),
  (2350, 284, '即墨市', 3, 0),
  (2351, 284, '平度市', 3, 0),
  (2352, 284, '膠南市', 3, 0),
  (2353, 284, '萊西市', 3, 0),
  (2354, 285, '濱城區', 3, 0),
  (2355, 285, '惠民縣', 3, 0),
  (2356, 285, '陽信縣', 3, 0),
  (2357, 285, '無棣縣', 3, 0),
  (2358, 285, '沾化縣', 3, 0),
  (2359, 285, '博興縣', 3, 0),
  (2360, 285, '鄒平縣', 3, 0),
  (2361, 286, '德城區', 3, 0),
  (2362, 286, '陵縣', 3, 0),
  (2363, 286, '樂陵市', 3, 0),
  (2364, 286, '禹城市', 3, 0),
  (2365, 286, '寧津縣', 3, 0),
  (2366, 286, '慶雲縣', 3, 0),
  (2367, 286, '臨邑縣', 3, 0),
  (2368, 286, '齊河縣', 3, 0),
  (2369, 286, '平原縣', 3, 0),
  (2370, 286, '夏津縣', 3, 0),
  (2371, 286, '武城縣', 3, 0),
  (2372, 287, '東營區', 3, 0),
  (2373, 287, '河口區', 3, 0),
  (2374, 287, '墾利縣', 3, 0),
  (2375, 287, '利津縣', 3, 0),
  (2376, 287, '廣饒縣', 3, 0),
  (2377, 288, '牡丹區', 3, 0),
  (2378, 288, '曹縣', 3, 0),
  (2379, 288, '單縣', 3, 0),
  (2380, 288, '成武縣', 3, 0),
  (2381, 288, '巨野縣', 3, 0),
  (2382, 288, '鄆城縣', 3, 0),
  (2383, 288, '鄄城縣', 3, 0),
  (2384, 288, '定陶縣', 3, 0),
  (2385, 288, '東明縣', 3, 0),
  (2386, 289, '市中區', 3, 0),
  (2387, 289, '任城區', 3, 0),
  (2388, 289, '曲阜市', 3, 0),
  (2389, 289, '兗州市', 3, 0),
  (2390, 289, '鄒城市', 3, 0),
  (2391, 289, '微山縣', 3, 0),
  (2392, 289, '魚臺縣', 3, 0),
  (2393, 289, '金鄉縣', 3, 0),
  (2394, 289, '嘉祥縣', 3, 0),
  (2395, 289, '汶上縣', 3, 0),
  (2396, 289, '泗水縣', 3, 0),
  (2397, 289, '梁山縣', 3, 0),
  (2398, 290, '萊城區', 3, 0),
  (2399, 290, '鋼城區', 3, 0),
  (2400, 291, '東昌府區', 3, 0),
  (2401, 291, '臨清市', 3, 0),
  (2402, 291, '陽谷縣', 3, 0),
  (2403, 291, '莘縣', 3, 0),
  (2404, 291, '茌平縣', 3, 0),
  (2405, 291, '東阿縣', 3, 0),
  (2406, 291, '冠縣', 3, 0),
  (2407, 291, '高唐縣', 3, 0),
  (2408, 292, '蘭山區', 3, 0),
  (2409, 292, '羅莊區', 3, 0),
  (2410, 292, '河東區', 3, 0),
  (2411, 292, '沂南縣', 3, 0),
  (2412, 292, '郯城縣', 3, 0),
  (2413, 292, '沂水縣', 3, 0),
  (2414, 292, '蒼山縣', 3, 0),
  (2415, 292, '費縣', 3, 0),
  (2416, 292, '平邑縣', 3, 0),
  (2417, 292, '莒南縣', 3, 0),
  (2418, 292, '蒙陰縣', 3, 0),
  (2419, 292, '臨述縣', 3, 0),
  (2420, 293, '東港區', 3, 0),
  (2421, 293, '嵐山區', 3, 0),
  (2422, 293, '五蓮縣', 3, 0),
  (2423, 293, '莒縣', 3, 0),
  (2424, 294, '泰山區', 3, 0),
  (2425, 294, '岱嶽區', 3, 0),
  (2426, 294, '新泰市', 3, 0),
  (2427, 294, '肥城市', 3, 0),
  (2428, 294, '寧陽縣', 3, 0),
  (2429, 294, '東平縣', 3, 0),
  (2430, 295, '榮成市', 3, 0),
  (2431, 295, '乳山市', 3, 0),
  (2432, 295, '環翠區', 3, 0),
  (2433, 295, '文登市', 3, 0),
  (2434, 296, '濰城區', 3, 0),
  (2435, 296, '寒亭區', 3, 0),
  (2436, 296, '坊子區', 3, 0),
  (2437, 296, '奎文區', 3, 0),
  (2438, 296, '青州市', 3, 0),
  (2439, 296, '諸城市', 3, 0),
  (2440, 296, '壽光市', 3, 0),
  (2441, 296, '安丘市', 3, 0),
  (2442, 296, '高密市', 3, 0),
  (2443, 296, '昌邑市', 3, 0),
  (2444, 296, '臨朐縣', 3, 0),
  (2445, 296, '昌樂縣', 3, 0),
  (2446, 297, '芝罘區', 3, 0),
  (2447, 297, '福山區', 3, 0),
  (2448, 297, '牟平區', 3, 0),
  (2449, 297, '萊山區', 3, 0),
  (2450, 297, '開發區', 3, 0),
  (2451, 297, '龍口市', 3, 0),
  (2452, 297, '萊陽市', 3, 0),
  (2453, 297, '萊州市', 3, 0),
  (2454, 297, '蓬萊市', 3, 0),
  (2455, 297, '招遠市', 3, 0),
  (2456, 297, '棲霞市', 3, 0),
  (2457, 297, '海陽市', 3, 0),
  (2458, 297, '長島縣', 3, 0),
  (2459, 298, '市中區', 3, 0),
  (2460, 298, '山亭區', 3, 0),
  (2461, 298, '嶧城區', 3, 0),
  (2462, 298, '臺兒莊區', 3, 0),
  (2463, 298, '薛城區', 3, 0),
  (2464, 298, '滕州市', 3, 0),
  (2465, 299, '張店區', 3, 0),
  (2466, 299, '臨淄區', 3, 0),
  (2467, 299, '淄川區', 3, 0),
  (2468, 299, '博山區', 3, 0),
  (2469, 299, '周村區', 3, 0),
  (2470, 299, '桓臺縣', 3, 0),
  (2471, 299, '高青縣', 3, 0),
  (2472, 299, '沂源縣', 3, 0),
  (2473, 300, '杏花嶺區', 3, 0),
  (2474, 300, '小店區', 3, 0),
  (2475, 300, '迎澤區', 3, 0),
  (2476, 300, '尖草坪區', 3, 0),
  (2477, 300, '萬柏林區', 3, 0),
  (2478, 300, '晉源區', 3, 0),
  (2479, 300, '高新開發區', 3, 0),
  (2480, 300, '民營經濟開發區', 3, 0),
  (2481, 300, '經濟技術開發區', 3, 0),
  (2482, 300, '清徐縣', 3, 0),
  (2483, 300, '陽曲縣', 3, 0),
  (2484, 300, '婁煩縣', 3, 0),
  (2485, 300, '古交市', 3, 0),
  (2486, 301, '城區', 3, 0),
  (2487, 301, '郊區', 3, 0),
  (2488, 301, '沁縣', 3, 0),
  (2489, 301, '潞城市', 3, 0),
  (2490, 301, '長治縣', 3, 0),
  (2491, 301, '襄垣縣', 3, 0),
  (2492, 301, '屯留縣', 3, 0),
  (2493, 301, '平順縣', 3, 0),
  (2494, 301, '黎城縣', 3, 0),
  (2495, 301, '壺關縣', 3, 0),
  (2496, 301, '長子縣', 3, 0),
  (2497, 301, '武鄉縣', 3, 0),
  (2498, 301, '沁源縣', 3, 0),
  (2499, 302, '城區', 3, 0),
  (2500, 302, '礦區', 3, 0),
  (2501, 302, '南郊區', 3, 0),
  (2502, 302, '新榮區', 3, 0),
  (2503, 302, '陽高縣', 3, 0),
  (2504, 302, '天鎮縣', 3, 0),
  (2505, 302, '廣靈縣', 3, 0),
  (2506, 302, '靈丘縣', 3, 0),
  (2507, 302, '渾源縣', 3, 0),
  (2508, 302, '左雲縣', 3, 0),
  (2509, 302, '大同縣', 3, 0),
  (2510, 303, '城區', 3, 0),
  (2511, 303, '高平市', 3, 0),
  (2512, 303, '沁水縣', 3, 0),
  (2513, 303, '陽城縣', 3, 0),
  (2514, 303, '陵川縣', 3, 0),
  (2515, 303, '澤州縣', 3, 0),
  (2516, 304, '榆次區', 3, 0),
  (2517, 304, '介休市', 3, 0),
  (2518, 304, '榆社縣', 3, 0),
  (2519, 304, '左權縣', 3, 0),
  (2520, 304, '和順縣', 3, 0),
  (2521, 304, '昔陽縣', 3, 0),
  (2522, 304, '壽陽縣', 3, 0),
  (2523, 304, '太谷縣', 3, 0),
  (2524, 304, '祁縣', 3, 0),
  (2525, 304, '平遙縣', 3, 0),
  (2526, 304, '靈石縣', 3, 0),
  (2527, 305, '堯都區', 3, 0),
  (2528, 305, '侯馬市', 3, 0),
  (2529, 305, '霍州市', 3, 0),
  (2530, 305, '曲沃縣', 3, 0),
  (2531, 305, '翼城縣', 3, 0),
  (2532, 305, '襄汾縣', 3, 0),
  (2533, 305, '洪洞縣', 3, 0),
  (2534, 305, '吉縣', 3, 0),
  (2535, 305, '安澤縣', 3, 0),
  (2536, 305, '浮山縣', 3, 0),
  (2537, 305, '古縣', 3, 0),
  (2538, 305, '鄉寧縣', 3, 0),
  (2539, 305, '大寧縣', 3, 0),
  (2540, 305, '隰縣', 3, 0),
  (2541, 305, '永和縣', 3, 0),
  (2542, 305, '蒲縣', 3, 0),
  (2543, 305, '汾西縣', 3, 0),
  (2544, 306, '離石市', 3, 0),
  (2545, 306, '離石區', 3, 0),
  (2546, 306, '孝義市', 3, 0),
  (2547, 306, '汾陽市', 3, 0),
  (2548, 306, '文水縣', 3, 0),
  (2549, 306, '交城縣', 3, 0),
  (2550, 306, '興縣', 3, 0),
  (2551, 306, '臨縣', 3, 0),
  (2552, 306, '柳林縣', 3, 0),
  (2553, 306, '石樓縣', 3, 0),
  (2554, 306, '嵐縣', 3, 0),
  (2555, 306, '方山縣', 3, 0),
  (2556, 306, '中陽縣', 3, 0),
  (2557, 306, '交口縣', 3, 0),
  (2558, 307, '朔城區', 3, 0),
  (2559, 307, '平魯區', 3, 0),
  (2560, 307, '山陰縣', 3, 0),
  (2561, 307, '應縣', 3, 0),
  (2562, 307, '右玉縣', 3, 0),
  (2563, 307, '懷仁縣', 3, 0),
  (2564, 308, '忻府區', 3, 0),
  (2565, 308, '原平市', 3, 0),
  (2566, 308, '定襄縣', 3, 0),
  (2567, 308, '五臺縣', 3, 0),
  (2568, 308, '代縣', 3, 0),
  (2569, 308, '繁峙縣', 3, 0),
  (2570, 308, '寧武縣', 3, 0),
  (2571, 308, '靜樂縣', 3, 0),
  (2572, 308, '神池縣', 3, 0),
  (2573, 308, '五寨縣', 3, 0),
  (2574, 308, '苛嵐縣', 3, 0),
  (2575, 308, '河曲縣', 3, 0),
  (2576, 308, '保德縣', 3, 0),
  (2577, 308, '偏關縣', 3, 0),
  (2578, 309, '城區', 3, 0),
  (2579, 309, '礦區', 3, 0),
  (2580, 309, '郊區', 3, 0),
  (2581, 309, '平定縣', 3, 0),
  (2582, 309, '盂縣', 3, 0),
  (2583, 310, '鹽湖區', 3, 0),
  (2584, 310, '永濟市', 3, 0),
  (2585, 310, '河津市', 3, 0),
  (2586, 310, '臨猗縣', 3, 0),
  (2587, 310, '萬榮縣', 3, 0),
  (2588, 310, '聞喜縣', 3, 0),
  (2589, 310, '稷山縣', 3, 0),
  (2590, 310, '新絳縣', 3, 0),
  (2591, 310, '絳縣', 3, 0),
  (2592, 310, '垣曲縣', 3, 0),
  (2593, 310, '夏縣', 3, 0),
  (2594, 310, '平陸縣', 3, 0),
  (2595, 310, '芮城縣', 3, 0),
  (2596, 311, '蓮湖區', 3, 0),
  (2597, 311, '新城區', 3, 0),
  (2598, 311, '碑林區', 3, 0),
  (2599, 311, '雁塔區', 3, 0),
  (2600, 311, '灞橋區', 3, 0),
  (2601, 311, '未央區', 3, 0),
  (2602, 311, '閻良區', 3, 0),
  (2603, 311, '臨潼區', 3, 0),
  (2604, 311, '長安區', 3, 0),
  (2605, 311, '藍田縣', 3, 0),
  (2606, 311, '周至縣', 3, 0),
  (2607, 311, '戶縣', 3, 0),
  (2608, 311, '高陵縣', 3, 0),
  (2609, 312, '漢濱區', 3, 0),
  (2610, 312, '漢陰縣', 3, 0),
  (2611, 312, '石泉縣', 3, 0),
  (2612, 312, '寧陜縣', 3, 0),
  (2613, 312, '紫陽縣', 3, 0),
  (2614, 312, '嵐臯縣', 3, 0),
  (2615, 312, '平利縣', 3, 0),
  (2616, 312, '鎮坪縣', 3, 0),
  (2617, 312, '旬陽縣', 3, 0),
  (2618, 312, '白河縣', 3, 0),
  (2619, 313, '陳倉區', 3, 0),
  (2620, 313, '渭濱區', 3, 0),
  (2621, 313, '金臺區', 3, 0),
  (2622, 313, '鳳翔縣', 3, 0),
  (2623, 313, '岐山縣', 3, 0),
  (2624, 313, '扶風縣', 3, 0),
  (2625, 313, '眉縣', 3, 0),
  (2626, 313, '隴縣', 3, 0),
  (2627, 313, '千陽縣', 3, 0),
  (2628, 313, '麟遊縣', 3, 0),
  (2629, 313, '鳳縣', 3, 0),
  (2630, 313, '太白縣', 3, 0),
  (2631, 314, '漢臺區', 3, 0),
  (2632, 314, '南鄭縣', 3, 0),
  (2633, 314, '城固縣', 3, 0),
  (2634, 314, '洋縣', 3, 0),
  (2635, 314, '西鄉縣', 3, 0),
  (2636, 314, '勉縣', 3, 0),
  (2637, 314, '寧強縣', 3, 0),
  (2638, 314, '略陽縣', 3, 0),
  (2639, 314, '鎮巴縣', 3, 0),
  (2640, 314, '留壩縣', 3, 0),
  (2641, 314, '佛坪縣', 3, 0),
  (2642, 315, '商州區', 3, 0),
  (2643, 315, '洛南縣', 3, 0),
  (2644, 315, '丹鳳縣', 3, 0),
  (2645, 315, '商南縣', 3, 0),
  (2646, 315, '山陽縣', 3, 0),
  (2647, 315, '鎮安縣', 3, 0),
  (2648, 315, '柞水縣', 3, 0),
  (2649, 316, '耀州區', 3, 0),
  (2650, 316, '王益區', 3, 0),
  (2651, 316, '印臺區', 3, 0),
  (2652, 316, '宜君縣', 3, 0),
  (2653, 317, '臨渭區', 3, 0),
  (2654, 317, '韓城市', 3, 0),
  (2655, 317, '華陰市', 3, 0),
  (2656, 317, '華縣', 3, 0),
  (2657, 317, '潼關縣', 3, 0),
  (2658, 317, '大荔縣', 3, 0),
  (2659, 317, '合陽縣', 3, 0),
  (2660, 317, '澄城縣', 3, 0),
  (2661, 317, '蒲城縣', 3, 0),
  (2662, 317, '白水縣', 3, 0),
  (2663, 317, '富平縣', 3, 0),
  (2664, 318, '秦都區', 3, 0),
  (2665, 318, '渭城區', 3, 0),
  (2666, 318, '楊陵區', 3, 0),
  (2667, 318, '興平市', 3, 0),
  (2668, 318, '三原縣', 3, 0),
  (2669, 318, '涇陽縣', 3, 0),
  (2670, 318, '乾縣', 3, 0),
  (2671, 318, '禮泉縣', 3, 0),
  (2672, 318, '永壽縣', 3, 0),
  (2673, 318, '彬縣', 3, 0),
  (2674, 318, '長武縣', 3, 0),
  (2675, 318, '旬邑縣', 3, 0),
  (2676, 318, '淳化縣', 3, 0),
  (2677, 318, '武功縣', 3, 0),
  (2678, 319, '吳起縣', 3, 0),
  (2679, 319, '寶塔區', 3, 0),
  (2680, 319, '延長縣', 3, 0),
  (2681, 319, '延川縣', 3, 0),
  (2682, 319, '子長縣', 3, 0),
  (2683, 319, '安塞縣', 3, 0),
  (2684, 319, '誌丹縣', 3, 0),
  (2685, 319, '甘泉縣', 3, 0),
  (2686, 319, '富縣', 3, 0),
  (2687, 319, '洛川縣', 3, 0),
  (2688, 319, '宜川縣', 3, 0),
  (2689, 319, '黃龍縣', 3, 0),
  (2690, 319, '黃陵縣', 3, 0),
  (2691, 320, '榆陽區', 3, 0),
  (2692, 320, '神木縣', 3, 0),
  (2693, 320, '府谷縣', 3, 0),
  (2694, 320, '橫山縣', 3, 0),
  (2695, 320, '靖邊縣', 3, 0),
  (2696, 320, '定邊縣', 3, 0),
  (2697, 320, '綏德縣', 3, 0),
  (2698, 320, '米脂縣', 3, 0),
  (2699, 320, '佳縣', 3, 0),
  (2700, 320, '吳堡縣', 3, 0),
  (2701, 320, '清澗縣', 3, 0),
  (2702, 320, '子洲縣', 3, 0),
  (2703, 321, '長寧區', 3, 0),
  (2704, 321, '閘北區', 3, 0),
  (2705, 321, '閔行區', 3, 0),
  (2706, 321, '徐匯區', 3, 0),
  (2707, 321, '浦東新區', 3, 0),
  (2708, 321, '楊浦區', 3, 0),
  (2709, 321, '普陀區', 3, 0),
  (2710, 321, '靜安區', 3, 0),
  (2711, 321, '盧灣區', 3, 0),
  (2712, 321, '虹口區', 3, 0),
  (2713, 321, '黃浦區', 3, 0),
  (2714, 321, '南匯區', 3, 0),
  (2715, 321, '松江區', 3, 0),
  (2716, 321, '嘉定區', 3, 0),
  (2717, 321, '寶山區', 3, 0),
  (2718, 321, '青浦區', 3, 0),
  (2719, 321, '金山區', 3, 0),
  (2720, 321, '奉賢區', 3, 0),
  (2721, 321, '崇明縣', 3, 0),
  (2722, 322, '青羊區', 3, 0),
  (2723, 322, '錦江區', 3, 0),
  (2724, 322, '金牛區', 3, 0),
  (2725, 322, '武侯區', 3, 0),
  (2726, 322, '成華區', 3, 0),
  (2727, 322, '龍泉驛區', 3, 0),
  (2728, 322, '青白江區', 3, 0),
  (2729, 322, '新都區', 3, 0),
  (2730, 322, '溫江區', 3, 0),
  (2731, 322, '高新區', 3, 0),
  (2732, 322, '高新西區', 3, 0),
  (2733, 322, '都江堰市', 3, 0),
  (2734, 322, '彭州市', 3, 0),
  (2735, 322, '邛崍市', 3, 0),
  (2736, 322, '崇州市', 3, 0),
  (2737, 322, '金堂縣', 3, 0),
  (2738, 322, '雙流縣', 3, 0),
  (2739, 322, '郫縣', 3, 0),
  (2740, 322, '大邑縣', 3, 0),
  (2741, 322, '蒲江縣', 3, 0),
  (2742, 322, '新津縣', 3, 0),
  (2743, 322, '都江堰市', 3, 0),
  (2744, 322, '彭州市', 3, 0),
  (2745, 322, '邛崍市', 3, 0),
  (2746, 322, '崇州市', 3, 0),
  (2747, 322, '金堂縣', 3, 0),
  (2748, 322, '雙流縣', 3, 0),
  (2749, 322, '郫縣', 3, 0),
  (2750, 322, '大邑縣', 3, 0),
  (2751, 322, '蒲江縣', 3, 0),
  (2752, 322, '新津縣', 3, 0),
  (2753, 323, '涪城區', 3, 0),
  (2754, 323, '遊仙區', 3, 0),
  (2755, 323, '江油市', 3, 0),
  (2756, 323, '鹽亭縣', 3, 0),
  (2757, 323, '三臺縣', 3, 0),
  (2758, 323, '平武縣', 3, 0),
  (2759, 323, '安縣', 3, 0),
  (2760, 323, '梓潼縣', 3, 0),
  (2761, 323, '北川縣', 3, 0),
  (2762, 324, '馬爾康縣', 3, 0),
  (2763, 324, '汶川縣', 3, 0),
  (2764, 324, '理縣', 3, 0),
  (2765, 324, '茂縣', 3, 0),
  (2766, 324, '松潘縣', 3, 0),
  (2767, 324, '九寨溝縣', 3, 0),
  (2768, 324, '金川縣', 3, 0),
  (2769, 324, '小金縣', 3, 0),
  (2770, 324, '黑水縣', 3, 0),
  (2771, 324, '壤塘縣', 3, 0),
  (2772, 324, '阿壩縣', 3, 0),
  (2773, 324, '若爾蓋縣', 3, 0),
  (2774, 324, '紅原縣', 3, 0),
  (2775, 325, '巴州區', 3, 0),
  (2776, 325, '通江縣', 3, 0),
  (2777, 325, '南江縣', 3, 0),
  (2778, 325, '平昌縣', 3, 0),
  (2779, 326, '通川區', 3, 0),
  (2780, 326, '萬源市', 3, 0),
  (2781, 326, '達縣', 3, 0),
  (2782, 326, '宣漢縣', 3, 0),
  (2783, 326, '開江縣', 3, 0),
  (2784, 326, '大竹縣', 3, 0),
  (2785, 326, '渠縣', 3, 0),
  (2786, 327, '旌陽區', 3, 0),
  (2787, 327, '廣漢市', 3, 0),
  (2788, 327, '什邡市', 3, 0),
  (2789, 327, '綿竹市', 3, 0),
  (2790, 327, '羅江縣', 3, 0),
  (2791, 327, '中江縣', 3, 0),
  (2792, 328, '康定縣', 3, 0),
  (2793, 328, '丹巴縣', 3, 0),
  (2794, 328, '瀘定縣', 3, 0),
  (2795, 328, '爐霍縣', 3, 0),
  (2796, 328, '九龍縣', 3, 0),
  (2797, 328, '甘孜縣', 3, 0),
  (2798, 328, '雅江縣', 3, 0),
  (2799, 328, '新龍縣', 3, 0),
  (2800, 328, '道孚縣', 3, 0),
  (2801, 328, '白玉縣', 3, 0),
  (2802, 328, '理塘縣', 3, 0),
  (2803, 328, '德格縣', 3, 0),
  (2804, 328, '鄉城縣', 3, 0),
  (2805, 328, '石渠縣', 3, 0),
  (2806, 328, '稻城縣', 3, 0),
  (2807, 328, '色達縣', 3, 0),
  (2808, 328, '巴塘縣', 3, 0),
  (2809, 328, '得榮縣', 3, 0),
  (2810, 329, '廣安區', 3, 0),
  (2811, 329, '華鎣市', 3, 0),
  (2812, 329, '嶽池縣', 3, 0),
  (2813, 329, '武勝縣', 3, 0),
  (2814, 329, '鄰水縣', 3, 0),
  (2815, 330, '利州區', 3, 0),
  (2816, 330, '元壩區', 3, 0),
  (2817, 330, '朝天區', 3, 0),
  (2818, 330, '旺蒼縣', 3, 0),
  (2819, 330, '青川縣', 3, 0),
  (2820, 330, '劍閣縣', 3, 0),
  (2821, 330, '蒼溪縣', 3, 0),
  (2822, 331, '峨眉山市', 3, 0),
  (2823, 331, '樂山市', 3, 0),
  (2824, 331, '犍為縣', 3, 0),
  (2825, 331, '井研縣', 3, 0),
  (2826, 331, '夾江縣', 3, 0),
  (2827, 331, '沐川縣', 3, 0),
  (2828, 331, '峨邊', 3, 0),
  (2829, 331, '馬邊', 3, 0),
  (2830, 332, '西昌市', 3, 0),
  (2831, 332, '鹽源縣', 3, 0),
  (2832, 332, '德昌縣', 3, 0),
  (2833, 332, '會理縣', 3, 0),
  (2834, 332, '會東縣', 3, 0),
  (2835, 332, '寧南縣', 3, 0),
  (2836, 332, '普格縣', 3, 0),
  (2837, 332, '布拖縣', 3, 0),
  (2838, 332, '金陽縣', 3, 0),
  (2839, 332, '昭覺縣', 3, 0),
  (2840, 332, '喜德縣', 3, 0),
  (2841, 332, '冕寧縣', 3, 0),
  (2842, 332, '越西縣', 3, 0),
  (2843, 332, '甘洛縣', 3, 0),
  (2844, 332, '美姑縣', 3, 0),
  (2845, 332, '雷波縣', 3, 0),
  (2846, 332, '木裏', 3, 0),
  (2847, 333, '東坡區', 3, 0),
  (2848, 333, '仁壽縣', 3, 0),
  (2849, 333, '彭山縣', 3, 0),
  (2850, 333, '洪雅縣', 3, 0),
  (2851, 333, '丹棱縣', 3, 0),
  (2852, 333, '青神縣', 3, 0),
  (2853, 334, '閬中市', 3, 0),
  (2854, 334, '南部縣', 3, 0),
  (2855, 334, '營山縣', 3, 0),
  (2856, 334, '蓬安縣', 3, 0),
  (2857, 334, '儀隴縣', 3, 0),
  (2858, 334, '順慶區', 3, 0),
  (2859, 334, '高坪區', 3, 0),
  (2860, 334, '嘉陵區', 3, 0),
  (2861, 334, '西充縣', 3, 0),
  (2862, 335, '市中區', 3, 0),
  (2863, 335, '東興區', 3, 0),
  (2864, 335, '威遠縣', 3, 0),
  (2865, 335, '資中縣', 3, 0),
  (2866, 335, '隆昌縣', 3, 0),
  (2867, 336, '東  區', 3, 0),
  (2868, 336, '西  區', 3, 0),
  (2869, 336, '仁和區', 3, 0),
  (2870, 336, '米易縣', 3, 0),
  (2871, 336, '鹽邊縣', 3, 0),
  (2872, 337, '船山區', 3, 0),
  (2873, 337, '安居區', 3, 0),
  (2874, 337, '蓬溪縣', 3, 0),
  (2875, 337, '射洪縣', 3, 0),
  (2876, 337, '大英縣', 3, 0),
  (2877, 338, '雨城區', 3, 0),
  (2878, 338, '名山縣', 3, 0),
  (2879, 338, '滎經縣', 3, 0),
  (2880, 338, '漢源縣', 3, 0),
  (2881, 338, '石棉縣', 3, 0),
  (2882, 338, '天全縣', 3, 0),
  (2883, 338, '蘆山縣', 3, 0),
  (2884, 338, '寶興縣', 3, 0),
  (2885, 339, '翠屏區', 3, 0),
  (2886, 339, '宜賓縣', 3, 0),
  (2887, 339, '南溪縣', 3, 0),
  (2888, 339, '江安縣', 3, 0),
  (2889, 339, '長寧縣', 3, 0),
  (2890, 339, '高縣', 3, 0),
  (2891, 339, '拱縣', 3, 0),
  (2892, 339, '筠連縣', 3, 0),
  (2893, 339, '興文縣', 3, 0),
  (2894, 339, '屏山縣', 3, 0),
  (2895, 340, '雁江區', 3, 0),
  (2896, 340, '簡陽市', 3, 0),
  (2897, 340, '安嶽縣', 3, 0),
  (2898, 340, '樂至縣', 3, 0),
  (2899, 341, '大安區', 3, 0),
  (2900, 341, '自流井區', 3, 0),
  (2901, 341, '貢井區', 3, 0),
  (2902, 341, '沿灘區', 3, 0),
  (2903, 341, '榮縣', 3, 0),
  (2904, 341, '富順縣', 3, 0),
  (2905, 342, '江陽區', 3, 0),
  (2906, 342, '納溪區', 3, 0),
  (2907, 342, '龍馬潭區', 3, 0),
  (2908, 342, '瀘縣', 3, 0),
  (2909, 342, '合江縣', 3, 0),
  (2910, 342, '敘永縣', 3, 0),
  (2911, 342, '古藺縣', 3, 0),
  (2912, 343, '和平區', 3, 0),
  (2913, 343, '河西區', 3, 0),
  (2914, 343, '南開區', 3, 0),
  (2915, 343, '河北區', 3, 0),
  (2916, 343, '河東區', 3, 0),
  (2917, 343, '紅橋區', 3, 0),
  (2918, 343, '東麗區', 3, 0),
  (2919, 343, '津南區', 3, 0),
  (2920, 343, '西青區', 3, 0),
  (2921, 343, '北辰區', 3, 0),
  (2922, 343, '塘沽區', 3, 0),
  (2923, 343, '漢沽區', 3, 0),
  (2924, 343, '大港區', 3, 0),
  (2925, 343, '武清區', 3, 0),
  (2926, 343, '寶坻區', 3, 0),
  (2927, 343, '經濟開發區', 3, 0),
  (2928, 343, '寧河縣', 3, 0),
  (2929, 343, '靜海縣', 3, 0),
  (2930, 343, '薊縣', 3, 0),
  (2931, 344, '城關區', 3, 0),
  (2932, 344, '林周縣', 3, 0),
  (2933, 344, '當雄縣', 3, 0),
  (2934, 344, '尼木縣', 3, 0),
  (2935, 344, '曲水縣', 3, 0),
  (2936, 344, '堆龍德慶縣', 3, 0),
  (2937, 344, '達孜縣', 3, 0),
  (2938, 344, '墨竹工卡縣', 3, 0),
  (2939, 345, '噶爾縣', 3, 0),
  (2940, 345, '普蘭縣', 3, 0),
  (2941, 345, '劄達縣', 3, 0),
  (2942, 345, '日土縣', 3, 0),
  (2943, 345, '革吉縣', 3, 0),
  (2944, 345, '改則縣', 3, 0),
  (2945, 345, '措勤縣', 3, 0),
  (2946, 346, '昌都縣', 3, 0),
  (2947, 346, '江達縣', 3, 0),
  (2948, 346, '貢覺縣', 3, 0),
  (2949, 346, '類烏齊縣', 3, 0),
  (2950, 346, '丁青縣', 3, 0),
  (2951, 346, '察雅縣', 3, 0),
  (2952, 346, '八宿縣', 3, 0),
  (2953, 346, '左貢縣', 3, 0),
  (2954, 346, '芒康縣', 3, 0),
  (2955, 346, '洛隆縣', 3, 0),
  (2956, 346, '邊壩縣', 3, 0),
  (2957, 347, '林芝縣', 3, 0),
  (2958, 347, '工布江達縣', 3, 0),
  (2959, 347, '米林縣', 3, 0),
  (2960, 347, '墨脫縣', 3, 0),
  (2961, 347, '波密縣', 3, 0),
  (2962, 347, '察隅縣', 3, 0),
  (2963, 347, '朗縣', 3, 0),
  (2964, 348, '那曲縣', 3, 0),
  (2965, 348, '嘉黎縣', 3, 0),
  (2966, 348, '比如縣', 3, 0),
  (2967, 348, '聶榮縣', 3, 0),
  (2968, 348, '安多縣', 3, 0),
  (2969, 348, '申紮縣', 3, 0),
  (2970, 348, '索縣', 3, 0),
  (2971, 348, '班戈縣', 3, 0),
  (2972, 348, '巴青縣', 3, 0),
  (2973, 348, '尼瑪縣', 3, 0),
  (2974, 349, '日喀則市', 3, 0),
  (2975, 349, '南木林縣', 3, 0),
  (2976, 349, '江孜縣', 3, 0),
  (2977, 349, '定日縣', 3, 0),
  (2978, 349, '薩迦縣', 3, 0),
  (2979, 349, '拉孜縣', 3, 0),
  (2980, 349, '昂仁縣', 3, 0),
  (2981, 349, '謝通門縣', 3, 0),
  (2982, 349, '白朗縣', 3, 0),
  (2983, 349, '仁布縣', 3, 0),
  (2984, 349, '康馬縣', 3, 0),
  (2985, 349, '定結縣', 3, 0),
  (2986, 349, '仲巴縣', 3, 0),
  (2987, 349, '亞東縣', 3, 0),
  (2988, 349, '吉隆縣', 3, 0),
  (2989, 349, '聶拉木縣', 3, 0),
  (2990, 349, '薩嘎縣', 3, 0),
  (2991, 349, '崗巴縣', 3, 0),
  (2992, 350, '乃東縣', 3, 0),
  (2993, 350, '紮囊縣', 3, 0),
  (2994, 350, '貢嘎縣', 3, 0),
  (2995, 350, '桑日縣', 3, 0),
  (2996, 350, '瓊結縣', 3, 0),
  (2997, 350, '曲松縣', 3, 0),
  (2998, 350, '措美縣', 3, 0),
  (2999, 350, '洛紮縣', 3, 0),
  (3000, 350, '加查縣', 3, 0),
  (3001, 350, '隆子縣', 3, 0),
  (3002, 350, '錯那縣', 3, 0),
  (3003, 350, '浪卡子縣', 3, 0),
  (3004, 351, '天山區', 3, 0),
  (3005, 351, '沙依巴克區', 3, 0),
  (3006, 351, '新市區', 3, 0),
  (3007, 351, '水磨溝區', 3, 0),
  (3008, 351, '頭屯河區', 3, 0),
  (3009, 351, '達阪城區', 3, 0),
  (3010, 351, '米東區', 3, 0),
  (3011, 351, '烏魯木齊縣', 3, 0),
  (3012, 352, '阿克蘇市', 3, 0),
  (3013, 352, '溫宿縣', 3, 0),
  (3014, 352, '庫車縣', 3, 0),
  (3015, 352, '沙雅縣', 3, 0),
  (3016, 352, '新和縣', 3, 0),
  (3017, 352, '拜城縣', 3, 0),
  (3018, 352, '烏什縣', 3, 0),
  (3019, 352, '阿瓦提縣', 3, 0),
  (3020, 352, '柯坪縣', 3, 0),
  (3021, 353, '阿拉爾市', 3, 0),
  (3022, 354, '庫爾勒市', 3, 0),
  (3023, 354, '輪臺縣', 3, 0),
  (3024, 354, '尉犁縣', 3, 0),
  (3025, 354, '若羌縣', 3, 0),
  (3026, 354, '且末縣', 3, 0),
  (3027, 354, '焉耆', 3, 0),
  (3028, 354, '和靜縣', 3, 0),
  (3029, 354, '和碩縣', 3, 0),
  (3030, 354, '博湖縣', 3, 0),
  (3031, 355, '博樂市', 3, 0),
  (3032, 355, '精河縣', 3, 0),
  (3033, 355, '溫泉縣', 3, 0),
  (3034, 356, '呼圖壁縣', 3, 0),
  (3035, 356, '米泉市', 3, 0),
  (3036, 356, '昌吉市', 3, 0),
  (3037, 356, '阜康市', 3, 0),
  (3038, 356, '瑪納斯縣', 3, 0),
  (3039, 356, '奇臺縣', 3, 0),
  (3040, 356, '吉木薩爾縣', 3, 0),
  (3041, 356, '木壘', 3, 0),
  (3042, 357, '哈密市', 3, 0),
  (3043, 357, '伊吾縣', 3, 0),
  (3044, 357, '巴裏坤', 3, 0),
  (3045, 358, '和田市', 3, 0),
  (3046, 358, '和田縣', 3, 0),
  (3047, 358, '墨玉縣', 3, 0),
  (3048, 358, '皮山縣', 3, 0),
  (3049, 358, '洛浦縣', 3, 0),
  (3050, 358, '策勒縣', 3, 0),
  (3051, 358, '於田縣', 3, 0),
  (3052, 358, '民豐縣', 3, 0),
  (3053, 359, '喀什市', 3, 0),
  (3054, 359, '疏附縣', 3, 0),
  (3055, 359, '疏勒縣', 3, 0),
  (3056, 359, '英吉沙縣', 3, 0),
  (3057, 359, '澤普縣', 3, 0),
  (3058, 359, '莎車縣', 3, 0),
  (3059, 359, '葉城縣', 3, 0),
  (3060, 359, '麥蓋提縣', 3, 0),
  (3061, 359, '嶽普湖縣', 3, 0),
  (3062, 359, '伽師縣', 3, 0),
  (3063, 359, '巴楚縣', 3, 0),
  (3064, 359, '塔什庫爾幹', 3, 0),
  (3065, 360, '克拉瑪依市', 3, 0),
  (3066, 361, '阿圖什市', 3, 0),
  (3067, 361, '阿克陶縣', 3, 0),
  (3068, 361, '阿合奇縣', 3, 0),
  (3069, 361, '烏恰縣', 3, 0),
  (3070, 362, '石河子市', 3, 0),
  (3071, 363, '圖木舒克市', 3, 0),
  (3072, 364, '吐魯番市', 3, 0),
  (3073, 364, '鄯善縣', 3, 0),
  (3074, 364, '托克遜縣', 3, 0),
  (3075, 365, '五家渠市', 3, 0),
  (3076, 366, '阿勒泰市', 3, 0),
  (3077, 366, '布克賽爾', 3, 0),
  (3078, 366, '伊寧市', 3, 0),
  (3079, 366, '布爾津縣', 3, 0),
  (3080, 366, '奎屯市', 3, 0),
  (3081, 366, '烏蘇市', 3, 0),
  (3082, 366, '額敏縣', 3, 0),
  (3083, 366, '富蘊縣', 3, 0),
  (3084, 366, '伊寧縣', 3, 0),
  (3085, 366, '福海縣', 3, 0),
  (3086, 366, '霍城縣', 3, 0),
  (3087, 366, '沙灣縣', 3, 0),
  (3088, 366, '鞏留縣', 3, 0),
  (3089, 366, '哈巴河縣', 3, 0),
  (3090, 366, '托裏縣', 3, 0),
  (3091, 366, '青河縣', 3, 0),
  (3092, 366, '新源縣', 3, 0),
  (3093, 366, '裕民縣', 3, 0),
  (3094, 366, '和布克賽爾', 3, 0),
  (3095, 366, '吉木乃縣', 3, 0),
  (3096, 366, '昭蘇縣', 3, 0),
  (3097, 366, '特克斯縣', 3, 0),
  (3098, 366, '尼勒克縣', 3, 0),
  (3099, 366, '察布查爾', 3, 0),
  (3100, 367, '盤龍區', 3, 0),
  (3101, 367, '五華區', 3, 0),
  (3102, 367, '官渡區', 3, 0),
  (3103, 367, '西山區', 3, 0),
  (3104, 367, '東川區', 3, 0),
  (3105, 367, '安寧市', 3, 0),
  (3106, 367, '呈貢縣', 3, 0),
  (3107, 367, '晉寧縣', 3, 0),
  (3108, 367, '富民縣', 3, 0),
  (3109, 367, '宜良縣', 3, 0),
  (3110, 367, '嵩明縣', 3, 0),
  (3111, 367, '石林縣', 3, 0),
  (3112, 367, '祿勸', 3, 0),
  (3113, 367, '尋甸', 3, 0),
  (3114, 368, '蘭坪', 3, 0),
  (3115, 368, '瀘水縣', 3, 0),
  (3116, 368, '福貢縣', 3, 0),
  (3117, 368, '貢山', 3, 0),
  (3118, 369, '寧洱', 3, 0),
  (3119, 369, '思茅區', 3, 0),
  (3120, 369, '墨江', 3, 0),
  (3121, 369, '景東', 3, 0),
  (3122, 369, '景谷', 3, 0),
  (3123, 369, '鎮元', 3, 0),
  (3124, 369, '江城', 3, 0),
  (3125, 369, '孟連', 3, 0),
  (3126, 369, '瀾滄', 3, 0),
  (3127, 369, '西盟', 3, 0),
  (3128, 370, '古城區', 3, 0),
  (3129, 370, '寧蒗', 3, 0),
  (3130, 370, '玉龍', 3, 0),
  (3131, 370, '永勝縣', 3, 0),
  (3132, 370, '華坪縣', 3, 0),
  (3133, 371, '隆陽區', 3, 0),
  (3134, 371, '施甸縣', 3, 0),
  (3135, 371, '騰沖縣', 3, 0),
  (3136, 371, '龍陵縣', 3, 0),
  (3137, 371, '昌寧縣', 3, 0),
  (3138, 372, '楚雄市', 3, 0),
  (3139, 372, '雙柏縣', 3, 0),
  (3140, 372, '牟定縣', 3, 0),
  (3141, 372, '南華縣', 3, 0),
  (3142, 372, '姚安縣', 3, 0),
  (3143, 372, '大姚縣', 3, 0),
  (3144, 372, '永仁縣', 3, 0),
  (3145, 372, '元謀縣', 3, 0),
  (3146, 372, '武定縣', 3, 0),
  (3147, 372, '祿豐縣', 3, 0),
  (3148, 373, '大理市', 3, 0),
  (3149, 373, '祥雲縣', 3, 0),
  (3150, 373, '賓川縣', 3, 0),
  (3151, 373, '彌渡縣', 3, 0),
  (3152, 373, '永平縣', 3, 0),
  (3153, 373, '雲龍縣', 3, 0),
  (3154, 373, '洱源縣', 3, 0),
  (3155, 373, '劍川縣', 3, 0),
  (3156, 373, '鶴慶縣', 3, 0),
  (3157, 373, '漾濞', 3, 0),
  (3158, 373, '南澗', 3, 0),
  (3159, 373, '巍山', 3, 0),
  (3160, 374, '潞西市', 3, 0),
  (3161, 374, '瑞麗市', 3, 0),
  (3162, 374, '梁河縣', 3, 0),
  (3163, 374, '盈江縣', 3, 0),
  (3164, 374, '隴川縣', 3, 0),
  (3165, 375, '香格裏拉縣', 3, 0),
  (3166, 375, '德欽縣', 3, 0),
  (3167, 375, '維西', 3, 0),
  (3168, 376, '瀘西縣', 3, 0),
  (3169, 376, '蒙自縣', 3, 0),
  (3170, 376, '個舊市', 3, 0),
  (3171, 376, '開遠市', 3, 0),
  (3172, 376, '綠春縣', 3, 0),
  (3173, 376, '建水縣', 3, 0),
  (3174, 376, '石屏縣', 3, 0),
  (3175, 376, '彌勒縣', 3, 0),
  (3176, 376, '元陽縣', 3, 0),
  (3177, 376, '紅河縣', 3, 0),
  (3178, 376, '金平', 3, 0),
  (3179, 376, '河口', 3, 0),
  (3180, 376, '屏邊', 3, 0),
  (3181, 377, '臨翔區', 3, 0),
  (3182, 377, '鳳慶縣', 3, 0),
  (3183, 377, '雲縣', 3, 0),
  (3184, 377, '永德縣', 3, 0),
  (3185, 377, '鎮康縣', 3, 0),
  (3186, 377, '雙江', 3, 0),
  (3187, 377, '耿馬', 3, 0),
  (3188, 377, '滄源', 3, 0),
  (3189, 378, '麒麟區', 3, 0),
  (3190, 378, '宣威市', 3, 0),
  (3191, 378, '馬龍縣', 3, 0),
  (3192, 378, '陸良縣', 3, 0),
  (3193, 378, '師宗縣', 3, 0),
  (3194, 378, '羅平縣', 3, 0),
  (3195, 378, '富源縣', 3, 0),
  (3196, 378, '會澤縣', 3, 0),
  (3197, 378, '沾益縣', 3, 0),
  (3198, 379, '文山縣', 3, 0),
  (3199, 379, '硯山縣', 3, 0),
  (3200, 379, '西疇縣', 3, 0),
  (3201, 379, '麻栗坡縣', 3, 0),
  (3202, 379, '馬關縣', 3, 0),
  (3203, 379, '丘北縣', 3, 0),
  (3204, 379, '廣南縣', 3, 0),
  (3205, 379, '富寧縣', 3, 0),
  (3206, 380, '景洪市', 3, 0),
  (3207, 380, '猛海縣', 3, 0),
  (3208, 380, '猛臘縣', 3, 0),
  (3209, 381, '紅塔區', 3, 0),
  (3210, 381, '江川縣', 3, 0),
  (3211, 381, '澄江縣', 3, 0),
  (3212, 381, '通海縣', 3, 0),
  (3213, 381, '華寧縣', 3, 0),
  (3214, 381, '易門縣', 3, 0),
  (3215, 381, '峨山', 3, 0),
  (3216, 381, '新平', 3, 0),
  (3217, 381, '元江', 3, 0),
  (3218, 382, '昭陽區', 3, 0),
  (3219, 382, '魯甸縣', 3, 0),
  (3220, 382, '巧家縣', 3, 0),
  (3221, 382, '鹽津縣', 3, 0),
  (3222, 382, '大關縣', 3, 0),
  (3223, 382, '永善縣', 3, 0),
  (3224, 382, '綏江縣', 3, 0),
  (3225, 382, '鎮雄縣', 3, 0),
  (3226, 382, '彜良縣', 3, 0),
  (3227, 382, '威信縣', 3, 0),
  (3228, 382, '水富縣', 3, 0),
  (3229, 383, '西湖區', 3, 0),
  (3230, 383, '上城區', 3, 0),
  (3231, 383, '下城區', 3, 0),
  (3232, 383, '拱墅區', 3, 0),
  (3233, 383, '濱江區', 3, 0),
  (3234, 383, '江幹區', 3, 0),
  (3235, 383, '蕭山區', 3, 0),
  (3236, 383, '余杭區', 3, 0),
  (3237, 383, '市郊', 3, 0),
  (3238, 383, '建德市', 3, 0),
  (3239, 383, '富陽市', 3, 0),
  (3240, 383, '臨安市', 3, 0),
  (3241, 383, '桐廬縣', 3, 0),
  (3242, 383, '淳安縣', 3, 0),
  (3243, 384, '吳興區', 3, 0),
  (3244, 384, '南潯區', 3, 0),
  (3245, 384, '德清縣', 3, 0),
  (3246, 384, '長興縣', 3, 0),
  (3247, 384, '安吉縣', 3, 0),
  (3248, 385, '南湖區', 3, 0),
  (3249, 385, '秀洲區', 3, 0),
  (3250, 385, '海寧市', 3, 0),
  (3251, 385, '嘉善縣', 3, 0),
  (3252, 385, '平湖市', 3, 0),
  (3253, 385, '桐鄉市', 3, 0),
  (3254, 385, '海鹽縣', 3, 0),
  (3255, 386, '婺城區', 3, 0),
  (3256, 386, '金東區', 3, 0),
  (3257, 386, '蘭溪市', 3, 0),
  (3258, 386, '市區', 3, 0),
  (3259, 386, '佛堂鎮', 3, 0),
  (3260, 386, '上溪鎮', 3, 0),
  (3261, 386, '義亭鎮', 3, 0),
  (3262, 386, '大陳鎮', 3, 0),
  (3263, 386, '蘇溪鎮', 3, 0),
  (3264, 386, '赤岸鎮', 3, 0),
  (3265, 386, '東陽市', 3, 0),
  (3266, 386, '永康市', 3, 0),
  (3267, 386, '武義縣', 3, 0),
  (3268, 386, '浦江縣', 3, 0),
  (3269, 386, '磐安縣', 3, 0),
  (3270, 387, '蓮都區', 3, 0),
  (3271, 387, '龍泉市', 3, 0),
  (3272, 387, '青田縣', 3, 0),
  (3273, 387, '縉雲縣', 3, 0),
  (3274, 387, '遂昌縣', 3, 0),
  (3275, 387, '松陽縣', 3, 0),
  (3276, 387, '雲和縣', 3, 0),
  (3277, 387, '慶元縣', 3, 0),
  (3278, 387, '景寧', 3, 0),
  (3279, 388, '海曙區', 3, 0),
  (3280, 388, '江東區', 3, 0),
  (3281, 388, '江北區', 3, 0),
  (3282, 388, '鎮海區', 3, 0),
  (3283, 388, '北侖區', 3, 0),
  (3284, 388, '鄞州區', 3, 0),
  (3285, 388, '余姚市', 3, 0),
  (3286, 388, '慈溪市', 3, 0),
  (3287, 388, '奉化市', 3, 0),
  (3288, 388, '象山縣', 3, 0),
  (3289, 388, '寧海縣', 3, 0),
  (3290, 389, '越城區', 3, 0),
  (3291, 389, '上虞市', 3, 0),
  (3292, 389, '嵊州市', 3, 0),
  (3293, 389, '紹興縣', 3, 0),
  (3294, 389, '新昌縣', 3, 0),
  (3295, 389, '諸暨市', 3, 0),
  (3296, 390, '椒江區', 3, 0),
  (3297, 390, '黃巖區', 3, 0),
  (3298, 390, '路橋區', 3, 0),
  (3299, 390, '溫嶺市', 3, 0),
  (3300, 390, '臨海市', 3, 0),
  (3301, 390, '玉環縣', 3, 0),
  (3302, 390, '三門縣', 3, 0),
  (3303, 390, '天臺縣', 3, 0),
  (3304, 390, '仙居縣', 3, 0),
  (3305, 391, '鹿城區', 3, 0),
  (3306, 391, '龍灣區', 3, 0),
  (3307, 391, '甌海區', 3, 0),
  (3308, 391, '瑞安市', 3, 0),
  (3309, 391, '樂清市', 3, 0),
  (3310, 391, '洞頭縣', 3, 0),
  (3311, 391, '永嘉縣', 3, 0),
  (3312, 391, '平陽縣', 3, 0),
  (3313, 391, '蒼南縣', 3, 0),
  (3314, 391, '文成縣', 3, 0),
  (3315, 391, '泰順縣', 3, 0),
  (3316, 392, '定海區', 3, 0),
  (3317, 392, '普陀區', 3, 0),
  (3318, 392, '岱山縣', 3, 0),
  (3319, 392, '嵊泗縣', 3, 0),
  (3320, 393, '衢州市', 3, 0),
  (3321, 393, '江山市', 3, 0),
  (3322, 393, '常山縣', 3, 0),
  (3323, 393, '開化縣', 3, 0),
  (3324, 393, '龍遊縣', 3, 0),
  (3325, 394, '合川區', 3, 0),
  (3326, 394, '江津區', 3, 0),
  (3327, 394, '南川區', 3, 0),
  (3328, 394, '永川區', 3, 0),
  (3329, 394, '南岸區', 3, 0),
  (3330, 394, '渝北區', 3, 0),
  (3331, 394, '萬盛區', 3, 0),
  (3332, 394, '大渡口區', 3, 0),
  (3333, 394, '萬州區', 3, 0),
  (3334, 394, '北碚區', 3, 0),
  (3335, 394, '沙坪壩區', 3, 0),
  (3336, 394, '巴南區', 3, 0),
  (3337, 394, '涪陵區', 3, 0),
  (3338, 394, '江北區', 3, 0),
  (3339, 394, '九龍坡區', 3, 0),
  (3340, 394, '渝中區', 3, 0),
  (3341, 394, '黔江開發區', 3, 0),
  (3342, 394, '長壽區', 3, 0),
  (3343, 394, '雙橋區', 3, 0),
  (3344, 394, '綦江縣', 3, 0),
  (3345, 394, '潼南縣', 3, 0),
  (3346, 394, '銅梁縣', 3, 0),
  (3347, 394, '大足縣', 3, 0),
  (3348, 394, '榮昌縣', 3, 0),
  (3349, 394, '璧山縣', 3, 0),
  (3350, 394, '墊江縣', 3, 0),
  (3351, 394, '武隆縣', 3, 0),
  (3352, 394, '豐都縣', 3, 0),
  (3353, 394, '城口縣', 3, 0),
  (3354, 394, '梁平縣', 3, 0),
  (3355, 394, '開縣', 3, 0),
  (3356, 394, '巫溪縣', 3, 0),
  (3357, 394, '巫山縣', 3, 0),
  (3358, 394, '奉節縣', 3, 0),
  (3359, 394, '雲陽縣', 3, 0),
  (3360, 394, '忠縣', 3, 0),
  (3361, 394, '石柱', 3, 0),
  (3362, 394, '彭水', 3, 0),
  (3363, 394, '酉陽', 3, 0),
  (3364, 394, '秀山', 3, 0),
  (3365, 395, '沙田區', 3, 0),
  (3366, 395, '東區', 3, 0),
  (3367, 395, '觀塘區', 3, 0),
  (3368, 395, '黃大仙區', 3, 0),
  (3369, 395, '九龍城區', 3, 0),
  (3370, 395, '屯門區', 3, 0),
  (3371, 395, '葵青區', 3, 0),
  (3372, 395, '元朗區', 3, 0),
  (3373, 395, '深水埗區', 3, 0),
  (3374, 395, '西貢區', 3, 0),
  (3375, 395, '大埔區', 3, 0),
  (3376, 395, '灣仔區', 3, 0),
  (3377, 395, '油尖旺區', 3, 0),
  (3378, 395, '北區', 3, 0),
  (3379, 395, '南區', 3, 0),
  (3380, 395, '荃灣區', 3, 0),
  (3381, 395, '中西區', 3, 0),
  (3382, 395, '離島區', 3, 0),
  (3383, 396, '澳門', 3, 0),
  (3384, 397, '臺北', 3, 0),
  (3385, 397, '高雄', 3, 0),
  (3386, 397, '基隆', 3, 0),
  (3387, 397, '臺中', 3, 0),
  (3388, 397, '臺南', 3, 0),
  (3389, 397, '新竹', 3, 0),
  (3390, 397, '嘉義', 3, 0),
  (3391, 397, '宜蘭縣', 3, 0),
  (3392, 397, '桃園縣', 3, 0),
  (3393, 397, '苗栗縣', 3, 0),
  (3394, 397, '彰化縣', 3, 0),
  (3395, 397, '南投縣', 3, 0),
  (3396, 397, '雲林縣', 3, 0),
  (3397, 397, '屏東縣', 3, 0),
  (3398, 397, '臺東縣', 3, 0),
  (3399, 397, '花蓮縣', 3, 0),
  (3400, 397, '澎湖縣', 3, 0),
  (3401, 3, '合肥', 2, 0),
  (3402, 3401, '廬陽區', 3, 0),
  (3403, 3401, '瑤海區', 3, 0),
  (3404, 3401, '蜀山區', 3, 0),
  (3405, 3401, '包河區', 3, 0),
  (3406, 3401, '長豐縣', 3, 0),
  (3407, 3401, '肥東縣', 3, 0),
  (3408, 3401, '肥西縣', 3, 0);

--
-- `ecs_shop_config`
--
INSERT INTO `ecs_shop_config` (
    `id`,
    `parent_id`,
    `code`,
    `type`,
    `store_range`,
    `store_dir`,
    `value`,
    `sort_order`
  )
VALUES (1, 0, 'shop_info', 'group', '', '', '', '1'),
  (2, 0, 'basic', 'group', '', '', '', '1'),
  (3, 0, 'display', 'group', '', '', '', '1'),
  (4, 0, 'shopping_flow', 'group', '', '', '', '1'),
  (5, 0, 'smtp', 'group', '', '', '', '1'),
  (6, 0, 'hidden', 'hidden', '', '', '', '1'),
  (7, 0, 'goods', 'group', '', '', '', '1'),
  (8, 0, 'sms', 'group', '', '', '', '1'),
  (9, 0, 'wap', 'group', '', '', '', '1'),
  (
    101,
    1,
    'shop_name',
    'text',
    '',
    '',
    'ECSHOP',
    '1'
  ),
  (
    102,
    1,
    'shop_title',
    'text',
    '',
    '',
    'ECSHOP演示站',
    '1'
  ),
  (
    103,
    1,
    'shop_desc',
    'text',
    '',
    '',
    'ECSHOP演示站',
    '1'
  ),
  (
    104,
    1,
    'shop_keywords',
    'text',
    '',
    '',
    'ECSHOP演示站',
    '1'
  ),
  (
    105,
    1,
    'shop_country',
    'manual',
    '',
    '',
    '1',
    '1'
  ),
  (
    106,
    1,
    'shop_province',
    'manual',
    '',
    '',
    '2',
    '1'
  ),
  (107, 1, 'shop_city', 'manual', '', '', '52', '1'),
  (108, 1, 'shop_address', 'text', '', '', '', '1'),
  (109, 1, 'qq', 'text', '', '', '', '1'),
  (110, 1, 'ww', 'text', '', '', '', '1'),
  (111, 1, 'skype', 'text', '', '', '', '1'),
  (112, 1, 'ym', 'text', '', '', '', '1'),
  (113, 1, 'msn', 'text', '', '', '', '1'),
  (114, 1, 'service_email', 'text', '', '', '', '1'),
  (115, 1, 'service_phone', 'text', '', '', '', '1'),
  (
    116,
    1,
    'shop_closed',
    'select',
    '0,1',
    '',
    '0',
    '1'
  ),
  (
    117,
    1,
    'close_comment',
    'textarea',
    '',
    '',
    '',
    '1'
  ),
  (
    118,
    1,
    'shop_logo',
    'file',
    '',
    '../themes/{$template}/images/',
    '',
    '1'
  ),
  (
    119,
    1,
    'licensed',
    'select',
    '0,1',
    '',
    '1',
    '1'
  ),
  (
    120,
    1,
    'user_notice',
    'textarea',
    '',
    '',
    '用戶中心公告！',
    '1'
  ),
  (
    121,
    1,
    'shop_notice',
    'textarea',
    '',
    '',
    '',
    '1'
  ),
  (
    122,
    1,
    'shop_reg_closed',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (201, 2, 'lang', 'manual', '', '', 'zh_tw', '1'),
  (202, 2, 'icp_number', 'text', '', '', '', '1'),
  (
    203,
    2,
    'icp_file',
    'file',
    '',
    '../cert/',
    '',
    '1'
  ),
  (
    204,
    2,
    'watermark',
    'file',
    '',
    '../images/',
    '',
    '1'
  ),
  (
    205,
    2,
    'watermark_place',
    'select',
    '0,1,2,3,4,5',
    '',
    '1',
    '1'
  ),
  (
    206,
    2,
    'watermark_alpha',
    'text',
    '',
    '',
    '65',
    '1'
  ),
  (
    207,
    2,
    'use_storage',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    208,
    2,
    'market_price_rate',
    'text',
    '',
    '',
    '1.2',
    '1'
  ),
  (
    209,
    2,
    'rewrite',
    'select',
    '0,1,2',
    '',
    '0',
    '1'
  ),
  (
    210,
    2,
    'integral_name',
    'text',
    '',
    '',
    '積分',
    '1'
  ),
  (
    211,
    2,
    'integral_scale',
    'text',
    '',
    '',
    '1',
    '1'
  ),
  (
    212,
    2,
    'integral_percent',
    'text',
    '',
    '',
    '1',
    '1'
  ),
  (213, 2, 'sn_prefix', 'text', '', '', 'ECS', '1'),
  (
    214,
    2,
    'comment_check',
    'select',
    '0,1',
    '',
    '1',
    '1'
  ),
  (
    215,
    2,
    'no_picture',
    'file',
    '',
    '../images/',
    '',
    '1'
  ),
  (
    218,
    2,
    'stats_code',
    'textarea',
    '',
    '',
    '',
    '1'
  ),
  (
    219,
    2,
    'cache_time',
    'text',
    '',
    '',
    '3600',
    '1'
  ),
  (
    220,
    2,
    'register_points',
    'text',
    '',
    '',
    '0',
    '1'
  ),
  (
    221,
    2,
    'enable_gzip',
    'select',
    '0,1',
    '',
    '0',
    '1'
  ),
  (
    222,
    2,
    'top10_time',
    'select',
    '0,1,2,3,4',
    '',
    '0',
    '1'
  ),
  (
    223,
    2,
    'timezone',
    'options',
    '-12,-11,-10,-9,-8,-7,-6,-5,-4,-3.5,-3,-2,-1,0,1,2,3,3.5,4,4.5,5,5.5,5.75,6,6.5,7,8,9,9.5,10,11,12',
    '',
    '8',
    '1'
  ),
  (
    224,
    2,
    'upload_size_limit',
    'options',
    '-1,0,64,128,256,512,1024,2048,4096',
    '',
    '64',
    '1'
  ),
  (
    226,
    2,
    'cron_method',
    'select',
    '0,1',
    '',
    '0',
    '1'
  ),
  (
    227,
    2,
    'comment_factor',
    'select',
    '0,1,2,3',
    '',
    '0',
    '1'
  ),
  (
    228,
    2,
    'enable_order_check',
    'select',
    '0,1',
    '',
    '1',
    '1'
  ),
  (
    229,
    2,
    'default_storage',
    'text',
    '',
    '',
    '1',
    '1'
  ),
  (
    230,
    2,
    'bgcolor',
    'text',
    '',
    '',
    '#FFFFFF',
    '1'
  ),
  (
    231,
    2,
    'visit_stats',
    'select',
    'on,off',
    '',
    'on',
    '1'
  ),
  (
    232,
    2,
    'send_mail_on',
    'select',
    'on,off',
    '',
    'off',
    '1'
  ),
  (
    233,
    2,
    'auto_generate_gallery',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    234,
    2,
    'retain_original_img',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    235,
    2,
    'member_email_validate',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    236,
    2,
    'message_board',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    239,
    2,
    'certificate_id',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (240, 2, 'token', 'hidden', '', '', '', '1'),
  (
    241,
    2,
    'certi',
    'hidden',
    '',
    '',
    'http://service.shopex.cn/openapi/api.php',
    '1'
  ),
  (
    242,
    2,
    'send_verify_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (243, 2, 'ent_id', 'hidden', '', '', '', '1'),
  (244, 2, 'ent_ac', 'hidden', '', '', '', '1'),
  (245, 2, 'ent_sign', 'hidden', '', '', '', '1'),
  (246, 2, 'ent_email', 'hidden', '', '', '', '1'),
  (
    301,
    3,
    'date_format',
    'hidden',
    '',
    '',
    'Y-m-d',
    '1'
  ),
  (
    302,
    3,
    'time_format',
    'text',
    '',
    '',
    'Y-m-d H:i:s',
    '1'
  ),
  (
    303,
    3,
    'currency_format',
    'text',
    '',
    '',
    '￥%s元',
    '1'
  ),
  (
    304,
    3,
    'thumb_width',
    'text',
    '',
    '',
    '100',
    '1'
  ),
  (
    305,
    3,
    'thumb_height',
    'text',
    '',
    '',
    '100',
    '1'
  ),
  (
    306,
    3,
    'image_width',
    'text',
    '',
    '',
    '230',
    '1'
  ),
  (
    307,
    3,
    'image_height',
    'text',
    '',
    '',
    '230',
    '1'
  ),
  (312, 3, 'top_number', 'text', '', '', '10', '1'),
  (
    313,
    3,
    'history_number',
    'text',
    '',
    '',
    '5',
    '1'
  ),
  (
    314,
    3,
    'comments_number',
    'text',
    '',
    '',
    '5',
    '1'
  ),
  (315, 3, 'bought_goods', 'text', '', '', '3', '1'),
  (
    316,
    3,
    'article_number',
    'text',
    '',
    '',
    '8',
    '1'
  ),
  (
    317,
    3,
    'goods_name_length',
    'text',
    '',
    '',
    '7',
    '1'
  ),
  (
    318,
    3,
    'price_format',
    'select',
    '0,1,2,3,4,5',
    '',
    '5',
    '1'
  ),
  (319, 3, 'page_size', 'text', '', '', '10', '1'),
  (
    320,
    3,
    'sort_order_type',
    'select',
    '0,1,2',
    '',
    '0',
    '1'
  ),
  (
    321,
    3,
    'sort_order_method',
    'select',
    '0,1',
    '',
    '0',
    '1'
  ),
  (
    322,
    3,
    'show_order_type',
    'select',
    '0,1,2',
    '',
    '1',
    '1'
  ),
  (
    323,
    3,
    'attr_related_number',
    'text',
    '',
    '',
    '5',
    '1'
  ),
  (
    324,
    3,
    'goods_gallery_number',
    'text',
    '',
    '',
    '5',
    '1'
  ),
  (
    325,
    3,
    'article_title_length',
    'text',
    '',
    '',
    '16',
    '1'
  ),
  (
    326,
    3,
    'name_of_region_1',
    'text',
    '',
    '',
    '國家',
    '1'
  ),
  (
    327,
    3,
    'name_of_region_2',
    'text',
    '',
    '',
    '省',
    '1'
  ),
  (
    328,
    3,
    'name_of_region_3',
    'text',
    '',
    '',
    '市',
    '1'
  ),
  (
    329,
    3,
    'name_of_region_4',
    'text',
    '',
    '',
    '區',
    '1'
  ),
  (
    330,
    3,
    'search_keywords',
    'text',
    '',
    '',
    '',
    '0'
  ),
  (
    332,
    3,
    'related_goods_number',
    'text',
    '',
    '',
    '4',
    '1'
  ),
  (
    333,
    3,
    'help_open',
    'select',
    '0,1',
    '',
    '1',
    '1'
  ),
  (
    334,
    3,
    'article_page_size',
    'text',
    '',
    '',
    '10',
    '1'
  ),
  (
    335,
    3,
    'page_style',
    'select',
    '0,1',
    '',
    '1',
    '1'
  ),
  (
    336,
    3,
    'recommend_order',
    'select',
    '0,1',
    '',
    '0',
    '1'
  ),
  (337, 3, 'index_ad', 'hidden', '', '', 'sys', '1'),
  (
    401,
    4,
    'can_invoice',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    402,
    4,
    'use_integral',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    403,
    4,
    'use_bonus',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    404,
    4,
    'use_surplus',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    405,
    4,
    'use_how_oos',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    406,
    4,
    'send_confirm_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    407,
    4,
    'send_ship_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    408,
    4,
    'send_cancel_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    409,
    4,
    'send_invalid_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    410,
    4,
    'order_pay_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    411,
    4,
    'order_unpay_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    412,
    4,
    'order_ship_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    413,
    4,
    'order_receive_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    414,
    4,
    'order_unship_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    415,
    4,
    'order_return_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    416,
    4,
    'order_invalid_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    417,
    4,
    'order_cancel_note',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    418,
    4,
    'invoice_content',
    'textarea',
    '',
    '',
    '',
    '1'
  ),
  (
    419,
    4,
    'anonymous_buy',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    420,
    4,
    'min_goods_amount',
    'text',
    '',
    '',
    '0',
    '1'
  ),
  (
    421,
    4,
    'one_step_buy',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    422,
    4,
    'invoice_type',
    'manual',
    '',
    '',
    'a:2:{s:4:"type";a:3:{i:0;s:1:"1";i:1;s:1:"2";i:2;s:0:"";}s:4:"rate";a:3:{i:0;d:1;i:1;d:1.5;i:2;d:0;}}',
    '1'
  ),
  (
    423,
    4,
    'stock_dec_time',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    424,
    4,
    'cart_confirm',
    'options',
    '1,2,3,4',
    '',
    '3',
    '0'
  ),
  (
    425,
    4,
    'send_service_email',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    426,
    4,
    'show_goods_in_cart',
    'select',
    '1,2,3',
    '',
    '3',
    '1'
  ),
  (
    427,
    4,
    'show_attr_in_cart',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    501,
    5,
    'smtp_host',
    'text',
    '',
    '',
    'localhost',
    '1'
  ),
  (502, 5, 'smtp_port', 'text', '', '', '25', '1'),
  (503, 5, 'smtp_user', 'text', '', '', '', '1'),
  (504, 5, 'smtp_pass', 'password', '', '', '', '1'),
  (505, 5, 'smtp_mail', 'text', '', '', '', '1'),
  (
    506,
    5,
    'mail_charset',
    'select',
    'UTF8,GB2312,BIG5',
    '',
    'UTF8',
    '1'
  ),
  (
    507,
    5,
    'mail_service',
    'select',
    '0,1',
    '',
    '0',
    '0'
  ),
  (
    508,
    5,
    'smtp_ssl',
    'select',
    '0,1',
    '',
    '0',
    '0'
  ),
  (
    601,
    6,
    'integrate_code',
    'hidden',
    '',
    '',
    'ecshop',
    '1'
  ),
  (
    602,
    6,
    'integrate_config',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (
    603,
    6,
    'hash_code',
    'hidden',
    '',
    '',
    '31693422540744c0a6b6da635b7a5a93',
    '1'
  ),
  (
    604,
    6,
    'template',
    'hidden',
    '',
    '',
    'default',
    '1'
  ),
  (
    605,
    6,
    'install_date',
    'hidden',
    '',
    '',
    '1224919217',
    '1'
  ),
  (
    606,
    6,
    'ecs_version',
    'hidden',
    '',
    '',
    'v2.7.2',
    '1'
  ),
  (
    607,
    6,
    'sms_user_name',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (
    608,
    6,
    'sms_password',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (
    609,
    6,
    'sms_auth_str',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (610, 6, 'sms_domain', 'hidden', '', '', '', '1'),
  (611, 6, 'sms_count', 'hidden', '', '', '', '1'),
  (
    612,
    6,
    'sms_total_money',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (613, 6, 'sms_balance', 'hidden', '', '', '', '1'),
  (
    614,
    6,
    'sms_last_request',
    'hidden',
    '',
    '',
    '',
    '1'
  ),
  (
    616,
    6,
    'affiliate',
    'hidden',
    '',
    '',
    'a:3:{s:6:"config";a:7:{s:6:"expire";d:24;s:11:"expire_unit";s:4:"hour";s:11:"separate_by";i:0;s:15:"level_point_all";s:2:"5%";s:15:"level_money_all";s:2:"1%";s:18:"level_register_all";i:2;s:17:"level_register_up";i:60;}s:4:"item";a:4:{i:0;a:2:{s:11:"level_point";s:3:"60%";s:11:"level_money";s:3:"60%";}i:1;a:2:{s:11:"level_point";s:3:"30%";s:11:"level_money";s:3:"30%";}i:2;a:2:{s:11:"level_point";s:2:"7%";s:11:"level_money";s:2:"7%";}i:3;a:2:{s:11:"level_point";s:2:"3%";s:11:"level_money";s:2:"3%";}}s:2:"on";i:1;}',
    '1'
  ),
  (617, 6, 'captcha', 'hidden', '', '', '36', '1'),
  (
    618,
    6,
    'captcha_width',
    'hidden',
    '',
    '',
    '100',
    '1'
  ),
  (
    619,
    6,
    'captcha_height',
    'hidden',
    '',
    '',
    '20',
    '1'
  ),
  (
    620,
    6,
    'sitemap',
    'hidden',
    '',
    '',
    'a:6:{s:19:"homepage_changefreq";s:6:"hourly";s:17:"homepage_priority";s:3:"0.9";s:19:"category_changefreq";s:6:"hourly";s:17:"category_priority";s:3:"0.8";s:18:"content_changefreq";s:6:"weekly";s:16:"content_priority";s:3:"0.7";}',
    '0'
  ),
  (621, 6, 'points_rule', 'hidden', '', '', '', '0'),
  (
    622,
    6,
    'flash_theme',
    'hidden',
    '',
    '',
    'dynfocus',
    '1'
  ),
  (623, 6, 'stylename', 'hidden', '', '', '', 1),
  (
    701,
    7,
    'show_goodssn',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    702,
    7,
    'show_brand',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    703,
    7,
    'show_goodsweight',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    704,
    7,
    'show_goodsnumber',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    705,
    7,
    'show_addtime',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    706,
    7,
    'goodsattr_style',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    707,
    7,
    'show_marketprice',
    'select',
    '1,0',
    '',
    '1',
    '1'
  ),
  (
    801,
    8,
    'sms_shop_mobile',
    'text',
    '',
    '',
    '',
    '1'
  ),
  (
    802,
    8,
    'sms_order_placed',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    803,
    8,
    'sms_order_payed',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    804,
    8,
    'sms_order_shipped',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    901,
    9,
    'wap_config',
    'select',
    '1,0',
    '',
    '0',
    '1'
  ),
  (
    902,
    9,
    'wap_logo',
    'file',
    '',
    '../images/',
    '',
    '1'
  ),
  (
    903,
    2,
    'message_check',
    'select',
    '1,0',
    '',
    '1',
    '1'
  );

--
-- user_rank
--
INSERT INTO `ecs_user_rank` (
    `rank_id`,
    `rank_name`,
    `min_points`,
    `max_points`,
    `discount`,
    `show_price`,
    `special_rank`
  )
VALUES (NULL, '註冊用戶', '0', '10000', '100', 1, 0);

-- 文章默認分類
INSERT INTO `ecs_article_cat` (
    `cat_id`,
    `cat_name`,
    `cat_type`,
    `keywords`,
    `cat_desc`,
    `sort_order`,
    `parent_id`
  )
VALUES (1, '系統分類', 2, '', '系統保留分類', 50, 0);

INSERT INTO `ecs_article_cat` (
    `cat_id`,
    `cat_name`,
    `cat_type`,
    `keywords`,
    `cat_desc`,
    `sort_order`,
    `parent_id`
  )
VALUES (2, '網店信息', 3, '', '網店信息分類', 50, 1);

INSERT INTO `ecs_article_cat` (
    `cat_id`,
    `cat_name`,
    `cat_type`,
    `keywords`,
    `cat_desc`,
    `sort_order`,
    `parent_id`
  )
VALUES (3, '網店幫助分類', 4, '', '網店幫助分類', 50, 1);

--
-- `ecs_article`
--
INSERT INTO `ecs_article` (
    `article_id`,
    `cat_id`,
    `title`,
    `content`,
    `author`,
    `author_email`,
    `keywords`,
    `article_type`,
    `is_open`,
    `add_time`,
    `file_url`,
    `open_type`
  )
VALUES (
    1,
    2,
    '免責條款',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  ),
  (
    2,
    2,
    '隱私保護',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  ),
  (
    3,
    2,
    '咨詢熱點',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  ),
  (
    4,
    2,
    '聯系我們',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  ),
  (
    5,
    2,
    '公司簡介',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  ),
  (
    6,
    -1,
    '用戶協議',
    '',
    '',
    '',
    '',
    0,
    1,
    UNIX_TIMESTAMP(),
    '',
    0
  );

--
-- `ecs_template`
--
INSERT INTO `ecs_template` (
    `filename`,
    `region`,
    `library`,
    `sort_order`,
    `id`,
    `number`,
    `type`,
    `theme`,
    `remarks`
  )
VALUES (
    'index',
    '左邊區域',
    '/library/vote_list.lbi',
    8,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/email_list.lbi',
    9,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/order_query.lbi',
    6,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/cart.lbi',
    0,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/promotion_info.lbi',
    3,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/auction.lbi',
    4,
    0,
    3,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/group_buy.lbi',
    5,
    0,
    3,
    0,
    'default',
    ''
  ),
  (
    'index',
    '',
    '/library/recommend_promotion.lbi',
    0,
    0,
    4,
    0,
    'default',
    ''
  ),
  (
    'index',
    '右邊主區域',
    '/library/recommend_hot.lbi',
    2,
    0,
    10,
    0,
    'default',
    ''
  ),
  (
    'index',
    '右邊主區域',
    '/library/recommend_new.lbi',
    1,
    0,
    10,
    0,
    'default',
    ''
  ),
  (
    'index',
    '右邊主區域',
    '/library/recommend_best.lbi',
    0,
    0,
    10,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/invoice_query.lbi',
    7,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/top10.lbi',
    2,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '左邊區域',
    '/library/category_tree.lbi',
    1,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'index',
    '',
    '/library/brands.lbi',
    '0',
    '0',
    '11',
    '0',
    'default',
    ''
  ),
  (
    'category',
    '左邊區域',
    '/library/category_tree.lbi',
    1,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'category',
    '右邊區域',
    '/library/recommend_best.lbi',
    0,
    0,
    5,
    0,
    'default',
    ''
  ),
  (
    'category',
    '右邊區域',
    '/library/goods_list.lbi',
    1,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'category',
    '右邊區域',
    '/library/pages.lbi',
    2,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'category',
    '左邊區域',
    '/library/cart.lbi',
    0,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'category',
    '左邊區域',
    '/library/price_grade.lbi',
    3,
    0,
    0,
    0,
    'default',
    ''
  ),
  (
    'category',
    '左邊區域',
    '/library/filter_attr.lbi',
    2,
    0,
    0,
    0,
    'default',
    ''
  );

--
-- 會員註冊項 ecs_reg_fields
--
INSERT INTO `ecs_reg_fields` (
    `id`,
    `reg_field_name`,
    `dis_order`,
    `display`,
    `type`,
    `is_need`
  )
VALUES (1, 'MSN', 0, 1, 1, 1),
  (2, 'QQ', 0, 1, 1, 1),
  (3, '辦公電話', 0, 1, 1, 1),
  (4, '家庭電話', 0, 1, 1, 1),
  (5, '手機', 0, 1, 1, 1),
  (6, '密碼找回問題', 0, 1, 1, 1);