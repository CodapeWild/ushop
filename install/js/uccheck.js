...
function setupUCenter() {
  ...
  if (f["js-ucip"] && f["js-ucip"].value.length > 1) {
    var params =
      "ucapi=" +
      encodeURIComponent(f["js-ucapi"].value) +
      "&" +
      "ucfounderpw=" +
      encodeURIComponent(f["js-ucfounderpw"].value) +
      "&" +
      "ucip=" +
      encodeURIComponent(f["js-ucip"].value);
  } else {
    var params =
      "ucapi=" +
      encodeURIComponent(f["js-ucapi"].value) +
...
function displayres(res) {
  if (res.error !== 0) {
    ...
    if (res.error == 2) {
      ...
      td2.innerHTML =
        '<input name="js-ucip" type="text" id="js-ucip"  value="" size="40" /><span id="ucipnotice" style="color:#FF0000">连接的过程中出了点问题，请您填写服务器 IP 地址，如果您的 UC 与 ECShop 装在同一服务器上，我们建议您尝试填写 127.0.0.1</span>';
      $("ucip").appendChild(td1);
      $("ucip").appendChild(td2);
    }
  } else {
    var ui =
      $("user_interface_1").checked === true
        ? $("user_interface_1").value
        : $("user_interface_2").value;
    location.href =
      "index.php?lang=" + getAddressLang() + "&step=check" + "&ui=" + ui;
  }
}
