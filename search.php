...
/*------------------------------------------------------ */ else {
  ...
  if (!empty($_REQUEST['keywords'])) {
    ...
    foreach ($arr as $key => $val) {
      ...
      $db->autoReplace($ecs->table('keywords'), array(
        ...
        'searchengine' => 'ecshop', 'keyword' => addslashes(str_replace('%', '', $val)), 'count' => 1
      ), array('count' => 1));
    }
    $keywords .= ')';
  ...
  $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, " .
    ...
    "g.promote_price, g.promote_start_date, g.wide_href,g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type " .
  ...
  while ($row = $db->FetchRow($res)) {
    ...
    $arr[$row['goods_id']]['wide_href']          = $row['wide_href'];
