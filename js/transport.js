...
var Transport = {
  ...
  run: function (
    ...
    transferMode =
      typeof transferMode === "string" && transferMode.toUpperCase() === "GET"
        ? "GET"
        : "POST";
            ...
            switch (xhr.status) {
              ...
              case 200: // OK!
                ...
                if (typeof callback === "function") {
                  callback.call(
                    self,
                    self.parseResult(responseType, xhr),
                    xhr.responseText
                  );
                }
                break;
              ...
              default:
                alert(
                  "XmlHttpRequest status: [" + xhr.status + "] Unknow status."
                );
            }
  ...
  parseResult: function (responseType, xhr) {
    ...
    if (this.debugging.isDebugging) {
      ...
      if (responseType === "JSON") {
        info =
          "[Response Result of TEXT Format]" +
          lf +
          xhr.responseText +
          lf +
          lf +
...
function showLoader() {
  ...
  if (
    top.frames["header-frame"] &&
    top.frames["header-frame"].document.getElementById("load-div")
  ) {
    top.frames["header-frame"].document.getElementById(
      "load-div"
    ).style.display = "block";
  } else {
