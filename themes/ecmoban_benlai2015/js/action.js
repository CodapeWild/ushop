...
function reg(str) {
  ...
  for (var i = 0; i < bt.length; i++) {
    ...
    bt[i].onclick = function () {
      $(this.subj + "_v").innerHTML = $(this.subj + "_h").getElementsByTagName(
        "blockquote"
      )[this.pai].innerHTML;
      for (
        var j = 0;
        j < $(this.subj + "_b").getElementsByTagName("h2").length;
        j++
      ) {
  ...
  $(str + "_v").innerHTML = $(str + "_h").getElementsByTagName(
    "blockquote"
  )[0].innerHTML;
}
  ...
  for (var i = 0; i < imgnum.length; i++) {
    imgnum[i].onclick = function () {
      ...
      for (var j = 0; j < imgnum.length; j++) {
        if (
          imgnum[j].getAttribute("class") == "onbg" ||
          imgnum[j].getAttribute("className") == "onbg"
        ) {
...
function colorStyle(id, color1, color2) {
  ...
  if (elem.getAttribute("id") == id) {
    ...
    if (elem.className == color1) elem.className = color2;
    else elem.className = color1;
  }
}
  ...
  for (var i = 0; i < id.length; i++) {
    ...
    id[i].onmouseover = function () {
      ...
      for (var j = 0; j < id.length; j++) {
        if (
          id[j].getAttribute("class") == cur ||
          id[j].getAttribute("className") == cur
        ) {
          id[j].className = "";
          break;
        }
