...
MagicZoom.prototype.showrect = function () {
  ...
  if (document.documentElement.dir == "rtl") {
    perX =
      (this.positionX + this.popupSizeX / 2 - this.smallImageSizeX) *
  ...
  if (this.bigImageSizeY > parseInt(this.bigImageCont.style.height) - headerH) {
    if (
      this.bigImageSizeY - perY <
      parseInt(this.bigImageCont.style.height) - headerH
    ) {
      perY =
        this.bigImageSizeY - parseInt(this.bigImageCont.style.height) + headerH;
    }
  }
...
MagicZoom.prototype.recalculatePopupDimensions = function () {
  ...
  if (this.settings && this.settings["header"] != "") {
    ...
    this.popupSizeY =
      parseInt(this.bigImageCont.style.height) /
...
MagicZoom.prototype.initPopup = function () {
  ...
  this.pup.style["-html-opacity"] = parseFloat(
    this.settings["opacity"] / 100.0
  );
...
MagicZoom.prototype.initBigContainer = function () {
  ...
  if ("undefined" !== typeof gd56f7fsgd) {
    ...
    f.style.left = "0px";
    f.style.top =
      parseInt(this.bigImageCont.style.height) - gd56f7fsgd[6] + "px";
    this.bigImageCont.appendChild(f);
  }
};
MagicZoom.prototype.initZoom = function () {
  if (
    this.loadingCont != null &&
    (!this.bigImage.complete ||
      0 == this.bigImage.width ||
      0 == this.bigImage.height) &&
    this.smallImage.width != 0 &&
    this.smallImage.height != 0
  ) {
    this.loadingCont.style.left =
      parseInt(this.smallImage.width) / 2 -
      parseInt(this.loadingCont.offsetWidth) / 2 +
      "px";
    this.loadingCont.style.top =
      parseInt(this.smallImage.height) / 2 -
      parseInt(this.loadingCont.offsetHeight) / 2 +
      "px";
  ...
  if (
    ...
    this.smallImageSizeX -= parseInt(
      MagicZoom_getStyle(this.smallImage, "paddingLeft")
    );
    this.smallImageSizeX -= parseInt(
      MagicZoom_getStyle(this.smallImage, "paddingRight")
    );
    this.smallImageSizeY -= parseInt(
      MagicZoom_getStyle(this.smallImage, "paddingTop")
    );
    this.smallImageSizeY -= parseInt(
      MagicZoom_getStyle(this.smallImage, "paddingBottom")
    );
  }
  ...
  if (!r) {
    this.bigImageCont.style.left =
      this.smallImageSizeX +
      parseInt(MagicZoom_getStyle(this.smallImage, "borderLeftWidth")) +
      parseInt(MagicZoom_getStyle(this.smallImage, "borderRightWidth")) +
  ...
  if (
    this.settings &&
    (this.settings["drag_mode"] == true ||
      this.settings["bigImage_always_visible"] == true)
  ) {
...
(function () {
  window.MagicTools = {
    ...
    bind: function () {
      ...
      return function () {
        return __method.apply(
          object,
          MagicTools.concat(args, MagicTools.$A(arguments))
        );
      };
    },
    bindAsEvent: function () {
      ...
      return function (event) {
        return __method.apply(
          object,
          MagicTools.concat([event || window.event], args)
        );
      };
    },
    ...
    getPageSize: function () {
      ...
      var ieBody = !MagicTools.browser.backCompatMode
        ? document.documentElement
        : document.body;
      ...
      xScroll =
        window.innerWidth && window.scrollMaxX
          ...
          ? body.scrollWidth
          : MagicTools.browser.ie && MagicTools.browser.backCompatMode
          ? body.scrollWidth
          : body.offsetWidth;
      yScroll =
      ...
      (windowWidth = MagicTools.browser.ie
        ? ieBody.scrollWidth
        : document.documentElement.clientWidth || self.innerWidth),
        (windowHeight = MagicTools.browser.ie
          ? ieBody.clientHeight
      ...
      return {
        ...
        width: MagicTools.browser.ie
          ? ieBody.clientWidth
          : document.documentElement.clientWidth || self.innerWidth,
        height: MagicTools.browser.ie
          ? ieBody.clientHeight
          : MagicTools.browser.opera
          ? self.innerHeight
    ...
    Element: {
      ...
      removeClass: function (el, klass) {
        ...
        el.className = MagicTools.String.trim(
      ...
      getStyle: function (el, style) {
        el = MagicTools.$(el);
        style =
          style == "float" ? "cssFloat" : MagicTools.String.camelize(style);
      ...
      setStyle: function (el, styles) {
        ...
        for (var s in styles) {
          try {
            ...
            if ("float" === s) {
              elStyle[
                "undefined" === typeof elStyle.styleFloat
                  ? "cssFloat"
                  : "styleFloat"
              ] = styles[s];
              continue;
            }
            elStyle[MagicTools.String.camelize(s)] =
              styles[s] + addpx(MagicTools.String.camelize(s), styles[s]);
          } catch (e) {}
        }
        return el;
      },
  ...
  MagicTools.Render.prototype = {
    ...
    init: function (el, opt) {
      this.el = el;
      this.options = MagicTools.extend(
        MagicTools.extend({}, this.defaults),
        opt
      );
      this.timer = false;
    },
    ...
    start: function (styles) {
      ...
      this.timer = setInterval(
        MagicTools.bind(this.loop, this),
    ...
    loop: function () {
      ...
      var dx = this.options.transition(
        (now - this.startTime) / (this.options.duration * 1000)
      );
      this.render(dx);
    },
    render: function (dx) {
...
var MagicThumb = {
  ...
  init: function (refresh) {
    ...
    var matches = /(auto|center|absolute|relative)/i.exec(
    ...
    this.options.zoomTrigger = /mouseover/i.test(this.options.zoomTrigger)
      ? "mouseover"
      : "click";
    ...
    for (var i = 0; i < l; i++) {
      if (MagicTools.Element.hasClass(as[i], "MagicThumb")) {
        MagicThumb.thumbs.push(
          new MagicThumb.Item(as[i], null, thumbIndex++, {
            expandDuration: this.options.zoomDuration || this.options.duration,
            collapseDuration:
              this.options.restoreDuration || this.options.duration,
  ...
  stop: function () {
    for (
      var t = MagicThumb.thumbs.pop();
      t != null && undefined != t;
      t = MagicThumb.thumbs.pop()
    ) {
      t.destroy();
      delete t;
    }
  ...
  expand: function (e, idx) {
    ...
    if (
      !MagicThumb.options.allowMultipleImages &&
      undefined != t &&
      idx != t.index
    ) {
  ...
  getFocused: function () {
    return this.activeIndexes.length > 0
      ? this.getItem(this.activeIndexes[this.activeIndexes.length - 1])
      : undefined;
  },
  ...
  getNext: function (group, repeat) {
    ...
    return pos >= items.length
      ? !repeat
        ? undefined
        : MagicThumb.getItem(items[0])
      : MagicThumb.getItem(items[pos]);
  },
  getPrev: function (group, repeat) {
    ...
    return pos < 0
      ? !repeat
        ? undefined
        : MagicThumb.getItem(items[items.length - 1])
      : MagicThumb.getItem(items[pos]);
  },
  ...
  fadeInBackground: function () {
    if (
      MagicThumb.bgFader &&
      "none" != MagicTools.Element.getStyle(MagicThumb.bgFader, "display")
    ) {
      return;
    }
...
MagicThumb.Item.prototype = {
  init: function (a, group, idx, opt) {
    ...
    this.loader.appendChild(
      document.createTextNode(MagicThumb.options.loadingMsg)
    );
  ...
  destroy: function () {
    ...
    for (
      var c = this.eventsCache.pop();
      c != null && undefined != c;
      c = this.eventsCache.pop()
    ) {
    ...
    if (
      MagicTools.inArray(
        this.loader,
        MagicTools.$A(document.body.getElementsByTagName(this.loader.tagName))
      )
    ) {
    ...
    if (!this.zoomed) {
      if (
        MagicTools.inArray(
          this.bigImg,
          MagicTools.$A(document.body.getElementsByTagName(this.bigImg.tagName))
        )
      ) {
    ...
    if (
      MagicTools.inArray(
        this.cont,
        MagicTools.$A(document.body.getElementsByTagName(this.cont.tagName))
      )
    ) {
  ...
  createControlBar: function () {
    ...
    var icons = [];
    var buttons =
      this.options.controlbarButtons || MagicThumb.options.controlbarButtons;
    ...
    for (var i = 0; i < cbLength; i++) {
      ...
      var w =
        -cbBtn.index * parseInt(MagicTools.Element.getStyle(cbA, "width"));
      ...
      bgIMG.src = MagicTools.Element.getStyle(
        cbBgWrapper,
        "background-image"
      ).replace(/url\s*\(\s*\"{0,1}([^\"]*)\"{0,1}\s*\)/i, "$1");
      if (MagicTools.browser.ie6) {
        var bgURL = MagicTools.Element.getStyle(
          cbBgWrapper,
          "background-image"
        );
      ...
      if (
        "close" == cbA.rel &&
        /left/i.test(
          this.options.controlbarPosition ||
  ...
  prepare: function () {
    ...
    if (!this.smallImg) {
      ...
      this.smallImg.src =
        "data:image/gif;base64,R0lGODlhAQABAIAAACqk1AAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==";
      MagicTools.Element.setStyle(this.smallImg, {
        width: 0,
        height: 0,
        opacity: 0,
    ...
    if (
      "img:alt" == this.options.captionSrc.toLowerCase() &&
      "" != (this.smallImg.alt || "")
    ) {
      ...
      "img:title" == this.options.captionSrc.toLowerCase() &&
      "" != (this.smallImg.title || "")
    ) {
      ...
      this.hasCaption = true;
      this.caption.innerHTML = formatCaptionText(
        this.anchor
    ...
    MagicTools.extend(this.caption, {
      paddingLeft: parseInt(
        MagicTools.Element.getStyle(this.caption, "padding-left")
      ),
      paddingRight: parseInt(
        MagicTools.Element.getStyle(this.caption, "padding-right")
      ),
    });
    ...
    MagicTools.Element.setStyle(this.caption, {
      width:
        this.bigImg.completeWidth -
        this.caption.paddingLeft -
        this.caption.paddingRight -
        parseInt(
          MagicTools.Element.getStyle(this.bigImg, "border-left-width")
        ) -
        parseInt(
          MagicTools.Element.getStyle(this.bigImg, "border-right-width")
        ) -
        parseInt(
          MagicTools.Element.getStyle(this.caption, "border-left-width")
        ) -
        parseInt(
          MagicTools.Element.getStyle(this.caption, "border-right-width")
        ),
      "padding-left":
        this.caption.paddingLeft +
        parseInt(MagicTools.Element.getStyle(this.bigImg, "border-left-width")),
      "padding-right":
        this.caption.paddingRight +
        parseInt(
          MagicTools.Element.getStyle(this.bigImg, "border-right-width")
    ...
    if (
      true ===
      (this.options.controlbarEnable || MagicThumb.options.controlbarEnable)
    ) {
      this.createControlBar();
      this.addEvent(
        this.cont,
        "mouseover",
        MagicTools.bindAsEvent(this.toggleControlBar, this, true)
      );
      this.addEvent(
        this.cont,
        "mouseout",
        MagicTools.bindAsEvent(this.toggleControlBar, this)
      );
    }
    MagicTools.Element.setStyle(this.cont, {
      display: "none",
    ...
    if ("mouseover" == this.options.zoomTrigger) {
      this.addEvent(
        this.anchor,
        "mouseover",
        MagicTools.bindAsEvent(function (e) {
          MagicTools.Event.stop(e);
          this.hoverTimer = setTimeout(
            MagicTools.bind(MagicThumb.expand, MagicThumb, null, this.index),
            this.options.zoomTriggerDelay * 1000
          );
          this.addEvent(
            this.anchor,
            "mouseout",
            MagicTools.bindAsEvent(function () {
              MagicTools.Event.stop(e);
              if (this.hoverTimer) {
      ...
      this.addEvent(
        this.anchor,
        "click",
        MagicTools.bindAsEvent(MagicThumb.expand, MagicThumb, this.index)
      );
    }
    this.loaded = true;
    document.body.removeChild(this.loader);
  ...
  adjustPosition: function (ps) {
    var padW =
        parseInt(MagicTools.Element.getStyle(this.cont, "padding-left")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "padding-right")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "border-left-width")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "border-right-width")),
      padH =
        parseInt(MagicTools.Element.getStyle(this.cont, "padding-top")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "padding-bottom")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "border-top-width")) +
        parseInt(MagicTools.Element.getStyle(this.cont, "border-bottom-width"));
    ...
    if ("center" == this.options.zoomPosition) {
      ...
      destLeft = Math.round(
        (ps.width - padW) / 2 + ps.scrollX - imgSize.width / 2
      );
    ...
    if ("auto" == this.options.zoomPosition) {
      ...
      if (
        destTop + imgSize.height + this.caption.fullHeight >
      ...
      destLeft = Math.round(
        sRect.right - (sRect.right - sRect.left) / 2 - imgSize.width / 2
      );
      ...
      if (this.cr) {
        ...
        MagicTools.Element.setStyle(this.cr, {
          ...
          top:
            ...
            MagicTools.Element.getSize(this.cr).height -
            8,
        });
      }
    }
      ...
      onComplete: MagicTools.bind(function () {
        ...
        if (this.controlbar) {
          ...
          MagicTools.Element.setStyle(this.controlbar, {
            ...
            top: /bottom/i.test(
              ...
              ? imgSize.height - cbSize.height - 5
              : 5,
            left: /right/i.test(
              this.options.controlbarPosition ||
                MagicThumb.options.controlbarPosition
            )
              ? imgSize.width - cbSize.width - 5
              : 5,
          });
          if (MagicTools.browser.ie6) {
            MagicTools.Element.setStyle(this.cbOverlay, {
              ...
              "background-position":
                "" +
                (MagicTools.Element.getPosition(this.cont).left -
                  MagicTools.Element.getPosition(this.controlbar).left +
                  parseInt(
                    MagicTools.Element.getStyle(
                      this.bigImg,
                      "border-left-width"
                    )
                  )) +
                "px " +
                (MagicTools.Element.getPosition(this.cont).top -
                  MagicTools.Element.getPosition(this.controlbar).top +
                  parseInt(
                    MagicTools.Element.getStyle(this.bigImg, "border-top-width")
                  )) +
                "px",
            });
          }
        ...
        if (this.firstRun) {
          ...
          this.addEvent(
            this.bigImg,
            "click",
            (this.collapseEvent = MagicTools.bindAsEvent(this.collapse, this))
          );
        }
        if ("" != this.caption.innerHTML) {
  ...
  collapse: function (e, nextThumb, hide) {
    ...
    new MagicTools.Render(this.caption, {
      duration:
        !this.hasCaption || hide ? 0 : this.options.captionSlideDuration,
      transition: MagicTools.Transition.sin,
      onStart: MagicTools.bind(function () {
        MagicTools.Element.setStyle(this.caption, {
          "margin-top": 0,
  ...
  toggleControlBar: function (e, show) {
    ...
    var eX =
      e.clientX +
      parseInt(self.pageXOffset ? self.pageXOffset : ieBody.scrollLeft);
    var eY =
      e.clientY +
      parseInt(self.pageYOffset ? self.pageYOffset : ieBody.scrollTop);
  ...
  resizeImage: function (ps) {
    ...
    var x = Math.min(this.bigImg.displayWidth, ps.width - 35 - padW),
      y = Math.min(
        this.bigImg.displayHeight,
        ps.height - 35 - padH - this.caption.fullHeight
      );
