...
oRegex.PopupUri =
  /^javascript:void\(\s*window.open\(\s*'([^']+)'\s*,\s*(?:'([^']*)'|null)\s*,\s*'([^']*)'\s*\)\s*\)\s*$/;

// Accessible popups
oRegex.OnClickPopup =
  /^\s*on[cC]lick="\s*window.open\(\s*this\.href\s*,\s*(?:'([^']*)'|null)\s*,\s*'([^']*)'\s*\)\s*;\s*return\s*false;*\s*"$/;
...
oParser.CreateEMailUri = function (address, subject, body) {
  ...
  switch (FCKConfig.EMailProtection) {
    ...
    case "encode":
      ...
      if (body.length > 0) aParams.push("body=" + encodeURIComponent(body));
