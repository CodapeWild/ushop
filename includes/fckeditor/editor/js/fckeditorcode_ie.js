  ...
  if (!FCKBrowserInfo.IsIE && FCKDomTools.PaddingNode) {
    ...
    if (C && C.nodeName.IEquals("body")) {
      if (
        ...
        if (
          FCKSelection._GetSelectionDocument(FCK.EditorDocument.selection) !=
          FCK.EditorDocument
        )
          return;
...
var FCKDomTools = {
  ...
  ScrollIntoView: function (A, B) {
    ...
    if (B === false) {
      E += A.offsetHeight || 0;
      E +=
        parseInt(this.GetCurrentElementStyle(A, "marginBottom") || 0, 10) || 0;
    }
...
function InitializeAPI() {
  ...
  if (!(FCKeditorAPI = A.FCKeditorAPI)) {
    var B =
      'window.FCKeditorAPI = {Version : "2.6.3",VersionBuild : "19836",Instances : new Object(),GetInstance : function( name ){return this.Instances[ name ];},_FormSubmit : function(){for ( var name in FCKeditorAPI.Instances ){var oEditor = FCKeditorAPI.Instances[ name ] ;if ( oEditor.GetParentForm && oEditor.GetParentForm() == this )oEditor.UpdateLinkedField() ;}this._FCKOriginalSubmit() ;},_FunctionQueue	: {Functions : new Array(),IsRunning : false,Add : function( f ){this.Functions.push( f );if ( !this.IsRunning )this.StartNext();},StartNext : function(){var aQueue = this.Functions ;if ( aQueue.length > 0 ){this.IsRunning = true;aQueue[0].call();}else this.IsRunning = false;},Remove : function( f ){var aQueue = this.Functions;var i = 0, fFunc;while( (fFunc = aQueue[ i ]) ){if ( fFunc == f )aQueue.splice( i,1 );i++ ;}this.StartNext();}}}';
...
var FCKRegexLib = {
  AposEntity: /&apos;/gi,
  ObjectElements:
    /^(?:IMG|TABLE|TR|TD|TH|INPUT|SELECT|TEXTAREA|HR|OBJECT|A|UL|OL|LI)$/i,
  NamedCommands:
  ...
  InvalidSelfCloseTags:
    /(<(?!base|meta|link|hr|br|param|img|area|input)([a-zA-Z0-9:]+)[^>]*)\/>/gi,
...
var FCKLanguageManager = (FCK.Language = {
  ...
  Initialize: function () {
    ...
    this.ActiveLanguage.Name =
      this.AvailableLanguages[this.ActiveLanguage.Code];
  },
});
      ...
      if (!K) {
        ...
        if (F[M] && (!FCKBrowserInfo.IsIE || H.scopeName == "HTML")) {
          ...
          if (B) {
            B.SetEnd(H, 3, true);
            if (M != "br")
              this._NextNode = FCKDomTools.GetNextSourceNode(H, true, null, I);
          }
          J = true;
        } else {
          if (H.firstChild) {
...
FCKSelection.Restore = function () {
  if (this.SelectionData) {
    ...
    try {
      if (
        String(
          this._GetSelectionDocument(FCK.EditorDocument.selection).body
            .contentEditable
        ) == "true"
      ) {
...
FCKToolbarStyleCombo.prototype.RefreshActiveItems = function (A) {
  var B =
    FCK.ToolbarSet.CurrentInstance.Selection.GetBoundaryParentElement(true);
  if (B) {
    var C = new FCKElementPath(B);
    var D = C.Elements;
