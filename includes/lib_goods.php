...
function get_recommend_goods($type = '', $cats = '')
{
  ...
  if (empty($type_goods[$type])) {
    ...
    if ($data === false) {
      $sql = 'SELECT g.goods_id, g.wide_href,g.is_best, g.wide_href,g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
        ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.wide_href, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
    ...
    foreach ($result as $idx => $row) {
      ...
      $goods[$idx]['wide_href']         = $row['wide_href'];
...
function get_goods_info($goods_id)
{
  ...
  if ($row !== false) {
    ...
    $row['goods_weight']  = (intval($row['goods_weight']) > 0) ?
      $row['goods_weight'] . $GLOBALS['_LANG']['kilogram'] : ($row['goods_weight'] * 1000) . $GLOBALS['_LANG']['gram'];
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
  ...
  $sql = 'SELECT g.goods_id, g.goods_name, g.wide_href,g.market_price, g.shop_price AS org_price, ' .
    "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " .
