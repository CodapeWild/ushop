...
function check_file_type($filename, $realname = '', $limit_ext_types = '')
{
  ...
  if ($file) {
    ...
    if (stristr($filename, ROOT_PATH) === false) {
      if (
        $extname == 'jpg' || $extname == 'jpeg' || $extname == 'gif' || $extname == 'png' || $extname == 'doc' ||
...
function ecs_geoip($ip)
{
  ...
  for ($start = $start['len'] * 8 + 1024; $start < $length; $start += 8) {
    if ($index{
      ...
      $start + 2} . $index{
      $start + 3} >= $ip) {
      $index_offset = unpack('Vlen', $index{
        $start + 4} . $index{
        $start + 5} . $index{
        $start + 6} . "\x0");
      $index_length = unpack('Clen', $index{
        $start + 7});
      break;
    }
  }
...
{
  $aryTemp = explode('.', $file_name);
  $aryTemp = array_pop($aryTemp);
  $file_suffix = strtolower($aryTemp);
  if (empty($allow_type)) {
    return $file_suffix;
  } else {
