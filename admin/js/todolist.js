...
Todolist.prototype = {
  initialize: function (adminid) {
    ...
    this.bottomLeft.innerHTML +=
      '<label><input type="checkbox" value="1" name="remember" />' +
      todolist_autosave +
      "</label>";
    ...
    this.autoSaveCheckbox.checked =
      document.getCookie("todolist_notautosave_" + this.adminid) == null
        ? true
        : false;
    ...
    window.onbeforeunload = function () {
      ...
      if (confirm(todolist_confirm_save)) {
        Ajax.call(
          "index.php?is_ajax=1&act=save_todolist",
          "content=" + encodeURIComponent(self.textBox.value),
          null,
          "POST",
          "TEXT",
          false
  ...
  loadData: function () {
    Ajax.call(
      "index.php?is_ajax=1&act=get_todolist",
      "",
      this.loadDataResponse.bind(this),
      "POST",
      "TEXT"
    );
  },
  ...
  endDrag: function () {
    ...
    document.setCookie(
      "todolist_position_" + this.adminid,
      "({X:'" + this.box.style.left + "',Y:'" + this.box.style.top + "'})",
      date.toGMTString()
    );
  ...
  show: function () {
    ...
    if (
      parseInt(this.box.style.top) + parseInt(this.box.offsetHeight) >
      this.innerHeight
    ) {
  ...
  setSize: function (event) {
    ...
    var height =
      tY -
      (parseInt(pos.top) - this.box.offsetHeight) -
      parseInt(this.box.offsetHeight);
    var width =
      tX -
      (parseInt(pos.left) - this.box.offsetWidth) -
      parseInt(this.box.offsetWidth);
    if (height >= 95) this.virtualBox.style.height = height + "px";
    if (width >= 301) this.virtualBox.style.width = width + "px";
  },
  ...
  endSetSize: function (event) {
    ...
    this.textBox.style.height =
      parseInt(this.box.style.height) -
      parseInt(this.head.offsetHeight) -
      parseInt(this.bottom.offsetHeight) +
      "px";
    if (this.container.offsetHeight > this.box.offsetHeight) {
      var span = this.container.offsetHeight - this.box.offsetHeight;
      this.textBox.style.height =
        parseInt(this.textBox.style.height) - span - 4 + "px";
    }
    this.virtualBox.style.display = "none";
  },
  ...
  autoSave: function () {
    if (this.autoSaveCheckbox.checked) {
      ...
      date.setTime(date.getTime() + 99999999);
      document.setCookie(
        "todolist_notautosave_" + this.adminid,
        "1",
        date.toGMTString()
      );
