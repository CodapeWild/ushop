    ...
    if (Utils.trim(txt.value).length > 0) {
      res = Ajax.call(
        ...
        "act=" +
          act +
          "&val=" +
          encodeURIComponent(Utils.trim(txt.value)) +
          "&id=" +
          id,
        null,
        "POST",
      ...
      if (res.id && (res.act == "goods_auto" || res.act == "article_auto")) {
        document.getElementById("del" + res.id).innerHTML =
          '<a href="' +
          thisfile +
          "?goods_id=" +
          res.id +
          '&act=del" onclick="return confirm(\'' +
          deleteck +
          "');\">" +
          deleteid +
          "</a>";
...
listTable.compileFilter = function () {
  ...
  for (var i in this.filter) {
    if (
      typeof this.filter[i] != "function" &&
      typeof this.filter[i] != "undefined"
    ) {
...
listTable.addRow = function (checkFunc) {
  ...
  for (var i = 0; i < firstRow.cells.length; i++) {
    ...
    if (
      !cel.getAttribute("ReadOnly") &&
      cel.getAttribute("Type") == "TextBox"
    ) {
    ...
    if (cel.getAttribute("Type") == "Button") {
      ...
      this.saveFunc = function () {
        ...
        if (res.error) {
          ...
          if (
            document.getElementById("listDiv").childNodes[0].rows.length < 6
          ) {
