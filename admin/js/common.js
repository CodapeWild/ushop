...
function checkOrder() {
  ...
  if (lastCheckOrder == null || today - lastCheckOrder >= NEW_ORDER_INTERVAL) {
    ...
    try {
      Ajax.call(
        "index.php?is_ajax=1&act=check_order",
        "",
        checkOrderResponse,
        "GET",
        "JSON"
...
Message.show = function () {
  try {
    ...
    Message.bottom =
      0 - parseInt(document.getElementById(Message.elem).offsetHeight);
...
listZone.toggleLoader = function (disp) {
  document.getElementsByTagName("body").item(0).style.cursor = disp
    ? "wait"
    : "auto";
