...
function selectShipping(obj) {
  ...
  for (i = 0; i < theForm.elements.length; i++) {
    if (
      theForm.elements[i].name == "payment" &&
...
function changeNeedInv() {
  ...
  Ajax.call(
    ...
    "need_inv=" +
      needInv +
      "&inv_type=" +
      encodeURIComponent(invType) +
      "&inv_payee=" +
      encodeURIComponent(invPayee) +
      "&inv_content=" +
      encodeURIComponent(invContent),
    orderSelectedResponse,
    "GET"
  );
}
...
function checkConsignee(frm) {
  ...
  if (
    frm.elements["zipcode"] &&
    frm.elements["zipcode"].value.length > 0 &&
