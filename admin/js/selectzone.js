...
function SelectZone() {
  ...
  this.loadOptions = function (act, filters) {
    Ajax.call(
      this.filename + "&act=" + act,
      filters,
      this.loadOptionsResponse,
      "GET",
      "JSON"
    );
  };
  ...
  this.addItem = function (all, act) {
    ...
    for (var i = 0; i < this.sourceSel.length; i++) {
      ...
      if (this.targetSel.length > 0) {
        ...
        for (var j = 0; j < this.targetSel.length; j++) {
          if (
            this.targetSel.options[j].value == this.sourceSel.options[i].value
          ) {
            exsits = true;
    ...
    if (selOpt.length > 0) {
      ...
      Ajax.call(
        this.filename + "&act=" + act + "&add_ids=" + selOpt.toJSONString(),
        args,
        this.addRemoveItemResponse,
        "GET",
        "JSON"
      );
    }
  ...
  this.dropItem = function (all, act) {
    ...
    if (arr.length > 0) {
      ...
      Ajax.call(
        this.filename + "&act=" + act + "&drop_ids=" + arr.toJSONString(),
        args,
        this.addRemoveItemResponse,
        "GET",
        "JSON"
